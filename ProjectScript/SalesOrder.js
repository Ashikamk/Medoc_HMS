...
$(document).ready(function () {
    ...
    $("#btnordersaveedit").click(function (e) {
        // $('.tool_tip').tooltip({ trigger: 'manual' }).tooltip('hide');
...
function SaveAndUpdateOrder(flg) {
    debugger;
    if (flg == 1) { $('#btnordersubmit').prop('disabled', true); }
    else if (flg == 2) { $('#btnordersaveedit').prop('disabled', true); }
    ...
    for (var i = 1; i <= RowCount; i++) {
        if ($('#Product' + i).val() != undefined) {
            ...
            // Validate inputs
            if (!name || !mobile || !salesMan || !dueDate) {
                alert("Please fill all fileds in Patient Details and Optical Prescription forms");
                if (flg == 1) {
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesOrderInvoiceInsert",
                data: data,
                success: function (result) {
                    $('#btnordersubmit').prop('disabled', false);
                    if (result.oList.length > 0) {
        ...
        else if (flg == 2)                                            //Update  
        {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesOrderInvoiceUpdate",
                data: data,
                success: function (result) {
                    $('#btnordersaveedit').prop('disabled', false);
                    if (result.oList.length > 0) {
...
function BillOrderCopy(flg)      //flg:0 - Copy ,flg :1-View  
{
    Copyflag = 1;
    copyrefresh(0);
...
function CallOrderViewList() {
    ...
    $.ajax({
        type: "POST",
        url: '../SalesInvoice/SalesInvoiceOrderGetList',
        data: data,
        success: function (result) {
            ...
            if (result.length != 0) {
                ...
                for (var n = 0; n < result.length; n++) {
                    ...
                    ProdRow += "<tr class='jsgrid-row' ondblclick='SalesOrderGetCall(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)'>" +
                        //  "<td style='width:3%' class='text-center'>" +
                        //"<button class='btn white btn-round btn-xs' type='button' style='background-color:#FF9800;font-size:smaller;margin:0' onclick='SalesGetCall(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)' data-toggle='tooltip' data-placement='top' data-original-title=Print><i class='ft-printer'></i></button>" +
                        //"</td>" +
                        "<td style='width:3%' class='text-center'>" +
...
function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    if (flg == 3) {
        ...
        $('#saleinv,#btnsaveedit,#btnordersaveedit,#BtnClearproc').show();
        $('#Copysales,#btnprint,#btnedit,#btndelete,#btnorderdelete,#btnsubmit,#btnview,#btnlist,#btnacctran,#btnprec').hide();
        ...
        $('#saleinv,.btn:not(.avdbtn),#btnedit,#btndelete,#btnorderdelete').hide();
        $('.SlRow').remove();
        $('.PRCRow').remove();
        ProcedureGrandTotalCalc(1);
        if (flg != 2) {
    ...
    if (flg != 3) { $('#btnedit,#btndelete,#btnorderdelete,#btnacctran,#btnprint,#btnprec').hide(); }
    CheckEOD();
}
...
function SalesGetandGets(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        if (result[i].ProductId != 0) {
            ...
            $('.rdonl').removeClass('bgclrwhite');
            $('#btnedit,#btndelete,#btnorderdelete,#btnacctran,#btnprint,#btnprec').show();


            if (parseFloat(result[0].Variable2 || 0) == 0) {
                $('#btnedit,#btndelete,#btnorderdelete').show();
            }
            else {
                $('#btnedit,#btndelete#btnorderdelete').hide();
            }
        }
...
function DeleteBill(Flag) {      //Flag=0:Delete Contract  
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesOrderInvoiceDelete",
        data: data,
        success: function (result) {
...
function DeleteOrderBill(Flag) {      //Flag=0:Delete Contract  
    debugger;
    $('#btnorderdelete').prop('disabled', true);
    var data = {};
    data.HBillNo = $('#HBillNoCopy').val();
    data.DeptId = ERPDeptId;
    data.UserId = ERPUserId;
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            $('#btnorderdelete').prop('disabled', false);
            var status = result.oList[0].Status;
            var BillNo = result.oList[0].HBillNo;
            Showalerts(status, BillNo);
        }
...
function AddProduct() {
    ...
    var ProdRow =
        ...
        '<td class="jsgrid-align-center" style="width:5%"><input type="hidden"  class="form-control smallTextbox brnone bgclrwhite" id="Company' + Id + '"  style="width:100%" readonly />' + + '</td>' +
...
function formrefresh(flg) {
    if (flg == 0)     //New Button click
    {
        ...
        $('#Copysales,#btnsaveedit, #btnordersaveedit, #btnprint,#btnedit,#btndelete,#btnorderdelete,#btnacctran,#btnprec,#btnprecold').hide();
        $('select:not(.disb)').prop('disabled', false);
