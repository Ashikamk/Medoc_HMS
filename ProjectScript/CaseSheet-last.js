...
function SubDetailsSave(status, CaseSheetId)
{
    ...
    for (var k = 1; k <= MedCount; k++) {
        ...
        var Medicine = $('#Medicine' + k).val() + ',' + $('#Generic' + k).val();
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        if (result[i].IPNumber != 0) {
            ...
            var Visit = '<tr class="jsgrid-header-row VRow" style=' + AAAA + ' >' +
                       ...
                       '<td onclick=CallIPCaseLoad(' + result[i].IPNumber + ') class="" style="width: 30%;cursor:pointer"><input type=text readonly class="form-control" style="background-color:white;border:none" value="' + result[i].DoctorName + '" ></td>' +
        ...
        else
        {
            var Visit = '<tr class="jsgrid-header-row VRow"  >' +
                    ...
                    '<td onclick=CallCaseLoad(' + result[i].DoctorId + ',2,' + slno + ') class="" style="width: 15%;cursor:pointer"><input type=text readonly class="form-control" style="background-color:white;border:none" value="' + result[i].HealthCard + '" ></td>' +
                     '<td onclick=CallCaseLoad(' + result[i].DoctorId + ',2,' + slno + ') class="" style="width: 30%;cursor:pointer"><input type=text readonly class="form-control" style="background-color:white;border:none" value="' + result[i].DoctorName + '" ></td>' +
                     '<td class="" style="width: 5%;cursor:pointer" align=center>' +
...
function PrevVisitDetails(result, Flag) {
    ...
    if (result.length > 0) {
        ...
        $('#PMedAdvice').val(result[0].ICDDetails);
...
function AddMedicineRow(flg)      //flg-0 : Add Medicine
{
    if (flg == 0) {
        ...
        '<td class="" style="width: 30%;">' +
        '<input type="text" class="form-control mdclass brdles" name="medname" id="Medicine' + Id + '" onfocus="LoadMedicine(' + Id + ')" onfocusout=UpdateMedicineRow(' + Id + ',0),mednamesplite(' + Id + ') />' +
        '  <input type="hidden" id="Generic' + Id + '"   />  <input type="text" style="display:none" class="form-control" id="MedicineId' + Id + '" />' +
        '</td>' +

        '<td class="" style="width: 20%;"><input type="text" class="form-control mdclass brdles" onfocusin= mednamesplite(' + Id + ')  name="Notes" id="mednote' + Id + '" /></td>' +
...
function AddMedicineList(result)      //flg-0 : Add Medicine during List
{
    ...
    for (var i = 0; i < result.length;i++)
    {
        ...
        if (i == 0)
        {
            ...
            var med = (result[0].Medicine).split(",")
        ...
        {
            var med = (result[i].Medicine).split(",")
            var Id = parseInt(MedCount) + 1; var SlNo = $('.Mrow').length + 1;
            var MedRow =
            ...
            '<td class="" style="width: 30%;">' +
            ' <input type="hidden" id="Generic' + Id + '" value="' + med[1] + '" />  <input type="text" class="form-control mdclass brdles" name="medname" id="Medicine' + Id + '"  value="' + med[0] + '" onfocus="LoadMedicine(' + Id + ')" onfocusout=UpdateMedicineRow(' + Id + ',0) />' +
