var LocnSelect = ''; var SavingFlg = 0; var WorkSales = 0; var PLFlg = 0; var ExportSalesTax=0;
...
function AreaLoad(result, a) {
    ...
    $("#select_place").append("<option value='EXPORT'>EXPORT</option>"); 
    if (a == 0) {
...
function Billseriesload(result,flg) {
    if (flg == 0)
    {
         if (result.length == 0) {
        $("#addacnttype").show();
    ...
    {
        for (var i = 0; i < result.length; i++) {
            if ((result[i].id ) == $('#txtBillseriesId').val())
            { $('#txtBlSlNo').val(result[i].CurrentNo); }
        }               
    }
    else if (flg == 2) {
        for (var i = 0; i < result.length; i++) {
            if ((result[i].id) == $('#txtBillseriesId').val())
            { $('#txtBlSlNo,#txtBillSlNocopy').val(result[i].CurrentNo);  JsBarcode("#barcode1", $('#txtBillseriesId').val() + '-' + result[i].CurrentNo)}
        }               
    }
    else if (flg == 3)
    {
        for (var i = 0; i < result.length; i++) {
            if ((result[i].id) == $('#txtBillseriesId').val())
            { $('#txtBlSlNo,#txtBillSlNocopy,#txtBillSlNo').val(result[i].CurrentNo); $('#txtBillseriesId').focus(); }
        }            
    }
}

function BillLoad(flg) {
    var data = {};
    data.id = 0;
    ...
    $.ajax({
        ...
        success: function (result) {           
            Billseriesload(result.oList,flg);
        }
    });
}
...
$(document).keydown(function (e) {
        ...
        formrefreshconfirm();
        ...
        else if ((e.altKey && e.keyCode == 89) && ($("#salestranspopupdiv").is(":visible"))) {                  //Alt+Y       
            ...
            if ($('#status_type').prop("checked")) {
                ...
                $('#status_type').prop("checked", true);
            }
            CheckTransType()

        }
        else if ((e.altKey && e.keyCode == 89) && ($("#PurchaseTransactionPopup").is(":visible"))) {                  //Alt+Y       

            if ($("#select_status").is(":visible")) {
            ...
            else if ($("#select_status_sales").is(":visible")) {
                if ($('#select_status_sales').prop("checked")) {
                    $('#select_status_sales').prop("checked", false);
                }
                else {
                    $('#select_status_sales').prop("checked", true);
                }
                LastSalesTransactions();
            }
            else if ($("#select_status_PREV").is(":visible")) {
                if ($('#select_status_PREV').prop("checked")) {
                    $('#select_status_PREV').prop("checked", false);
                }
                else {
                    $('#select_status_PREV').prop("checked", true);
                }
                lastalltrans();
            }
        }
            ...
            {
                if ($("#iconForm").is(":visible"))
                { closepopupprdtlist(0), clearprodlist(1), $('#iconForm').hide(); }
                else
                {
                    $("#txtproduct0").focus(); 
                }
        ...
        if (($('#alertpopup').is(":visible")) && ($('#alertdiv1').is(":visible"))) {
            $('#btncnclalrt').click();
        }
                ...
                for (var p = 0; p < result.length; p++) {                   
                    ...
                    if (result[p].PurchaseType == 'Local' || SuppDetailsRight == 'Yes') {
                        ProdRow = ProdRow + "<tr id=" + 'pdctrow' + (p + 1) + ">" +
                            ...
                            "<td style='width:30%' class='text-left'>" + result[p].SupplierName + "</td>" +
    ...
    if (result != 0) {
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if((MultiPdtArray.indexOf("" + result[l].ItemId + "") == -1))
            {
                ...
                if ($('#select_place').val() == 'EXPORT')
                { $('#Itemtaxgrid' + slno).val(ExportSalesTax);}
                else { $('#Itemtaxgrid' + slno).val(result[l].VatId); }
                ...
                if ($("#SlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#Itemunitgrid" + m).val() == '') {
                        ...
                        var ProdRows = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $('#ItemIdgrid' + m).val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:4%;text-align:center' name=tdItem><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $('#ItemIdgrid' + m).val() + "' /><input type='text' style='height:30px;background-color:white' disabled='' onfocus=LoadProduct(" + id + ")  name='gridsitem' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $('#Itemcodegrid' + m).val() + "'  data-toggle='tooltip' title='" + $('#Itemcodegrid' + m).val() + "'></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select style='background-color:white;height:30px;' name='exporttax' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)' onkeydown=Focusnextgrid(event,'t'," + id + ") onfocusout=focusoutgrid('t'," + id + ") onfocusin=focusingrid('t'," + id + ")>" + TaxSelect + "</select></td>" +
    ...
    if (id != 0)
    {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                // // FAZAL AMIN   
                //if ((result.oList[0].CustLastSellingPrice || 0) > 0)
                //{ $('#prrate' + id).val(parseFloat(result.oList[0].CustLastSellingPrice).toFixed(Decimal)); }
                //else if ((result.oList[0].LastSellingPrice || 0) > 0)
                //{ $('#prrate' + id).val(parseFloat(result.oList[0].LastSellingPrice).toFixed(Decimal)); }
                //gridamountcalculation(id);
            }
        });
    }
...
function loadcust()
{
    ...
    else if ((!($('#txtCustId').val() > 0)) && (copyflag == 0))
    { 
        $('#txtcustomer').val('');
...
function savesales(flag) {
    ...
    //}
    else if ($.trim($('#CustPhnNew').val()) == '' && (usermenu1.indexOf("M318") == -1)) {
        warningshow('Please Enter Phone Number', 'CustPhnNew');
    }
...
function savesalesconfirm()
{
    ...
    var Place = ''; var ExpStatus = '';
    if ($('#select_place').val() == 'EXPORT')
    { Place = 0; ExpStatus = 'EXPORT' }
    else { Place = $('#select_place').val(); }
    ...
    for (var k = 1; k <= i - 1; k++) {
        ...
        var CustPhnNew = $('#CustPhnNew').val();
        var ChassisNo = $("#ChassisNo").val();
        var Variable1 = ExpStatus; 
        var Variable2 = '';
...
function FillPrevoiusUnsavedProduct(result) {
    ...
    if (result.length > 0)
    {
        ...
        for (var n = 0; n < result.length; n++) {
            ...
            if ($('#select_place').val() == 'EXPORT')
            { $('#select_tax' + id).val(ExportSalesTax); }
            else { $('#select_tax' + id).val(result[n].TaxId); }

            $('#txttaxpercent' + id).val($('#select_tax' + id).find("option:selected").attr("name"));
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (balo < 0) {
                ...
                $('#OutStanding').text(balo.toFixed(Decimal) + ' CR');
            }
            else {
                $('#CustOutStanding').text('OutStanding Amount : ' + balo.toFixed(Decimal) + ' DR');
                $('#OutStanding').text(balo.toFixed(Decimal) + ' DR');
            }
        }
        }
    });
...
function QtnNoload(result) {
    ...
    ExportSalesTax = result[0].ExportSalesTax;
}
...
$(document).ready(function () {
    ...
    if (window.CompanySettingsArray.BusinessType == 2) {
        $('.ChassisRow').show();
    }
    else {
        $('.ChassisRow').hide();
    ...
    if ((usermenu1.indexOf("M296") == -1)) {
        $("#soldmessage").text('');
        $("#btnNo").html('OK');
        $("#btnYes").hide();
        $("#soldmessage").text('Product Already Added...');
    }
    ...
    $("#btncustprnt2").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 39) {
            $("#btncustprnt1").focus();
        }
    ...
    $("#select_place").change(function () {
        ...
        {
            $("#select_tax0").val(ExportSalesTax);
            var Txid= document.getElementById("select_tax0");
            ChangeTax(0, Txid);
            if ($('#tblsalesinvoice tr').length > 0)
            {
                $('#confirm').show();
                $('#confirmOk').focus();
                $('#Confirmflag').val('salesexporttax'); $('#ConfirmRowId').val(0);
                $('#confirmmessage').text('Do you want to change Tax of all Items?');
            }           
        }      
    });
    ...
    $("#PhoneNo,#CustPhnNew").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            e.preventDefault();
            $('#txtproduct0').focus(); 
    ...
    $('#select_payterms').val(1);
    if (SalesPayType == 'Cash')
    { $('#typecash').prop("checked", true); }
    else if (SalesPayType == 'Credit')
    { $('#typecredit').prop("checked", true); }
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckQtnItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:1%'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:2%;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:4%;text-align:center' name=tdItem><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + qtnno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' onfocus=LoadProduct(" + id + ")  name='gridsitem' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:13%;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:4%'><select id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
...
function CustomerEnquiryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        if ((result[n].CustId || 0) > 0)
            $('#typecredit').prop("checked", true)
        else if ((result[n].CustId || 0) == 0)
            $('#typecash').prop("checked", true)
...
function QtnEntryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:1%'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:2%;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:4%;text-align:center' name=tdItem><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + result[n].QuotationNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left'  onfocus=LoadProduct(" + id + ")  name='gridsitem' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:13%;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:4%'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control'  onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function formrefresh() {
    ...
    // else {
        ...
        CustBillArray = []; CurnId = 0;            // CustEnable
        $("#txtcustomer").show();
        $('#txtcustomernew').val('').hide();
        ...
        $('#adr2').val('');
        $('#adr3').val('');
        $('#Trnno,#CustPhnNew,#ChassisNo').val('');
        $('#txtcreditlimit').val('');
  ...
  //  }
        ...
        $('#DateTo').val(CurDate);
        $('#CustOutStanding,#UserName,#OutStanding').text('');
...
function productadd()
{
    ...
    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $("#PrdtId0").val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")'  id=" + 'row' + id + " class='jsgrid-row'>" +
        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:1%'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +         
        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:2%;text-align:center'>" + slno + "</td>" +
        "<td class='jsgrid-cell jsgrid-align-right' style='width:4%;text-align:center' name=tdItem><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $("#PrdtId0").val() + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + $("#Otherdescription0").val() + "'><input type='text' style='display:none;height:30px' id=" + 'MRP' + id + " value='" + $("#MRP0").val() + "'><input type='text' style='height:30px;background-color:white' onfocus=LoadProduct(" + id + ")  name='gridsitem' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $("#txtproduct0").val() + "' data-toggle='tooltip' title='" + $("#txtproduct0").val() + "'></td>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
        "<td class='jsgrid-cell jsgrid-align-center' style='width:4%;'><select style='background-color:white;height:30px' id=" + 'select_location' + id + " class='form-control' onchange ='GetBin(" + id + "," + $("#PrdtId0").val() + ",0),Locationqtycheckmain(" + id + ")' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
...
function EditItem() {
    $(function () {
        ...
        $('td[name=tdItem]').bind("dblclick", function () {
            var Id = $(this).parent().attr('id').match(/\d+/)[0];
            $('#txtproduct' + Id).prop('disabled', false).css('background-color', '').focus();
        });


        $('input[name=gridsitem]').focusout(function () {
            var Id = $(this).attr('id').match(/\d+/)[0];

            window.setTimeout(function () {
                if (($.trim($('#ProductDesc' + Id).val()) == '')) {
                    warningshow('Please Select Item', 'txtproduct' + Id);
                    editflag = 1;
                    return false;
                }
                else
                {
                    editflag = 0;                    
                    var flg = updaterow(Id);
                    if (flg==true)
                    {
                        $('#txtproduct' + Id).prop('disabled', true);
                        $('#txtproduct' + Id).css("background-color", "white");                       
                    }
                    //else
                    //{ return updaterow(Id); }
                }
...
//AUTOCOMPLETE ITEM FROM GRID
function LoadProduct(Tid) {
    Cleartxt();
    $("#txtproduct" + Tid).autocomplete({
        delay: 0,
        minLength: 0,
        source: function (request, response) {
            
            //if (updateflag == 1) {
                $("#slordeno" + Tid).val('0'); $("#delvno" + Tid).val('0'); $("#delvsubid" + Tid).val('0'); $("#slordsubid" + Tid).val('0');
            //}
            $('#disc').val('0.00');
            $('#Discountpercent').val('');
            TaxClear();
            ...
            $('.arrow,.tooltip-inner').hide();
            $("#ProductDesc" + Tid).removeAttr('title');
            $("#ProductDesc" + Tid).removeAttr('data-original-title');
            $('#txtproduct' + Tid).removeAttr('title');
            $('#txtproduct' + Tid).removeAttr('data-original-title');
            
            if ($('#select_location'+ Tid).val() == 0) {
                warningshow('Please Select Location', 'select_location' + Tid);
                $("#txtproduct" + Tid).val('');
                return false;
            }
            else if ($('#txtproduct' + Tid).val() != '') {
               
                var data = {};
                data.ItemCode = $("#txtproduct" + Tid).val();
                data.LocId = $("#select_location" + Tid).val();
                data.DeptId = ERPDeptId;
                data.CustId = $("#txtCustId").val();
                data.UserId = ERPUserId;
                data.ProductMultiPriceId = $("#PriceGroupId").val();
                $.ajax({
                    url: '../SalesInvoice/ProductSearchSales',
                    type: "POST",
                    data: data,
                        ...
                        if (data.length) {
                            response($.map(data, function (item) {
                                return ({
                                    ...
                                    label3: item.stocktotloseqty,
                                    label4: item.SellingPrice.toFixed(Decimal),
                                    label5: item.GrpName.substring(0, 15),
                                    label6: item.SbgrpName.substring(0, 15),
                                    label7: item.CategoryName.substring(0, 15),
                                    label8: item.SubCategoryName.substring(0, 15),

                                    stocktotloseqty: item.stocktotloseqty,
                                    Unit: item.UnitId,
                                    Tax: item.VatId,
                                    Taxpercent: item.VatPer,
                                    Rates: item.SellingPrice,
                                    ...
                                    MRP: item.MRP,
                                    BelowCostFlag: item.MultiPriceId,
                                    headers: ["Code", "Description", "Quantity", "Price", "Group/Manufacturer", "SubGroup", "Category", "SubCategory"]
                                })
                            }));
                        }
                        else {
                            //$('#txtproduct0').autocomplete('close');
                            hidepopup(1);
                        }
                    }
                })
        ...
        select: function (event, ui) {
            ...
            for (var p = 1; p <= i; p++) {
                if ((ui.item.ProductId == $('#PrdtId' + p).val()) && (p!= Tid)) {
                    ProdFlag = 1;
                }
            }
            if (ProdFlag == 1) {        //Prodcut Already added in grid

              //  $("#txtproduct0,#select_location0,#select_unit0,#txtquantity0,#txtrate0,#txtdiscount0,#select_tax0").blur();
                document.getElementById("toast-containerinfo").style.zIndex = "-1";
                $('#modal_dialog').show();
                $('#btnNo').focus();

                dialog(function () {        // Ok Button

                    $('#modal_dialog').hide();

                    if ((ui.item.stocktotloseqty <= 0 && (Negativebill == 'NO')) && (ui.item.stocktotloseqty <= 0 && ($("#select_location" + Tid).find("option:selected").attr("name") == 0))) {
                        warningshow('No stock!', 'txtproduct' + Tid);
                        qtyflag = 1;
                        $('#txtproduct' + Tid).select();
                        return false;
                    }
                    else {
                        $('#txtquantity' + Tid).val(1);
                        $('#txtquantity' + Tid).select();
                        $('#txtquantity' + Tid).focus();

                        $("#ProductDesc" + Tid).attr('data-original-title', ui.item.ProductDesc);
                        $('#txtproduct' + Tid).attr('data-original-title', ui.item.Product);

                        $('#txtproduct' + Tid).attr('title', ui.item.Product);
                        $("#ProductDesc" + Tid).attr('title', ui.item.ProductDesc);

                        var desc = ui.item.ProductDesc;
                        var j = 0, strLength = desc.length;
                        for (j; j < strLength; j++) {
                            desc = desc.replace(" ", "@%@");
                        }

                        var code = ui.item.Product;
                        var k = 0, strLength1 = code.length;
                        for (k; k < strLength1; k++) {
                            code = code.replace(" ", "@%@");
                        }

                        $('#tblsalesinvoice #row' + Tid).attr('onclick', "GetproddetailsGrid(" + Tid + "),GetPrdtId(" + ui.item.ProductId + ",'" + desc + "','" + code + "'," + 2 + ")");

                        var n = ProdIdArray.includes(ui.item.ProductId);
                        if (n != true) {
                            ProdIdArray.push(ui.item.ProductId);
                        }

                        var m = autoProdIdArray.includes(ui.item.ProductId);
                        if (n != true) {
                            autoProdIdArray.push(ui.item.ProductId);
                        }
                        ...
                        $('#txtproduct' + Tid).val(ui.item.Product);
                        $('#ProductDesc' + Tid).val(ui.item.ProductDesc);
                        $('#AvgCost' + Tid).val(parseFloat(ui.item.AvgCost || 0).toFixed(Decimal));
                        $('#PrdtId' + Tid).val(ui.item.ProductId);
                        $('#BelowCostFlag' + Tid).val(ui.item.BelowCostFlag);
                        $('#Otherdescription' + Tid).val(ui.item.Otherdescription);
                        $('#MRP' + Tid).val(ui.item.MRP);
                        $('#select_unit' + Tid).val(ui.item.Unit);

                        if ($('#select_place').val() == 'EXPORT')
                        { $('#select_tax' + Tid).val(ExportSalesTax); $('#txttaxpercent' + Tid).val($('#select_tax' + Tid).find("option:selected").attr("name")); }
                        else { $('#select_tax' + Tid).val(ui.item.Tax); $('#txttaxpercent' + Tid).val(ui.item.Taxpercent); }


                        // $('#txtrate0').val((ui.item.Rates).toFixed(Decimal));
                        $('#txtunit').val(ui.item.UnitName);
                        $('#txtstocktotloseqty' + Tid).val(ui.item.stocktotloseqty);



                        if ((ui.item.UnitName).toLowerCase() == 'foc') {
                            foc = 1;
                            $('#txtrate' + Tid).val((0).toFixed(Decimal));
                        }
                        else {
                            foc = 0;
                        }
                        ...
                        var data = {};
                        data.ProductId = ui.item.ProductId;
                        data.CustId = $("#txtCustId").val();
                        data.DeptId = ERPDeptId;
                        $.ajax({
                            type: "POST",
                            ...
                            success: function (result) {
                                ...
                                //if ((result.oList[0].CustLastSellingPrice || 0) > 0)
                                //{ $('#txtrate' + Tid).val(parseFloat(result.oList[0].CustLastSellingPrice).toFixed(Decimal)); }
                                //else if ((result.oList[0].LastSellingPrice || 0) > 0)
                                //{ $('#txtrate' + Tid).val(parseFloat(result.oList[0].LastSellingPrice).toFixed(Decimal)); }
                                //else
                                //{
                                //    $('#txtrate' + Tid).val((ui.item.Rates).toFixed(Decimal));
                                //}
            ...
            document.getElementById("toast-containerinfo").style.zIndex = "2";
            if (($.trim($('#txtproduct' + Tid).val()) == '') || ($.trim($('#ProductDesc' + Tid).val()) == '')) {
                warningshow('Please Select Item', 'txtproduct' + Tid);
                editflag = 1;
                return false;
            }
            else
...
function PromptboxResult(Result, status, rowid, data1, data2) {
    if (Result == 'true' && status == 'credit') {                               //Creditlimit check during Product Add
        if (data1 == '')
            ...
            data.OTP = data1;
            data.Remarks = data2;
            data.Operation = 'Sales Invoice- OTP - CreditLimit Exceed , Customer : ' + $("#txtcustomer ").val();
            data.DeptId = ERPDeptId;
            $.ajax({
                type: "POST",
                url: "../Home/OTPCheckforUser",
...
function ExportTaxChange() {

    $('select[name="exporttax"]').each((i, item) => {
        var $item = $(item);
        var Id = $item.attr('id').match(/\d+/)[0];;
        $item.val(ExportSalesTax);       
        var TxId = document.getElementById('select_tax' + Id);
        ChangeTax(Id, TxId);
    });
    $('#txtproduct0').focus();
}
...
function copyrefresh() {
    ...
    $('#CustOutStanding,#OutStanding').text('');
    BillLoad(1);
}
...
function ConfirmboxResult(Result, status, rowid) {
    ...
    else if (Result == 'true' && status == 'salesexporttax') {
        ExportTaxChange();
    }
    else if (Result == 'false' && status == 'salesexporttax') {
        $('#select_place').focus();
    }
...
function CheckCancelInvoce() {
    if ($.trim($('#cancelotp').val()) == '') {
        ...
        data.Operation = 'Sales Invoice- OTP - Cancel , Bill No : ' + $("#txtBillseriesId ").find("option:selected").text() + ' - ' + $("#txtBillSlNocopy").val();
        data.DeptId = ERPDeptId;
        $.ajax({
            type: "POST",
            url: "../Home/OTPCheckforUser",
    ...
    try{
        ...
        if (charCode != 13) {
            if ($('#txtCustId').val() != 0) {
                ...
                if ($('#select_place').val() == 'EXPORT')
                { $('#select_place').val('EXPORT'); }
                else { $('#select_place').val(DefaultArea); } 
...
function CompanyDetGet()
{
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            WorkSales = result.oList[0].WorkAfterSales;
        }
    });
}

function Showprint(billsrs, billsrlno)
{
    $('#savealert').html('');   
    $('#alertdiv').hide();
...
function Showalerts(Status, billsrs, billsrlno) {
    ...
    if (Status == 1) {
        ...
        if (WorkSales == 0)
        {
            Showprint(billsrs, billsrlno)
        }
        else if (WorkSales == 1)
        {
            CreateNew(Status, billsrs, billsrlno);
        }
    ...
    if (result != 0) {
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if ($('#select_location0').val() == 1) { if (result[l].Bin_A != '') { bins = bins + result[l].Bin_A; } }
                ...
                if ($('#select_place').val() == 'EXPORT')
                { $('#autoItemtaxgrid' + slno).val(ExportSalesTax); }
                else { $('#autoItemtaxgrid' + slno).val(result[l].VatId); }
