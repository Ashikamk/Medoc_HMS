...
function LoadDate() {
    $('#DischargeDate,#SummaryDate').daterangepicker({
        startDate: CurDate,
        minDate: minDate,
...
function SaveAndUpdate(flag) {
   ...
   // data.Treatment = $('#Treatment').val();
    data.Treatment = $('#Treatment').val() + '@@@' + $('#Allergies').val() + '###' + $('#icd10').val() + '%%%' + $('#MedPrevious').val() + '***' + $('#emergencycase').val();
...
function ShowDischargeGet(result) {
    if (result.length > 0) {
        ...
        Regseriesdis = result[0].Variable4;
       ...
       // $('#Treatment').val(result[0].Treatment);
        var parts = result[0].Treatment.split(/@@@|###|%%%|\*\*\*/);
        $('#Treatment').val(parts[0]);
        $('#Allergies').val(parts[1]);
        $('#icd10').val(parts[2]);
        $('#MedPrevious').val(parts[3]);
        $('#emergencycase').val(parts[4]);
        ...
        debugger;
        const [day, month, year] = (result[0].DischargeDate).split('/').map(Number);
        const date = new Date(year, month - 1, day);
        date.setDate(date.getDate() + parseInt(result[0].AfterDays||0));
        const newDay = String(date.getDate()).padStart(2, '0');
        const newMonth = String(date.getMonth() + 1).padStart(2, '0'); // Months are 0-based
        const newYear = date.getFullYear();
        $('#AfterDate').val(`${newDay}/${newMonth}/${newYear}`);
