...
function formrefresh() {
    ...
    $("#Occupation option:selected").prop("selected", false);
    LoadDate();
...
function SaveAndUpdate(flag) {
    ...
    //}
    ...
    else if (($.trim($("#Bloodgroup").val()) != '') && (Bl == 0)) { warningshow('Please Select a valid Blood Group', 'Bloodgroup'); }
    ...
    else if (SCANLEN == null || SCANLEN==0) {
        warningshow('Please Select Scan Part(s)', 'Occupation');
    }
        ...
        data.Occupation = $('#Occupation').val().toString();
        ...
        $.ajax({
            type: "POST",
            url: "../Master/HMS_ScanRegistrationInsertandUpdate",
            data: data,
            success: function (result) {
...
function ShowRegistrationGet(result, fl) {
    ...
    LOADOCCU = (result[0].Occupation).split(","); ;
    ...
    createQRCode();
    generatebarcode();
    GetProcedurerecall(0)
}

var LOADOCCU = [];
function GetProcedurerecall(ProcedureId) {

    console.log(LOADOCCU)
     var data = {};
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            loadproceduredatacall(result.oList);
        }
    });
}


function loadproceduredatacall(result) {

    $("#Occupation").empty();
    $("#Occupation").append("<option value='0'>Select</option>");
    for (var i = 0; i < result.length; i++) {

        console.log(result[i].ProcedureId+'------'+LOADOCCU.includes(result[i].ProcedureId))

        //console.log(result[i].ProcedureId)

        if (LOADOCCU.indexOf(result[i].ProcedureId) !=0) {
            $("#Occupation").append("<option value='" + result[i].ProcedureId + "' >" + result[i].ProcedureName + "</option>");
        }
        else {
            $("#Occupation").append("<option   value='" + result[i].ProcedureId + "'  selected=''>" + result[i].ProcedureName + "</option>");
                    ...
                    switch (value) {
                        ...
                        case "defeat":
                           // PrintRegistration();
                            ...
                            break;
                        case "Other":
                            window.open('../Revisit/ProcedureBill?flag=scanreg', '_blank');
                            formrefresh();
                            break;
...
function PrintRegistration() {
    ...
    var TotalAmount = parseFloat($("#RegFee").val() || 0) + parseFloat($("#ConsultFee").val() || 0) + parseFloat($("#OtherFee").val() || 0)

    var AmountinWords = WordwithDecimal(TotalAmount);
    ...
    myWindow.document.write('<table width=100% ><tr rowspan=2 ><td width=70% align=left colspan=5 style=color:#008000;font-weight:bold>' + (ComapnydivToPrintLab.outerHTML) + '</td></tr>');
    myWindow.document.write('</table>');

    var TypeText = '';


    myWindow.document.write('<table width=100% ><tr ><td width=10% align=left >OP#&#160;&#160; :</td><td colspan=4 align=left width=60%>' + $('#RegNo').val() + '</td><td align=right width=10%>Date &#160;&#160;&#160;&#160;&#160;&#160;: </td><td  colspn=2 width=20% >' + $('#RegDate').val() + '</td></tr>');
    ...
    myWindow.document.write('<tr style="border:1px solid grey;" class=violetbg><td colspan=2 font-size: 13px;" align=center style="font-weight:bold">' + AmountinWords + '</td><td style="font-family:tahoma; font-size: 13px;border-left:1px solid grey;"  align=right ><b>' + addCommas(parseFloat(TotalAmount).toFixed(Decimal)) + '</b></td></tr>');
    myWindow.document.write('</table>');
