...
$(document).ready(function () {
    ...
    $('#signimg').attr('src', "../app-assets/img/Signs/" + ERPUserId + ".png");
    ...
    $("#btnlist").click(function (e) {
        debugger;
        console.log(usermenu1.indexOf("M418"))
        if (usermenu1.indexOf("M418") == -1) {
            alert('Invalid User Permission. Contact Admin.(LAB RESULT LIST VIEW)')
        }
        else {
...
function LabBillLoad(result, Flag) {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        var ProdEditRow = "<tr class='jsgrid-row Billtr' id='Brow_" + id + "' onclick=GetBill('" + id + "',0)>" +
            ...
            "<input type='hidden' id='Pphonenum_" + id + "' value='" + (result[i].Doctor).split('#')[6] + "' />" +
            "</td>" +
            "<td class='jsgrid-cell p-1 GridBRow' id='TxtBillNo_" + id + "'  style= 'width:15%;' >" + result[i].BillNo +
            "</td>" +
            "<td class='jsgrid-cell p-1 GridBRow' id='TxtDate_" + id + "' style= 'width:20%;' >" + result[i].Date +
            "</td>" +
            "<td class='jsgrid-cell p-1 GridBRow' id='TxtDoctorName_" + id + "'  style= 'width:40%;' >" + (result[i].Doctor).split('#')[0] +
            "</td>" +
...
function GetBill(Id, Flag) {
    if ($("#Resulted_" + Id).val() == 1 && Flag == 0) {
        ...
        $('#Remarksphone').val($('#Pphonenum_' + Id).val() + '##');
        ...
        data.Flag = Flag;
        data.DeptId = ERPDeptId;
        data.BillYear = $("#BillYear_" + Id).val();
        $.ajax({
            type: "POST",
...
//function LabBillTestLoadout(result) {
function LabBillTestLoad(result) {
    ...
    FirstId = 0; var datax = []
    var depthead = ""; var dp = ""; var TestRemarks = '';

    var Payflag = result[0].MedDeptName;

    if (Payflag == 'NOTPAID') {
        //alert('Payment Not Done')
        warningshow('⚠️ Payment Not Received ⚠️\nAccess to results is pending due to an unpaid bill. Please settle the payment to proceed.(settings)', '');
    }
    else {
        for (var i = 0; i < result.length; i++) {
            ...
            if (result[i].Flag == 1) {
                ...
                var ProdEditRow = "<tr class='jsgrid-sub-header-row Resulttr' id='Trow_" + id + "' >" +
                    ...
                    "</td>" +
                    '<td width=30px class="jsgrid-header-cell GridBRowTop">' +
                    "<input class='efgh " + result[i].TestId + "'  id='selprint_" + id + "'  onchange='printdeptselecttest(" + result[i].TestId + ',' + id + ")'  type='checkbox'  checked ></td>" +
                ...
                var ProdEditRow = "<tr class='jsgrid-row Resulttr' id='row_" + id + "' >" +
                    ...
                    "</td>" +
                    '<td width=30px class="jsgrid-header-cell GridBRowTop">' +
                    "<input class='efgh " + result[i].TestId + "'  id='selprint_" + id + "'  type='checkbox'  checked ></td>" +
                    ...
                    "<input type='text' class='form-control GridText' id='NormalValue_" + id + "' readonly value='" + result[i].NormalValue + "' onkeydown=FocusNext(\'" + id + "\',\'NormalValue_\',event) ondblclick=ShowModal(\'NormalValue_'\,\'" + id + "'\) />" +
                    "</td>" +
                    ...
                    "<input type='text' class='form-control GridText' id='Notes_" + id + "' value='" + TestRemarks + "' readonly onkeydown=FocusNext(\'" + id + "\',\'Notes_\',event) ondblclick=ShowModal(\'Notes_'\,\'" + id + "'\) />" +
                    "</td>" +
                    "</tr>";
                TestRemarks = "";
...
function OKSaveandUpdateResult(DFlag) {
    ...
    for (var k = 1; k <= $("#GridLength").val(); k++) {
        ...
        var Remarks = $("#Remarksphone").val() + '##' + $("#Remarks").val();
...
function GetResult(result) {
    ...
    if (result.length > 0) {
        ...
        $("#Remarksphone").val(result[0].Remarks);
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (((result[i].Status).toUpperCase() == 'YES') && CurrentHead != result[i].TestName) {
                ...
                ProdEditRow = "<tr class='jsgrid-sub-header-row Resulttr' id='Trow_" + id + "' >" +
                    ...
                    "</td>" +
                    '<td width=30px class="jsgrid-header-cell GridBRowTop">' +
                    // "<input class='efgh'  id='selprint_" + id + "'  type='checkbox'  checked >"+
                    "<input class='efgh " + result[i].TestId + "'  id='selprint_" + id + "'  onchange='printdeptselecttest(" + result[i].TestId + ',' + id + ")'  type='checkbox'  checked ></td>" +


                    "</td><td class='jsgrid-header-cell GridBRowTop' id='TestName_" + id + "'  style= 'width:384px;' >" + result[i].TestName +
                    "</td>" +
                    "<td class='jsgrid-header-cell GridBRowTop' id='Resulttd_" + id + "' style= 'width:384px;' >" +
                    "</td>" +
                    "<td class='jsgrid-header-cell GridBRowTop' id='NormalValuetd_" + id + "'  style= 'width:284px;' >" +
            ...
            var ProdEditRow1 = "<tr class='jsgrid-row Resulttr' id='row_" + id + "' >" +
                ...
                "<input type='hidden' id='subtype_" + id + "' value='" + result[i].Status + "'/>" +
                "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow " + Border + "' width=30px> <input class='efgh " + result[i].TestId + "'   id='selprint_" + id + "' checked  type='checkbox'></td><td class='jsgrid-cell p-1 GridBRow " + Border + "' id='TestName_" + id + "'  style= 'width:384px;font-weight:bold;' >" + result[i].SubTestName +
                "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow " + Border + "' id='Resulttd_" + id + "' style= 'width:384px;' >" +
                "<input type='text' class='form-control GridText' id='Result_" + id + "' value='" + (result[i].Result).replace("'", "") + "'  onkeydown=FocusNext(\'" + id + "\',\'Result_\',event) ondblclick=ShowModal(\'Result_'\,\'" + id + "'\) />" +
                "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow " + Border + "' id='NormalValuetd_" + id + "'  style= 'width:284px;' align='center'>" +
                "<input type='text' class='form-control GridText' id='NormalValue_" + id + "' readonly value='" + result[i].NormalValue + "' onkeydown=FocusNext(\'" + id + "\',\'NormalValue_\',event) ondblclick=ShowModal(\'NormalValue_'\,\'" + id + "'\) />" +
                "</td>" +
                ...
                "<input type='text' class='form-control GridText' id='Notes_" + id + "' readonly value='" + result[i].Notes + "' onkeydown=FocusNext(\'" + id + "\',\'Notes_\',event) ondblclick=ShowModal(\'Notes_'\,\'" + id + "'\) />" +
                "</td>" +
                "</tr>";
