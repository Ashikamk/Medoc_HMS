...
$(document).ready(function () {
    ...
    $("#btnopticalordersubmit").click(function (e) {
        // $('.tool_tip').tooltip({ trigger: 'manual' }).tooltip('hide');
        SaveAndUpdateOrder(1);
    });

    $("#btnpatdetailsave").click(function (e) {
        var name = $("#Name").val();
        var mobile = $("#Mobile").val();
        var salesMan = $("#SalesMan").val();
        var dueDate = $("#DelDate").val();


        // Validate inputs
        if (!name || !mobile || !salesMan || !dueDate) {
            ...
            $("#PatientDetailsPopup").modal('hide');
        }
    });
    ...
    $("#btnopticalsaveedit").click(function (e) {
        // $('.tool_tip').tooltip({ trigger: 'manual' }).tooltip('hide');
        SaveAndUpdateOrder(2);
    });
...
function BillLoad() {
    ...
    $.ajax({
        type: "POST",
        url: "../SalesInvoice/BillOrderSeriesGetandGets",
        data: data,
        success: function (result) {
    ...
    for (var i = 1; i <= RowCount; i++) {
        if ($('#Product' + i).val() != undefined) {
            ...
            oArray.push({
                ...
                'GSTNo': $("#GSTNo").val(),
                'GaragePhone': $("#Mobile").val(),
                'ExportDoc': $("#DOB").val(),
                'Email': $("#Email").val(),
                'ExportStatus': $("#DOW").val(),
                'SalesManId': $("#SalesMan").val(),
                'AgreementNo': $("#Age").val(),
                'DueDate': $("#DelDate").val(),
                'Gender': $("#Sex").val(),
                'Referance': $("#Referance").val(),
                'RegisterNo': $("#RegNo").val(),
                'RightFarSph': $("#farSphRight").val(),
                'RightFarCyl': $("#farCylRight").val(),
                'RightFarAxs': $("#farAxsRight").val(),
                'RightFarVA': $("#farVARight").val(),
                'RightFarPD': $("#farPDRRight").val(),
                'RightFarAdd': $("#farAddRight").val(),
                'LeftFarSph': $("#farSphLeft").val(),
                'LeftFarCyl': $("#farCylLeft").val(),
                'LeftFarAxs': $("#farAxsLeft").val(),
                'LeftFarVA': $("#farVALeft").val(),
                'LeftFarPD': $("#farPDLeft").val(),
                'LeftFarAdd': $("#farAddLeft").val(),
                'RightNearSph': $("#nearSphRight").val(),
                'RightNearCyl': $("#nearCylRight").val(),
                'RightNearAxs': $("#nearAxsRight").val(),
                'RightNearVA': $("#nearVARight").val(),
                'RightNearPD': $("#nearPDRRight").val(),
                'RightNearAdd': $("#nearAddRight").val(),
                'LeftNearSph': $("#nearSphLeft").val(),
                'LeftNearCyl': $("#nearCylLeft").val(),
                'LeftNearAxs': $("#nearAxsLeft").val(),
                'LeftNearVA': $("#nearVALeft").val(),
                'LeftNearPD': $("#nearPDLeft").val(),
                'LeftNearAdd': $("#nearAddLeft").val(),
                'Indax': $("#Index").val(),
                'Coating': $("#Coating").val(),
                'SplTreat': $("#SplTreat").val(),
                'OtherSpecs': $("#OtherSpecs").val(),
                'Optometrist': $("#Optometrist").val(),
                'LensConst': $("#LensConst").val(),
                'OptPrescRemarks': $("#OptPrescRemarks").val(),
                'InterState': $("#InterState").prop('checked'),
                'ReSendSMS': $("#ReSendSMS").prop('checked')
            });
            slno++;
        }
    }
...
function SaveAndUpdateOrder(flg) {
    debugger;
    if (flg == 1) { $('#btnopticalordersubmit').prop('disabled', true); }
    else if (flg == 2) { $('#btnopticalsaveedit').prop('disabled', true); }
    ...
    for (var i = 1; i <= RowCount; i++) {
        if ($('#Product' + i).val() != undefined) {
            ...
            // Validate inputs
            if (!name || !mobile || !salesMan || !dueDate) {
                alert("Please fill necessary fileds in Patient Details and Optical Prescription forms");
                if (flg == 1) {
                    $('#btnopticalordersubmit').prop('disabled', false);
                } else if (flg == 2) {
                    $('#btnopticalsaveedit').prop('disabled', false);
                }
                return;
            }
            var companyValue = $("#Company" + i).val();
            // If the Company value is null, assign the globalProductDescr value
            if (!companyValue && globalProductDescrArray[i - 1]) {
                companyValue = globalProductDescrArray[i - 1];
            }
            oArray.push({
                'SalesMainId': parseInt(slno),
                'HBillSeries': $("#HBillSeries").val(),
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_OpticalSalesInvoiceInsert",
                data: data,
                success: function (result) {
                    $('#btnopticalordersubmit').prop('disabled', false);
                    debugger;
        ...
        else if (flg == 2)                                            //Update  
        {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_OpticalSalesInvoiceUpdate",
                data: data,
                success: function (result) {
                    $('#btnordersaveedit').prop('disabled', false);
...
function GetOpticalInvoiceRows(flg) {
    debugger;
    if (Editflag != 0) {
        warningshow('Please Update Edit Mode');
    }
...
function BillOpticalInvoiceCopy(flg)      //flg:0 - Copy ,flg :1-View  
{
    debugger;
    Copyflag = 1;
    copyrefresh(0);
...
function CallOpticalInvoicesViewList() {
    debugger;
    var data = {};
    data.FromDate = $('#DateFrom').val();
    data.ToDate = $('#DateTo').val();
    ...
    $.ajax({
        type: "POST",
        url: '../SalesInvoice/SalesOpticalInvoiceGetList',
        data: data,
        success: function (result) {
            disable_datatable('tblViewList');
            $('#tblViewList tr').remove();
            ...
            if (result.length != 0) {
                ...
                for (var n = 0; n < result.length; n++) {
                    ...
                    ProdRow += "<tr class='jsgrid-row' ondblclick='OpticalSalesInvoiceGetCall(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)'>" +
                        //  "<td style='width:3%' class='text-center'>" +
                        //"<button class='btn white btn-round btn-xs' type='button' style='background-color:#FF9800;font-size:smaller;margin:0' onclick='SalesGetCall(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)' data-toggle='tooltip' data-placement='top' data-original-title=Print><i class='ft-printer'></i></button>" +
                        //"</td>" +
                        "<td style='width:3%' class='text-center'>" +
...
function copyrefresh(flg) {
    debugger;//flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    if (flg == 3) {
        $('#BillNo').val($('#BillNoCopy').val());
        $('#saleinv,#btnsaveedit,#btnordersaveedit,#BtnClearproc,#btnopticalsaveedit').show();
        $('#Copysales,#btnprint,#btnedit,#btndelete,#btnopticalinvoicedelete,#btnsubmit,#btnview,#btnlist,#btnacctran,#btnprec').hide();
        ...
        $('#saleinv,.btn:not(.avdbtn),#btnedit,#btndelete,#btnopticalinvoicedelete').hide();
        $('.SlRow').remove();
        $('.PRCRow').remove();
        ProcedureGrandTotalCalc(1);
        if (flg != 2) {
    ...
    if (flg != 3) { $('#btnedit,#btndelete,#btnopticalinvoicedelete,#btnacctran,#btnprint,#btnprec').hide(); }
    CheckEOD();
}
...
//function SalesOrderGetCall(BillSeriesId, BillSlNo, DeptId, flg) {
//    if (flg == 1) { PopUpClose(3); }
//    var data = {};
//    data.HBillNo = BillSeriesId;
//    data.HBillSeries = BillSlNo;
...
//    $.ajax({
//        type: "POST",
//        url: "../Hospital/HMS_SalesOrderGets",
//        data: data,
//        success: function (result) {
//            if (result.length > 0) {
//                SalesGetandGets(result);
...
function SalesGetandGets(result) {
    ...
    Billuser = uname[1];
    debugger;
    var sonNo = result[0].SONo === 0 ? "" : result[0].SONo;
    ...
    $('#PatDet').prop('disabled', false);
    $('#OptDet').prop('disabled', false);

    $('#btnpatdetclose').show();
    $('#btnpatdetailsave').show();
    ...
    //Patient Details
    $('#Name').val(uname[0]);
    $('#Address').val(result[0].CustAddress);
    $('#GSTNo').val(result[0].GSTNo);
    $('#Mobile').val(result[0].GaragePhone);
    $('#DOB').val(result[0].ExportDoc);
    $('#Email').val(result[0].Email);
    $('#DOW').val(result[0].ExportStatus);
    $('#SalesMan').val(result[0].SalesManId);
    $('#Age').val(result[0].AgreementNo);
    $('#DelDate').val(result[0].DueDate);
    $('#Sex').val(result[0].Gender);
    $('#Referance').val(result[0].Referance);
    $('#RegNo').val(result[0].RegisterNo);

    //Optical Details
    $('#farSphRight').val(result[0].RightFarSph);
    $('#farCylRight').val(result[0].RightFarCyl);
    $('#farAxsRight').val(result[0].RightFarAxs);
    $('#farVARight').val(result[0].RightFarVA);
    $('#farPDRRight').val(result[0].RightFarPD);
    $('#farAddRight').val(result[0].RightFarAdd);
    $('#farSphLeft').val(result[0].LeftFarSph);
    $('#farCylLeft').val(result[0].LeftFarCyl);
    $('#farAxsLeft').val(result[0].LeftFarAxs);
    $('#farVALeft').val(result[0].LeftFarVA);
    $('#farPDLeft').val(result[0].LeftFarPD);
    $('#farAddLeft').val(result[0].LeftFarAdd);
    $('#nearSphRight').val(result[0].RightNearSph);
    $('#nearCylRight').val(result[0].RightNearCyl);
    $('#nearAxsRight').val(result[0].RightNearAxs);
    $('#nearVARight').val(result[0].RightNearVA);
    $('#nearPDRRight').val(result[0].RightNearPD);
    $('#nearAddRight').val(result[0].RightNearAdd);
    $('#nearSphLeft').val(result[0].LeftNearSph);
    $('#nearCylLeft').val(result[0].LeftNearCyl);
    $('#nearAxsLeft').val(result[0].LeftNearAxs);
    $('#nearVALeft').val(result[0].LeftNearVA);
    $('#nearPDLeft').val(result[0].LeftNearPD);
    $('#nearAddLeft').val(result[0].LeftNearAdd);

    $('#Index').val(result[0].Indax);
    $('#Coating').val(result[0].Coating);
    $('#SplTreat').val(result[0].SplTreat);
    $('#OtherSpecs').val(result[0].OtherSpecs);
    $('#Optometrist').val(result[0].Optometrist);
    $('#OptPrescRemarks').val(result[0].OptPrescRemarks);
    $('#LensConst').val(result[0].LensConst);
    $('#ReSendSMS').prop('checked', result[0].ReSendSMS);
    $('#InterState').prop('checked', result[0].InterState);
    /*    anu  */
    ...
    for (var i = 0; i < result.length; i++) {
        if (result[i].ProductId != 0) {
            ...
            $('.rdonl').removeClass('bgclrwhite');
            $('#btnedit,#btndelete,#btnopticalinvoicedelete,#btnacctran,#btnprint,#btnprec').show();


            if (parseFloat(result[0].Variable2 || 0) == 0) {
                $('#btnedit,#btndelete,#btnopticalinvoicedelete').show();
            }
            else {
                $('#btnedit,#btndelete#btnopticalinvoicedelete').hide();
            }
        }
...
function OpticalSalesInvoiceGetCall(BillSeriesId, BillSlNo, DeptId, flg) {
    if (flg == 1) { PopUpClose(3); }
    var data = {};
    data.HBillNo = BillSeriesId;
    data.HBillSeries = BillSlNo;
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_OpticalSalesInvoiceGetProductDetails",
        data: data,
        success: function (result) {
            if (result.length > 0) {
                SalesGetandGets(result);
...
function DeleteOpticalInvoiceBill(Flag) {      //Flag=0:Delete Contract  
    debugger;
    $('#btnopticalinvoicedelete').prop('disabled', true);
    var data = {};
    data.HBillNo = $('#HBillNoCopy').val();
    data.DeptId = ERPDeptId;
    data.UserId = ERPUserId;
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_OpticalSalesInvoiceDelete",
        data: data,
        success: function (result) {
            $('#btnopticalinvoicedelete').prop('disabled', false);
            var status = result.oList[0].Status;
            var BillNo = result.oList[0].HBillNo;
            Showalerts(status, BillNo);
        }
...
function AddProduct() {
    //lll
    var startingSlNo = $('.SlRow').length > 0 ? parseInt($('.SlRow:last').find('td:eq(1)').text()) + 1 : 1;

    var Id = RowCount;
    var SlNo = startingSlNo;
    ...
    var ProdRow =
        ...
        '<td class="jsgrid-align-center" style="width:5%"><input type="hidden" class="form-control smallTextbox brnone bgclrwhite" id="Company' + Id + '" style="width:100%" value="' + (CMD[0] !== undefined ? CMD[0] : '') + '" readonly />' + (CMD[1] !== undefined ? CMD[1] : '') + '</td>' +
...
function formrefresh(flg) {
    if (flg == 0)     //New Button click
    {
        ...
        $('#Copysales,#btnsaveedit, #btnordersaveedit, #btnprint,#btnedit,#btndelete,#btnopticalinvoicedelete,#btnacctran,#btnprec,#btnprecold').hide();
        $('select:not(.disb)').prop('disabled', false);
