...
function ShowMultipleProductsList(result,st) { 
    ...
    if (result != 0) {
        var responseText1 = "<thead><tr><th style='width:90px;'> </th><th>Product</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th>Discount</th><th>Tax</th><th>Location</th></tr></thead><tbody>";
        $('#Productlist').append(responseText1);
        for (var l = 0; l < result.length; l++) {
            var slno = parseInt(l + 1);

            var responseText =
                '<tr><td style="width:90px;" ><input name="Checkprdtlist" onchange=clearcheck(' + slno + ') type="checkbox" disabled id= ' + 'SlNoHeadCheckgrid' + slno + '  class="form-control" style="zoom:.7;align:center"></td>' +
...
function LoadProddetails(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        if (result[n].custstats == 'Last Selling Price' && $('#txtCustId').val() == 0) {
            ...
            var ProdRow =
            "Average Cost: " + (parseFloat(result[n].AvgCost || 0).toFixed(Decimal)) + " ,Last Purchase Cost: " + (parseFloat(result[n].LPCost || 0).toFixed(Decimal)) + " ,Customer LS Price:" + (parseFloat(result[n].CustLastSellingPrice || 0).toFixed(Decimal)) + " ,Last Selling Price:" + (parseFloat(result[n].LastSellingPrice || 0).toFixed(Decimal)) + " ,Total Stock:" + (result[n].Sumtotqty || 0) + " ";
            var ProdRow2 = strr3;
            $('#pcol1').text(ProdRow);
            $('#pcol2').append(ProdRow2);
            $('#ItemDetails').show();
...
function savesalesconfirm() {
        ...
        $.ajax(
    {
        ...
        type: "POST",
        url: "../SalesInvoice/PressSalesInsertandUpdate", 
        data: data,
        success: function (result) {
...
$(document).ready(function () {
    ...
    $('#select_type').keypress(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            $('#txtquantity0').focus(); 
            e.preventDefault();
...
function SalesOrderGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        $('#DesignRate').val((parseFloat(result[n].DesignRate||0).toFixed(Decimal)));  
...
function SalesInvGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        $('#DesignRate').val((parseFloat(result[n].DesignRate).toFixed(Decimal)));
        var fcur = result[n].FCGrandTotal;
        $("#fc").text('FC : ' + fcur.toFixed(Decimal));
        $("#fc").css("opacity", '100');
        var rowcount = CountRows();
...
function formrefresh() {
    if (editflag != 0) {
        ...
        $('#btnprint').hide(); $('#btnsubmit').show(); $('#btnlist').show(); $('#btnprintjob').hide();
        ProductFlag1 = 0;
        removetblrow();
        $('#select_location0').prop('disabled', 'true');
...
function productadd() {
    ...
    if ($.trim($("#txtproduct0").val()) == '') {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_location' + id + " class='form-control' onchange ='Locationqtycheckmain(" + id + ")'>" + LocnSelect + "</select></td>" +
            "</tr>";
        $('#tblsalesinvoice').append(ProdRow1);
...
function CheckCancelInvoce() {
    ...
    data.Remarks = '';
    data.Operation = 'Sales Invoice- Update';
    data.DeptId = ERPDeptId;
    $.ajax({
        type: "POST",
