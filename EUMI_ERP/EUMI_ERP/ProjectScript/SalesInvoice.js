...
$(document).keydown(function (e) {
        ...
        if ((x > 111 && x < 124)) {                  //Functional Keys default function block  
            ...
            else if (x == 122) {                                 // F11 :   Pending Purchase Order Popup  
                //PendingPurchasespopup($('#ProductId').val());
                hidepopup(1);
...
function PendingPurchasespopup(ProductId, product, ProductDesc, flag) {
    ...
    if (ProductId != 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var ProdRow = "<thead><tr><th>Order#</th><th>Date</th><th>Rate</th><th>Order Qty.</th><th>Rec.Qty.</th><th>Pending Qty.</th><th>Location</th><th>Department</th><th>User</th></tr>" +
                 "<tr><th>Order#</th><th>Date</th><th style='text-align:right'>Rate</th><th>Order Qty.</th><th>Rec.Qty.</th><th>Pending Qty.</th><th>Location</th><th>Dept.</th><th>User</th></tr></thead><tbody>";
                ...
                if (result.length != 0) {
                    ...
                    for (var p = 0; p < result.length; p++) {
                        ...
                        "<td style='width:1%' class='text-center'>" + result[p].OrderNo + "</td>" +
                        "<td class='text-left' style='width:1%'>" + result[p].OrderDate + "</td>" +
                        "<td style='text-align:right' style='width:1%'>" + parseFloat(result[p].Rate).toFixed(Decimal) + "</td>" +
                        "<td style='text-align:center' style='width:25%'>" + result[p].OrderQty + "</td>" +
                        "<td style='text-align:center' style='width:20%'>" + result[p].RecQty + "</td>" +
                        "<td style='text-align:center' style='width:25%'>" + result[p].PendingQty + "</td>" +
                        "<td class='text-left' style='width:12%'>" + result[p].Location + "</td>" +
                        "<td class='text-left' style='width:10%'>" + result[p].Department + "</td>" +
                        "<td class='text-left' style='width:10%'>" + result[p].User + "</td>" +
                        "</tr>";

                    }

                    $('#tblpendingpurchasetrans').html(ProdRow + '</tbody>');
                    datatableWithsearch('tblpendingpurchasetrans', 'Multiple');
                ...
                $('#PendingPurchaseTransactionPopup').show();
                $('#PendingPurchaseTransactionheader').text(product + '(' + ProductDesc + ')' + ' :  Pending Purchase Order');
...
function PendingTransType() {
    ...
    if ($("#TransPrdtId0").val() != 0) {
        PendingPurchasespopup($("#TransPrdtId0").val(), $('#txtproduct0').val(), $('#ProductDesc0').val(), flag);
    }
    else if ($("#HiddenItemId").val() != 0) {
        PendingPurchasespopup($("#HiddenItemId").val(), $("#HiddenItem").val(), $("#HiddenDesc").val(), flag);
    }
    else if ($("#HiddenGridItemId").val() != 0) {
        PendingPurchasespopup($("#HiddenGridItemId").val(), $("#HiddenGridItem").val(), $("#HiddenGridDesc").val(), flag);
    }
    else if ($("#HiddenAutoItemId").val() != 0) {
        $("#automobiles").hide();
        PendingPurchasespopup($("#HiddenAutoItemId").val(), $("#HiddenAutoItem").val(), $("#HiddenAutoDesc").val(), flag);
    }
}
            ...
            if (result.length != 0) {
                ...
                for (var n = 0; n < result.length; n++) {
                    ...
                    ProdRow += "<tr class='jsgrid-row' id=" + 'pdctrow' + (n + 1) + ">" +
                       ...
                       //"<td style='width:15%' class='text-left'>" + result.oList[n].CustAddress + "                                   </td>" +
                       ...
                       //"<td style='width:7%' class='text-left'>" + result.oList[n].LPONumber + "                                   </td>" +
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if((MultiPdtArray.indexOf("" + result[l].ItemId + "") == -1))
            {
                ...
                var responseText =
                    ...
                    '<td style="text-align:center;" ><select  style="height:30px;" class=form-control id=' + 'Itemunitgrid' + slno + ' onkeydown=Focusnext(event,' + slno + ',"u") onchange=Checkme(' + slno + ')>' + UnitSelect + '</select></td>' +
...
function checkratestock(id) {
   ...
   // if ($("#SlNoHeadCheckgrid" + id).is(":checked")) {
    ...
    if (($('#Itemunitgrid' + id).find("option:selected").attr("name") != 'FOC') && (BelowCost == 'NO') && ($('#pravgcost' + id).val() != 0) && (parseFloat($('#prrate' + id).val()) < parseFloat($('#pravgcost' + id).val()))) {
...
function addgridproducts() {
        ...
        for (j = 1; j <= $('#mltrcurrlength').val() ; j++)
        {
            ...
            {
                if (($('#Itemunitgrid' + j).find("option:selected").attr("name") != 'FOC') && (BelowCost == 'NO') && ($('#pravgcost' + j).val() != 0) && (parseFloat($('#prrate' + j).val()) < parseFloat($('#pravgcost' + j).val()))) {
                    rateflg = 1;                   
                }
                else
                    rateflg = 0;
        ...
        if (Prodarray.length <= 0) {
            ...
            for (var b = 0; b < Prodarray.length; b++) {
                ...
                if ($("#SlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#Itemunitgrid" + m).val() == '') {
                    ...
                    else if (($("#prdamnt" + m).val() == '' || $("#prdamnt" + m).val() == 0) && ($('#Itemunitgrid' + m).find("option:selected").attr("name") != 'FOC')) {  
                        ...
                        var ProdRows = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $('#ItemIdgrid' + m).val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $('#ItemIdgrid' + m).val() + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $('#Itemcodegrid' + m).val() + "'  data-toggle='tooltip' title='" + $('#Itemcodegrid' + m).val() + "'></td>" +
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $('#Itemdescgrid' + m).text() + "' data-toggle='tooltip' title='" + $('#Itemdescgrid' + m).text() + "'></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat($('#prLineAvgCost' + m).val()).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat($('#pravgcost' + m).val()).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                ...
                if (usermenu1.indexOf("M230") == -1) {
                    $('.ShowAverageCost').css('display', 'none');
                }
                else {
                    $('.ShowAverageCost').css('display', '');
                }
            }
            TemporarySalessave();
        }
...
function Checkme(Id) {
        if (($('#qtypr' + Id).val() != '') && ((($('#prrate' + Id).val() != '') && ($('#prdamnt' + Id).val() != 0)) || ($('#Itemunitgrid' + Id).find("option:selected").attr("name") == 'FOC'))) { 
...
function savesalesconfirm()
{
    ...
    for (var k = 1; k <= i - 1; k++) {
        ...
        if ($('#select_locn').val() == 0 || $('#select_locn').val() == '' || $('#select_locn').val() == null)
    ...
    if (result.length > 0)
    {
        ...
        for (var n = 0; n < result.length; n++) {
            ...
            var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 3 + ") onfocusout='updaterow(" + id + ")'  id=" + 'row' + id + " class='jsgrid-row'>" +
                      "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                      "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + " </td>" +
                      "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + Otherdescription + "'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                      "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                      "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                      ...
                      "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (result[n].AvgCost * result[n].ProdQty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat(result[n].AvgCost).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                      "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
                      "</tr>";
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'enqno' + id + " style='display:none' value='" + enqno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        ...
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckQtnItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + qtnno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSoItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'slordsubid' + id + " style='display:none' value='" + ordersubid + "' /><input type='text' id=" + 'slordeno' + id + " style='display:none' value='" + orderno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSIItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'bilsers' + id + " style='display:none' value='" + srsno + "' /><input type='text' id=" + 'bilnum' + id + " style='display:none' value='" + srlno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSoItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'delvsubid' + id + " style='display:none' value='" + ordersubid + "' /><input type='text' id=" + 'delvno' + id + " style='display:none' value='" + orderno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "' data-toggle='tooltip' title='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
...
function CustomerEnquiryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'enqno' + id + " style='display:none' value='" + result[n].EnquiryNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)'   onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfctaxamnt' + id + " value=" + parseFloat(result[n].FCTaxAmount).toFixed(Decimal) + "  style='display:none'><input typ='text' style='background-color:white;height:30px' class='form-control text-center'  id=" + 'txttaxamnt' + id + " value=" + parseFloat(result[n].TaxAmount).toFixed(Decimal) + " disabled=''></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcamnt' + id + " value=" + parseFloat(result[n].FCAmount).toFixed(Decimal) + "  style='display:none'><input typ='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtamnt' + id + " value=" + parseFloat(result[n].Amount).toFixed(Decimal) + "></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat(result[n].AvgCost * result[n].ProdQty).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat(result[n].AvgCost).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                   "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
                  "</tr>";
        $('#tblsalesinvoice').append(ProdRow1);
        $('#select_unit' + id).val(result[n].UnitId);
...
function ShowItemGet(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'enqnoRow' + slno + '>' + result[l].EnquiryNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].EnquiryNo + '></td>' +
        ...
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="binenq' + slno + '" value=' + bins + '></td></tr>';
    }
...
function ShowItemGetQtn(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'qtnnoRow' + slno + '>' + result[l].QuotationNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].QuotationNo + '></td>' +
        ...
        '<td style=display:none;><input type="text" style="display:none;" id= ' + 'taxableamt' + slno + ' value= ' + result[l].TaxableAmount + '><input type="text" style="display:none;" id= ' + 'fctaxableamt' + slno + ' value= ' + result[l].FCTaxableAmount + '></td>' +
        '<td id=' + 'taxamt' + slno + '>' + parseFloat(result[l].TaxAmount).toFixed(Decimal) + '</td><input type="hidden" id="taxamt_' + slno + '" value=' + result[l].TaxAmount + '><input type="hidden" id="fctaxamt_' + slno + '" value=' + result[l].FCTaxAmount + '>' +
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="binqtn' + slno + '" value=' + bins + '></td></tr>';
    }
...
function QtnEntryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + result[n].QuotationNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function SalesOrderGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'slordsubid' + id + " style='display:none' value='" + result[n].SalesOrderSubId + "' /><input type='text' id=" + 'slordeno' + id + " style='display:none' value='" + result[n].OrderNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'   onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
...
function ShowItemGetSalesOrder(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'qtnnoRow' + slno + '>' + result[l].OrderNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].OrderNo + '></td>' +
        ...
        '<td style=display:none;><input type="text" style="display:none;" id= ' + 'taxableamt' + slno + ' value= ' + result[l].TaxableAmount + '><input type="text" style="display:none;" id= ' + 'fctaxableamt' + slno + ' value= ' + result[l].FCTaxableAmount + '></td>' +
        '<td id=' + 'taxamt' + slno + '>' + parseFloat(result[l].TaxAmount).toFixed(Decimal) + '</td><input type="hidden" id="taxamt_' + slno + '" value=' + result[l].TaxAmount + '><input type="hidden" id="fctaxamt_' + slno + '" value=' + result[l].FCTaxAmount + '>' +
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="binsor' + slno + '" value=' + bins + '></td></tr>';
    }
...
function SalesInvGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><select id=" + 'select_location' + id + " value=" + result[n].LocnId + " style='background-color:white;height:30px'  class='form-control' onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function ShowItemGetSalesInvoice(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'billnoRow' + slno + '>' + result[l].BillSlNo + '<input type="hidden" id=' + 'billnumber' + slno + ' value=' + result[l].BillSlNo + '></td>' +
        ...
        '<td style=display:none;><input type="text" style="display:none;" id= ' + 'taxableamt' + slno + ' value= ' + result[l].TaxableAmount + '><input type="text" style="display:none;" id= ' + 'fctaxableamt' + slno + ' value= ' + result[l].FCTaxableAmount + '></td>' +
        '<td id=' + 'taxamt' + slno + '>' + parseFloat(result[l].TaxAmount).toFixed(Decimal) + '</td><input type="hidden" id="taxamt_' + slno + '" value=' + result[l].TaxAmount + '><input type="hidden" id="fctaxamt_' + slno + '" value=' + result[l].FCTaxAmount + '>' +
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="binsin' + slno + '" value=' + bins + '></td></tr>';
    }
...
function DeliveryOrderGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'delvsubid' + id + " style='display:none' value='" + result[n].DeliveryOrderSubId + "' /><input type='text' id=" + 'delvno' + id + " style='display:none' value='" + result[n].DeliveryOrderNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "' data-toggle='tooltip' title='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "' data-toggle='tooltip' title='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><select id=" + 'select_location' + id + " value=" + result[n].LocnId + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                 "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumberInt(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function ShowItemGetDeliveryOrder(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'delvordsubid' + slno + ' style=display:none;>' + result[l].DeliveryOrderSubId + '</td>' +
        ...
        '<td style=display:none;><input type="text" style="display:none;" id= ' + 'taxableamt' + slno + ' value= ' + result[l].TaxableAmount + '><input type="text" style="display:none;" id= ' + 'fctaxableamt' + slno + ' value= ' + result[l].FCTaxableAmount + '></td>' +
        '<td id=' + 'taxamt' + slno + '>' + parseFloat(result[l].TaxAmount).toFixed(Decimal) + '</td><input type="hidden" id="taxamt_' + slno + '" value=' + result[l].TaxAmount + '><input type="hidden" id="fctaxamt_' + slno + '" value=' + result[l].FCTaxAmount + '>' +
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="bindelv' + slno + '" value=' + bins + '></td></tr>';
    }
...
function formrefresh() {
    ...
    // else {
        ...
        $('#CurrentTimeSales,#SalesDepartment').val('');
...
function productaddconfirm() {
    ...
    else if (($('#select_unit0').find("option:selected").attr("name") != 'FOC') && (parseFloat($("#txtrate0").val()) < parseFloat($('#AvgCost0').val())) && (BelowCost == 'NO')) {
        warningshow('Rate must be greater than' + $('#AvgCost0').val(), 'txtrate0');
        $('#txtrate0').select();
    }
    else if (($('#select_unit0').find("option:selected").attr("name") != 'FOC') && ($("#txtamnt0").val() == 0)) { 
...
function productadd()
{
    ...
    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $("#PrdtId0").val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")'  id=" + 'row' + id + " class='jsgrid-row'>" +
        "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +         
        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
        "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $("#PrdtId0").val() + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + $("#Otherdescription0").val() + "'><input type='text' style='display:none;height:30px' id=" + 'MRP' + id + " value='" + $("#MRP0").val() + "'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $("#txtproduct0").val() + "' data-toggle='tooltip' title='" + $("#txtproduct0").val() + "'></td>" +
        "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $("#ProductDesc0").val() + "' data-toggle='tooltip' title='" + $("#ProductDesc0").val() + "'></td>" +
        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
         "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select style='background-color:white;height:30px' id=" + 'select_location' + id + " class='form-control' onchange ='GetBin(" + id + "," + $("#PrdtId0").val() + ",0),Locationqtycheckmain(" + id + ")' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
        ...
        "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat($('#txtfcdiscount0').val()).toFixed(Decimal) + "  style='display:none'><input type='text' id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat($('#txtdiscount0').val() || 0).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'d'," + id + ") onfocusout=focusoutgrid('d'," + id + ") onfocusin=focusingrid('d'," + id + ")></td>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-center' style='width:38px'><select style='background-color:white;height:30px' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)' onkeydown=Focusnextgrid(event,'t'," + id + ") onfocusout=focusoutgrid('t'," + id + ") onfocusin=focusingrid('t'," + id + ")>" + TaxSelect + "</select></td>" +
        "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:22px'><input typ='text' disabled='s' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + ($('#txttaxpercent0').val() || 0) + " onkeyup='amountcalculation(" + id + ")'></td>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat($('#LineAvgCost0').val()).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat($('#AvgCost0').val()).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
        "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
        "</tr>";
...
function JobAdd() {
    ...
    //}
        ...
        var prodjob = "<tr onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='0' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $("#productjob").val() + "' data-toggle='tooltip' title='" + $("#productjob").val() + "'></td>" +
            "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='JOB' data-toggle='tooltip' title='JOB'></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value='0.00' style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value='0.00' style='background-color:white;height:30px'></td>" +
            "</tr>";
...
function updaterow(id) {
    ...
    else if (($('#select_unit' + id).find("option:selected").attr("name") != 'FOC') && ($("#txtamnt" + id).val() == 0)) {
    ...
    else if (($('#select_unit' + id).find("option:selected").attr("name") != 'FOC') && (BelowCost == 'NO') && ($('#PrdtId' + id).val() != 0) && ($('#AvgCost' + id).val() != 0) && (parseFloat($('#txtrate' + id).val()) < parseFloat($('#AvgCost' + id).val()))) {
...
function copyrefresh() {
    ...
    loadcust();
    $('#select_transfer').val(0);
    $('#txtcustomer,#SalesDepartment').val('');
    $('#select_terms').val(0);
    $('#txtduedate').val('');
    $('#txtaddress').val('');
    $('#adr1').val('');
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if ($('#select_location0').val() == 1) { if (result[l].Bin_A != '') { bins = bins + result[l].Bin_A; } }
                ...
                var responseText =
                    ...
                    '<td style="text-align:center;" ><select  style="height:30px;" class=form-control id=' + 'autoItemunitgrid' + slno + ' onkeydown=autoFocusnext(event,' + slno + ',"u") onchange=autoCheckme(' + slno + ')>' + UnitSelect + '</select></td>' +
...
function autoclearcheck(Id) {
     ...
     {
         if (($('#autoqtypr' + Id).val() != '') && ((($('#autoprrate' + Id).val() != '') && ($('#autoprdamnt' + Id).val() != 0)) || ($('#autoItemunitgrid' + Id).find("option:selected").attr("name") == 'FOC'))) {
         ...
         else if (($('#autoprrate' + Id).val() == '' || $('#autoprrate' + Id).val() == 0) && ($('#autoItemunitgrid' + Id).find("option:selected").attr("name") != 'FOC')) {
             $('#autoSlNoHeadCheckgrid' + Id).prop('checked', false);
    ...
    var flag = $("#autoSlNoHeadCheckgrid0").is(":checked") 
        for (var h = 1; h <= rowCount + 1; h++) {
            if (document.getElementById("autoSlNoHeadCheckgrid" + h) != null) {
                if (($('#autoqtypr' + h).val() != '') && ((($('#autoprrate' + h).val() != '') && ($('#autoprdamnt' + h).val() != 0)) || ($('#autoItemunitgrid' + h).find("option:selected").attr("name") != 'FOC'))) {
...
function autocheckratestock(id) {
   // if ($("#autoSlNoHeadCheckgrid" + id).is(":checked")) {
    if (($('#autoItemunitgrid' + id).find("option:selected").attr("name") != 'FOC') && (BelowCost == 'NO') && ($('#autopravgcost' + id).val() != 0) && (parseFloat($('#autoprrate' + id).val()) < parseFloat($('#autopravgcost' + id).val()))) {
...
function autoaddgridproducts() {
    ...
    for (j = 1; j <= cnt ; j++) {
        if ($("#autoSlNoHeadCheckgrid" + j).is(":checked")) {
            if (($('#autoItemunitgrid' + j).find("option:selected").attr("name") != 'FOC') && (BelowCost == 'NO') && ($('#autopravgcost' + j).val() != 0) && (parseFloat($('#autoprrate' + j).val()) < parseFloat($('#autopravgcost' + j).val()))) {
                rateflg = 1;
            }
            else
                rateflg = 0;  
        ...
        if (autoProdarray.length <= 0) {
            ...
            for (var b = 0; b < autoProdarray.length; b++) {
                ...
                if ($("#autoSlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#autoItemunitgrid" + m).val() == '') {
                    ...
                    else if (($("#autoprdamnt" + m).val() == '' || $("#autoprdamnt" + m).val() == 0) && ($('#autoItemunitgrid' + m).find("option:selected").attr("name") != 'FOC')) {
                        ...
                        var ProdRows = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $('#autoItemIdgrid' + m).val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                            "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                            "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:40px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $('#autoItemIdgrid' + m).val() + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $('#autoItemcodegrid' + m).val() + "'  data-toggle='tooltip' title='" + $('#autoItemcodegrid' + m).val() + "'></td>" +
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:110px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $('#autoItemdescgrid' + m).text() + "' data-toggle='tooltip' title='" + $('#autoItemdescgrid' + m).text() + "'></td>" +
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'  onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select style='background-color:white;height:30px'  id=" + 'select_location' + id + " class='form-control' onchange ='GetBin(" + id + "," + $('#autoItemIdgrid' + m).val()  + ",0),Locationqtycheckmain(" + id + ")' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat($('#autoprLineAvgCost' + m).val()).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat($('#autopravgcost' + m).val()).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                            "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
                            "</tr>";
                        $('#tblsalesinvoice').append(ProdRows);
