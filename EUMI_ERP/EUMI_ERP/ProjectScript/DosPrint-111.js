...
function PrintthisBillHospitalSales(Type, Rowlen, Flag) {
  ...
  //TypeLine += '<tr><td>' + Leftalign + '</td><td>' + String.fromCharCode(27) + "E" + 'No  Description                             Qty     Rate       Amount' + String.fromCharCode(27) + "F" + '</td></tr>'
    TypeLine += '<tr><td>' + Leftalign + '</td><td>Item               Batch    Exp   Qty  Rate   Amount</td></tr>'             //'MRP       Taxable      Tax     ':31  chars

    Line = "<tr><th></th></tr>";
    //Line += '<tr><td>.</td></tr>'               //<tr><td>.</td></tr><tr><td>.</td></tr><tr><td>.</td></tr>     
    //Line += '<tr><td>.</td></tr><tr><td>.</td></tr>'   //
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("         " + Space).substring(0, 9) + String.fromCharCode(27) + "E" + Hosp + String.fromCharCode(27) + "F" + '       </td></tr>'
    ...
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("         " + Space).substring(0, 9) + ' DLNO:KL-TVM-162016,KL-TVM-162017                              </td></tr>'


    Line += '<tr><td>' + Leftalign + '</td><td>' + Break + '</td></tr>'
    //Line += '<tr><td>' + Leftalign + '</td><td>' + ("        " + Space).substring(0, 23) + ' ' + String.fromCharCode(27) + BillType + String.fromCharCode(27)  + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Reg No :" + $('#HRegNo').val() + Space).substring(0, 30) +    (' Bill No   :' + BillNo + Space).substring(0, 25) + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Patient:" + $('#HPatient').val() + Space).substring(0, 30) +  (' Date      :' + $('#HSalesDate').val() + Space).substring(0, 25) + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Doctor :" + Doctor + Space).substring(0, 40) + '</td></tr>'
    ...
    for (var i = 1; i <= Rowlen; i++) {
        ...
        if ($.trim($('#Product' + Id).val()) != undefined && $.trim($('#Product' + Id).val()) != 'undefined' && $.trim($('#Product' + Id).val()) != ' ' && $.trim($('#Product' + Id).val()).length > 0) {
          ...
          //  Amnt = Amnt.substring(Amnt.length - 10, Amnt.length)
            ...
            if (i > 9) {
                Line += '<tr><td>' + Leftalign + '</td><td>' + (Descr + Space).substring(0, 20) + (Batch + Space).substring(0, 6) + " " + Exp.substring(0, 6) + Qty.substring(Qty.length - 3, Qty.length) + " " + Rate + "" + Amnt + '</td></tr>';    //+ Mrp.substring(Mrp.length - 10, Mrp.length) + " " + Taxable.substring(Taxable.length - 10, Taxable.length) + " " + Tax.substring(Tax.length - 10, Tax.length) + " "
            }
            else {
   ...
   // var Rof = parseFloat($('#TotRoundOff').val() || 0)
    ...
    Leftalign = '                              ';
    var space="                             "
    var TotalLine1 = space+'MRP TOTAL    :' + (Leftalign + Totalam).substr((Leftalign + Totalam).length - 10);
...
function PrintthisBillLabDotmatrix(Rowlen, Type) {
    ...
    Line += '<tr><td>' + Leftalign + '</td><td>' + Break + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Reg No :" + $('#RegNo').val() + Space).substring(0, 20) + '       ' + String.fromCharCode(27) + BillType + '  ' + String.fromCharCode(27) + 'Bill No     :' + BillNo + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Patient:" + $('#Name').val()  + Space).substring(0, 30) + String.fromCharCode(27) + '              ' + String.fromCharCode(27) + 'Date        :' + $('#BillDate').val() + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Doctor :" + Doctor            + Space).substring(0, 30) + String.fromCharCode(27) + '              ' + String.fromCharCode(27) + 'Gender/Age  :' + Gend +'</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("OP#    :" + OPNo).substring(0, 30) + String.fromCharCode(27) + '              ' + String.fromCharCode(27) + 'IP#         :' + IPNo + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + Break + '</td></tr>'
    Line += TypeLine;
