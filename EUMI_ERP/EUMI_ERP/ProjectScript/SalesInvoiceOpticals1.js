...
function Billseriesload(result) {
    if (result.length == 0) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            //}
            if (result[i].BillType == 'SALESORDERINVOICE') {
                $("#HBillSeries").append("<option value='" + result[i].id + "' name='" + result[i].CurrentNo + "' >" + result[i].BillDescription + "</option>");
                $('#HBillNo').val(result[i].CurrentNo);
            }
        }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.length > 0) {
                    ...
                    var responseText = "<thead><tr><th>Bill#</th><th>Date</th><th>Type</th><th>Customer</th><th>Status</th><th>Account</th><th>Quantity</th></tr>" +
                        "<tr><th>Bill Number</th><th> Date</th><th>TransType</th><th>Customer</th><th>Status</th><th>Account</th><th>Quantity</th></tr></thead><tbody>";
...
function SerachSalesBill() {
    $("#HBillNoCopy").autocomplete({
        ...
        source: function (request, response) {
            ...
            data.DeptId = ERPDeptId;
            $.ajax({
                url: '../SalesInvoice/SerialNoSearchOPTICALSINVOICE',
                type: "POST",
                data: data,
                dataType: "json",
                success: function (data) {
...
//SOsearch
///////////////////////////////correct///////////////////
function SerachSalesBillSO() {
   
    
    $("#HBillNoCopySO").autocomplete({
        delay: 0,
        minLength: 0,
        source: function (request, response) {
            
            var data = {};
            data.BillSlNo = $("#HBillNoCopySO").val();
            data.BillSeriesId = 3;
            data.LocId = $("#HDoctor").val();
            data.DeptId = ERPDeptId;
            $.ajax({
                ...
                success: function (data) {
                    response($.map(data, function (item) {
                        return ({
                            ...
                            billseries: item.BillSeriesId,
                            DeptId: item.DeptId,
                            BillDesc: (item.BillDescription).split('##')[1],
                            headers: ["Bill No", "Customer Name", "Inv Date"]
                        })
                    }));
                }
        ...
        autoFocus: true,
        select: function (event, ui) {
            if (ui.item.BillDesc != 0) {
                swal('SI No :' + ui.item.BillDesc + '*', "Already sales have been done against this order.", "error");
                $('.swal-button').hide();
               // $('#HBillNoCopySO').val('');
                SONUMBER = 0;
                ...
                SalesGetCallsalesorder(ui.item.billseries, ui.item.billsl, ui.item.DeptId, 0);
                SONUMBER = ui.item.billsl;
...
function SaveAndUpdate(flg) {
    ...
    for (var i = 1; i <= RowCount; i++) {
        if ($('#Product' + i).val() != undefined) {
            ...
            oArray.push({
                ...
                'CurrencyId': $("#TAdvamount").val(), //ADVANCE AMOUNTSAVE
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesInvoiceInsertOPTICALSINVOICE",
                data: data,
                success: function (result) {
        ...
        else if (flg == 2)                                            //Update  
        {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesInvoiceUpdteOPTICALSINVOICE",
                data: data,
                success: function (result) {
...
function BillCopySO(flg)      //flg:0 - Copy ,flg :1-View  
{
    CopyflagSO = 1;
    copyrefreshSO(0);
...
function CallViewList() {
    ...
    $.ajax({
        type: "POST",
        url: '../SalesInvoice/SalesInvoiceGetListOPTICALSINVOICE',
        data: data,
        success: function (result) {
...
//////////////////////FORSHOWING==>   VIEW==> LIST
function CallViewListSO() {
    var data = {};
    data.FromDate = $('#DateFrom').val();
    data.ToDate = $('#DateTo').val();
    data.DeptId = $('#select_dept').val();
        ...
        success: function (result) {
            ...
            if (result.length != 0) {
                ...
                for (var n = 0; n < result.length; n++) {
                    ...
                    ProdRow += "<tr class='jsgrid-row' ondblclick='SalesGetCallsalesorder(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)'>" +
                        //  "<td style='width:3%' class='text-center'>" +
                        //"<button class='btn white btn-round btn-xs' type='button' style='background-color:#FF9800;font-size:smaller;margin:0' onclick='SalesGetCall(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)' data-toggle='tooltip' data-placement='top' data-original-title=Print><i class='ft-printer'></i></button>" +
                        //"</td>" +
                        "<td style='width:3%' class='text-center'>" +
                        "<button class='btn  white btn-round btn-xs' type='button' style='background-color:black;font-size:smaller;margin:0' onclick='SalesGetCallsalesorder(" + result[n].BillSeriesId + "," + result[n].BillSlNo + "," + result[n].DeptId + ",1)' data-toggle='tooltip' data-placement='top' data-original-title=Preview> <i class=ft-eye></i></button>" +
                        "</td>" +
                        "<td class='text-left'> " + result[n].BillDescription + " - " + result[n].BillSlNo + "</td>" +
                        "<td class='text-center'>" + result[n].DepartmentName + " </td>" +
                        "<td class='text-center'>" + result[n].InvDate + "                                   </td>" +
...
function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    if (flg == 3) {
        ...
        $('#Copysales,#CopysalesSO,#btnprint,#btnedit,#btndelete,#btnsubmit,#btnview,#btnlist,#btnacctran,#btnprec').hide();
        ...
        if (flg != 2) {
            ...
            $('#HBillNoCopySO').val($('#HBillNoSO').val());
        }
        $('#Copysales,#CopysalesSO,#btnnew,#btnview,#btnadd,#btnsubmitsgo,#confirmCancel,#confirmOk').show();
        RowCount = 1;
        $('.SlRow').remove();
...
function SalesGetCall(BillSeriesId, BillSlNo, DeptId, flg) {
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesGetandGetsOPTICALSINVOICE",
        data: data,
        success: function (result) {
            if (result.length > 0) {
...
function SalesGetCallsalesorder(BillSeriesId, BillSlNo, DeptId, flg) {
...
function SalesGetandGets(result) {
    ...
    $('#TAdvamount').val(result[0].CurrencyId);//ADVANCE RECEVED
  ...
  $('#HBillNoCopySO').val(result[0].Terms);
...
function SalesGetandGetsOrder(result) {
    ...
    $('#SalesMainId').val(result[0].SalesMainId);
    $('#HBillSerieshiden').val(result[0].HBillSeries);
    $('#HBillNoSO').val(result[0].HBillNo);
   
    $('#HBillNoCopySO').val(result[0].HBillNo).focus().select();
    ...
    for (var i = 0; i < result.length; i++) {
        if (result[i].ProductId != 0) {
            ...
            var ProdRow =
                ...
                '<td class="jsgrid-align-center crspnt" style="width:2%" onclick=DeleteRow(0,' + Id + ')><i class="icon-trash"></i></td>' +
                '<td class="jsgrid-align-center" style="width:3%" id="tdSl' + Id + '">' + SlNo + '</td>' +
                '<td class="jsgrid-align-left" style="width:13%"><input type="text" class="form-control smallTextbox brnone " id="Product' + Id + '" name="Product"  style="width:100%"  value="' + result[i].ProductDesc + '" onfocusout=UpdateRow(1,' + Id + ')  /></td>' +

                '<td class="jsgrid-align-left" style="width:5%"><input type="text"  class="form-control smallTextbox brnone " id="ProductDesc' + Id + '" name="ProductDesc"  style="width:100%"  value="' + result[i].Batch + '" onfocusout=UpdateRow(2,' + Id + ')  /></td>' +


                '<td class="jsgrid-align-center" style="width:5%"><input type="hidden"  class="form-control smallTextbox brnone  bgclrwhite" id="Company' + Id + '"  style="width:100%"  value="' + BICOM[0] + '"  />' + BICOM[1] + '</td>' +

                '<td class="jsgrid-align-center" style="width:5%"><input type="text"  class="form-control smallTextbox brnone  bgclrwhite" id="Expiry' + Id + '"  style="width:100%"  value="' + result[i].Expiry + '"  /></td>' +
                '<td class="jsgrid-align-center" style="width:5%"><input type="text"  class="form-control smallTextbox brnone Itemclass" name="Quantity" id="Quantity' + Id + '" onkeypress="isNumberInt(event, this)" onkeyup="ClearFields(4,0),AmountCalc(' + Id + ')"  style="width:100%"  value="' + result[i].Quantity + '" /></td>' +
                '<td class="jsgrid-align-center" style="width:5%"><input type="text"  class="form-control smallTextbox brnone Itemclass" name="Free" id="Free' + Id + '"     onkeypress="isNumberInt(event, this)" style="width:100%"  value="' + result[i].Free + '" /></td>' +
                '<td class="jsgrid-align-center" style="width:5%"><input type="text"  class="form-control smallTextbox brnone Itemclass " name="SellPrice" id="SellPrice' + Id + '" onkeypress="isNumber(event, this)" onkeyup="ClearFields(4,0),AmountCalc(' + Id + ')"   style="width:100%"  value="' + parseFloat(result[i].SellPrice || 0).toFixed(Decimal) + '"  /></td>' +
                '<td class="jsgrid-align-right" style="width:4%"><input type="text"  class="form-control smallTextbox brnone rdonl bgclrwhite" id="PurPrice' + Id + '"  style="width:100%"  value="' + parseFloat(result[i].PurPrice || 0).toFixed(Decimal) + '"  /></td>' +

                '<td class="jsgrid-align-center" style="width:5%"><input type="text"  class="form-control smallTextbox brnone  bgclrwhite" id="Amount' + Id + '"  style="width:100%"  value="' + parseFloat(result[i].Amount) + '"  /></td>' +
                ...
                '</div>' +
                '</td>' +
                '<td class="jsgrid-align-center" style="width:6%"> <div class="input-group m-0"><select id="Tax' + Id + '" class="form-control smallTextbox Itemclass pl-0 IpTax" style="width:60%;border-radius:0px;border:none;border-right: 1px solid lightgrey!important;padding: 0;" onchange="TaxChange(' + Id + ')"> ' + TaxSelect + ' </select> <input type="text" id="TaxPercent' + Id + '" style="width:40%;border-radius:0px;border:none;border-left: 1px solid lightgrey!important;" class="form-control smallTextbox rdonl bgclrwhite Itemclass"> </div></td>' +
                '<td class="jsgrid-align-right" style="width:5%"> <input type="text" id="TaxableAmt' + Id + '" class="form-control smallTextbox rdonl bgclrwhite Itemclass brnone"   value="' + parseFloat(result[i].TaxableAmt || 0).toFixed(Decimal) + '"></td>' +
                '<td class="jsgrid-align-right" style="width:5%"><input type="text" id="TaxAmt' + Id + '" class="form-control smallTextbox rdonl bgclrwhite Itemclass brnone"   value="' + parseFloat(result[i].TaxAmt || 0).toFixed(Decimal) + '"></td>' +
                '<td class="jsgrid-align-right" style="width:5%"><input type="text" id="CessAmount' + Id + '" class="form-control smallTextbox rdonl bgclrwhite Itemclass brnone" onkeypress="isNumberInt(event, this)"  value="' + parseFloat(result[i].CessAmount || 0).toFixed(Decimal) + '"></td>' +
                '<td class="jsgrid-align-right" style="width:5%"><select id="DrugSchedule' + Id + '" class="form-control smallTextbox Itemclass brnone" name="DrugSchedule">' + DrugSchedule + '</select></td>' +
                '<td class="jsgrid-align-center" style="width:5%;display:none">' +
                '<input type="text" class="form-control" id="ProductId' + Id + '"  value="' + result[i].ProductId + '" />' +
                '<input type="text" class="form-control" id="BatchSlNo' + Id + '"  value="' + result[i].BatchSlNo + '" />' +
    ...
    $('#TAdvamount').val(parseFloat(result[0].SpecialFeeAmt + result[0].JobNo + result[0].Area))
    $('#Advancepaid').text('ADV-PAID :'+parseFloat(result[0].SpecialFeeAmt + result[0].JobNo + result[0].Area).toFixed(2));
    if ($('#TAdvamount').val() > 0) {
        swal('RS-' + parseFloat($('#TAdvamount').val()).toFixed(2) + '*', "Advance payment has already been made.", "warning");
        $('.swal-button').hide();
    }
...
function DeleteBill(Flag) {      //Flag=0:Delete Contract  
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesInvoiceDeleteOPTICALSINVOICE",
        data: data,
        success: function (result) {
...
function GetBillPrevousornextSO(Value) {
    var SlNo = parseInt($('#HBillNoCopySO').val() || 0);
    SlNo = SlNo + Value;
    if ((SlNo <= 0) || (SlNo >= CurrentBillNo)) {
        warningshow('Bill Number Not Valid', 'HBillNoCopySO');
        return false;
    }
    else {
        $('#HBillNoCopySO').val(SlNo);
        copyrefresh(2);
        SalesGetCallsalesorder($('#HBillSeries').val(), SlNo, ERPDeptId, 0);
    }
}
//-----End Edit and Delete
...
function CustPrdctLoad(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow = "<tr class='jsgrid-row' id='pdctrow'>" +
            ...
            "<tr class='jsgrid-row'><td colspan=2 class='text-left' style='border:none;font-weight:500'>    <b> SO STOCK  :" + result[n].Bin_E + "   </b> &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;    <b> LOCATION  :" + result[n].Bin_A + "</b></td ><td colspan=2  id='expdayse'  class='text-left' style='border:none;font-weight:700;color:yellow'><b></b></td ></tr>";
...
function PrintthisBillHospital(Type, Count, Flag) {
    ...
    if (PrintType == 'HMSSALESLOCAL')
        ...
        PrintthisBillWindowsHMS('SALESOPTICALS', Count, Flag, 0);
...
function DivHideShow(Flag) {
    ...
    else if (Flag == 2)     // All Transaction
    {
        ...
        $("#PurchaseTransactionheader").html('<span style="color:red;">' + ($('#Product0').val() + '</span> : SO Transactions'));
    }
...
function formrefresh(flg) {
    if (flg == 0)     //New Button click
    {
        ...
        $('#Copysales,#CopysalesSO,#btnsaveedit,#btnprint,#btnedit,#btndelete,#btnacctran,#btnprec,#btnprecold').hide();
        $('select:not(.disb)').prop('disabled', false);
