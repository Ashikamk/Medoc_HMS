...
function FocusManual(Id,Dest,key,e)
{
    ...
    if ((Id == 'Department') || (Id == 'TestAmount'))
    { SLClass = Id; }
...
function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    ...
    if (flg != 3)
    { $('.TsRw').remove(); $('#btnedit,#btndelete,#btnacctran,#btnsubmitnew').hide(); }
    CheckEOD();
}
...
function PrintthisBill(flg)
{
    if(flg==1)
    {
        ...
        { PrintthisBillWindowsHMS('LABBILL', Count, 'COPYHALF','LB'); }
        else if (LabBillType == 'FULLSIZE') 
        { PrintthisBillWindowsHMS('LABBILL', Count, 'COPYFULL', 'LB'); }
        else if (LabBillType == 'FULLSIZEHEADER')
        { PrintthisBillWindowsHMS('LABBILL', Count, 'COPYFULLHEADER', 'LB'); }
        else if (LabBillType == 'DOTMATRIXLAB')
        { PrintthisBillLabDotmatrix(Count, 'LAB BILL'); }
    }
}
