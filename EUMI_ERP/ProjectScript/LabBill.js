   ...
   /////////////////////////
            ...
            var sessionToken = sessionStorage.getItem("LB_Token");
            var fromWorksheet = sessionStorage.getItem("LB_FromWorksheet");

            // Read ALL values BEFORE clearing
            var patientId = sessionStorage.getItem("LB_PatientId");
            var opNumber = sessionStorage.getItem("LB_OPNumber");

            // Clear token and flag immediately
            sessionStorage.removeItem("LB_Token");
            sessionStorage.removeItem("LB_FromWorksheet");
            ...
            // Check if valid worksheet navigation
            var isValid = (urlToken && sessionToken && urlToken === sessionToken && fromWorksheet === "1");

            if (isValid && patientId) {
                setTimeout(function () {
                    $('#RegNo').val(opNumber);
                    GetLastRevisitDetails(patientId);
                }, 800);
            } else {
                // Clear all leftover session data
                sessionStorage.removeItem("LB_PatientId");
                sessionStorage.removeItem("LB_OPNumber");
                sessionStorage.removeItem("LB_OPSeries");
                sessionStorage.removeItem("LB_Gender");
                sessionStorage.removeItem("LB_Age");
                sessionStorage.removeItem("LB_Contact");
                sessionStorage.removeItem("LB_Doctor");
                sessionStorage.removeItem("LB_RevisitId");
...
////////////////////////////////
        ...
        $('#TestAmount0').keydown(function (e) {
            var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
            if (key == 13 || key == 39) {
                e.preventDefault();
                $('#btnadd').focus();
        ...
        $("#btnsubmitnew").click(function (e) {
            // $('#btnsubmitnew').hide();
            SaveAndUpdate(3);
            $('#btnSavedOk,#btnSavedCancelki').show();
        });
    ...
    function RevisitIDLoad() {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.oList.length > 0) {
                    $("#BillNo").val(result.oList[0].BillNo);
                    //$("#BillNoCopy").text(result.oList[0].BillNo); 
                }
                else {
                    $('.form-control').blur();
    ...
    function LoadRegNumber() {
        ...
        $("#RegNo").autocomplete({
            ...
            select: function (event, ui) {
                ...
                $('#Opseriesname').val(ui.item.opser)
                $('#contnum').val(ui.item.Contact);
                PRID = ui.item.PatientId;
    ...
    function GetPatientData(result) {
        ...
        if (age.years == 0) { yearString = age.months + " Months"; }
        else if (age.years > 0) { yearString = age.years + " Years"; }
        else { yearString = age.years + " Year"; }

        $('#RegNo').val(result[0].OPNumber);
        var savedOpNo = sessionStorage.getItem("LB_CorrectOpNo");
        $('#OpNo').val(savedOpNo ? savedOpNo : result[0].RevisitId);
    ...
    function Gettestadviceget() {
        var data = {};
        data.RevisitId = $('#OpNo').val();
        data.OPNumber = $('#RegNo').val();
        data.OPSerId = $('#RegSeries').val();
        $.ajax({
            type: "POST",
            url: "../Revisit/HMS_TestAdviceGets",
            data: data,
            success: function (result) {
                if (result.oList.length > 0) {
                    $('#PrescriptionInfoPopUp').show();
                }
            }
        });
    }
    ...
    function ClearData(flg, Id) {
        if (flg == 0) {
            ...
            RateArray = [];
            $('#TestId' + Id).val('');
            $('#Department' + Id).val($('#Department' + Id).find('option:first').val());
            if (Id == 0) {
                $('#TestAmount' + Id).val('');
            }
            else {
    ...
    function LoadTest(Id) {
        ...
        $('#TestName' + Id).autocomplete({
            ...
            autoFocus: true,
            select: function (event, ui) {
                $('#TestId' + Id).val(ui.item.TestId);
                $('#Department' + Id).val(ui.item.MedDept);
                RateArray.Rate = ui.item.Rate;
                RateArray.SpRate = ui.item.SpRate;
                RateArray.VSpRate = ui.item.VSpRate;
                RateArray.OutsideRate = ui.item.OutsideRate;
                LOadRate(Id);
                //$('#TestAmount' + Id).focus(); 
                $('#btnadd').focus();
            },
            ...
            if ((e.which == 13) && (($('#TestId' + Id).val() || 0) != 0)) {
                ...
                if ((e.which == 39) && ((($('#TestId' + Id).val() || 0) != 0) || ($('#TestName' + Id).val() == ''))) {
                    $('#TestAmount' + Id).focus();
                }
                else if ((e.which == 37) && ((($('#TestId' + Id).val() || 0) != 0) || ($('#TestName' + Id).val() == ''))) {
                    $('#btnadd').focus();
                }
            }
    ...
    function LOadRate(Id) {
        if (RateArray.Rate != undefined) {
            var Rate = 0; var Type = $('#RateType0').val();
            if (Type == 0) { Rate = RateArray.Rate }
            else if (Type == 1) { Rate = RateArray.SpRate }
            else if (Type == 2) { Rate = RateArray.VSpRate }
            else { Rate = RateArray.OutsideRate }

            $('#TestAmount' + Id).val(parseFloat(Rate).toFixed(Decimal));
        }
    }

    function AddTest() {
        var Amt = parseFloat($('#TestAmount0').val());
        Amt = isNaN(Amt) ? 0 : Amt;

        if ($.trim($('#TestName0').val()) == '') {
            warningshow('Please Select Test', 'TestName0');
        }
        else if (($('#TestId0').val() || 0) == 0) {
            warningshow('Please Select a Valid Test', 'TestName0');
        }
        else if (($('#Department0').val() || 0) == 0) {
            warningshow('Please Select Department', 'Department0');
        }
        else if ((Amt || 0) == 0) {
            warningshow('Please enter TestAmount', 'TestAmount0');
        }
        else {
    ...
    function AddTestConfirm() {
        ...
        var ProdRow1 = "<tr id=" + 'Row' + id + " class='jsgrid-row TsRw' onclick='EditRow(" + id + ")' onfocusout=updaterow(" + id + ")>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-left' style='width:53%'><input type='text' id='TestName" + id + "' value='" + $('#TestName0').val() + "' onfocus='LoadTest(" + id + ")' class='form-control TestName' style='height:30px' ></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:25%'><select id='Department" + id + "' class='form-control text-center Department cmdts' style='height:30px' onkeydown=FocusManual('Department','TestName'," + id + ",event)>" + DeptLoad + "</select></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:12%'><input type='text' id='TestAmount" + id + "' value=" + parseFloat(Amt).toFixed(Decimal) + " onkeydown=FocusManual('TestAmount','TestName'," + id + ",event) class='form-control text-center TestAmount cmdts' style='height:30px' onkeypress='isNumber(event,this)'></td>" +
            "<td style=display:none><input type='text' id='TestId" + id + "' value=" + $('#TestId0').val() + " class='form-control prmkey'>" +
            "<input type='text' id='RateType" + id + "' value=" + $('#RateType0').val() + " class='form-control'>" +
            "</td></tr>";
    ...
    function updaterow(id) {
        ...
        var Amt = parseFloat($('#TestAmount' + id).val());
        Amt = isNaN(Amt) ? 0 : Amt;

        if ($.trim($('#TestName' + id).val()) == '') {
            warningshow('Please Select Test', 'TestName' + id);
            return false;
        }
        else if (($('#TestId' + id).val() || 0) == 0) {
            warningshow('Please Select a Valid Test', 'TestName' + id);
            return false;
        }
        else if (($('#Department' + id).val() || 0) == 0) {
            warningshow('Please Select Department', 'Department' + id);
            return false;
        }
        else if ((Amt || 0) == 0) {
            warningshow('Please Enter the Amount', 'TestAmount' + id);
            return false;
        }
        else {
            $('#TestAmount' + id).val(parseFloat(Amt).toFixed(Decimal));
            CalcGrandTotal(1);
            Editflag = 0;
            return true;
        }
    ...
    function formrefresh() {
        ...
        CalcGrandTotal(1);
        $('#BillNo,.btn').show();
        $('#BillNoCopy,#btnprint,#btnedit,#btnupdate,#btndelete,#btnacctran,#btnsubmitnew').hide();
        Copyflag = 0;

        CheckEOD();
        SelectPayType();
        $('#printiuserdet').text('')
        $('#printscheme').prop("checked", false);
        BillYear = '';
    }
    ...
    function FocusManual(Id, Dest, key, e) {
        ...
        if ((Id == 'Department') || (Id == 'TestAmount')) { SLClass = Id; }
        ...
        if (char == 37 || char == 39) {
            if (Id == 'Department') {
                ...
                if (char == 37) {
                    ...
                    e.preventDefault();
                    $('#TestAmount' + key).focus().select();
                }
            }
            else if (Id == 'TestAmount') {
                SLClass = 'TestAmount';
                if (char == 39 || char == 37) {
                    e.preventDefault();
                    $('#TestName' + key).focus().select();
    ...
    function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
        if (flg == 3) {
            ...
            $('#BillNo,.btn').show();
            $('#BillNoCopy,#btnprint,#btnedit,#btndelete,#btnsubmit,#btnview,#btnlist,#btnacctran,#btnsubmitnew').hide();
            $('.rdonl').attr('readonly', true).css('background-color', 'white');
        ...
        if (flg != 3) { $('.TsRw').remove(); $('#btnedit,#btndelete,#btnacctran,#btnsubmitnew').hide(); }
        CheckEOD();
    }
    ...
    function ShowPrescription() {
        ...
        $.ajax({
            type: "POST",
            url: "../Revisit/HMS_LabAdvicegets",
            data: data,
            success: function (result) {
                LabAdviceLoad(result.oList);
            }
        });
        $('#PrescriptionInfoPopUp').hide();
    }

    function LabAdviceLoad(result) {
        for (var n = 0; n <= result.length; n++) {

            $('#TestId0').val(result[n].TestId);
            $('#TestName0').val(result[n].TestName);
            $('#TestAmount0').val(result[n].PRate);
            $('#Department0').val(result[n].Department);
            AddTest()
    ...
    function BillNumberGets(result) {
        ...
        //console.log(result[0].SurgeryDate)
        if (result.length > 0) {
            if (result[0].DeptId == ERPDeptId) {
                $('#btnedit,#btndelete,#btnacctran,#btnprint,#btnsubmitnew').show();
            }


            var offertype = (result[0].SurgeryDate).split("#")
            $('#printscheme').prop("checked", false);
            if (offertype[4] == 'O') {
                $('#printscheme').prop("checked", true);
            }
            ...
            console.log(result[0].SurgeryDate.split("##"))
            ...
            $('#contnum').val(result[0].SurgeryDate.split("#")[5]);
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                var ProdRow1 = "<tr id=" + 'Row' + id + " class='jsgrid-row TsRw' onclick='EditRow(" + id + ")' onfocusout=updaterow(" + id + ")>" +
                    "<td class='jsgrid-cell  jsgrid-control-field jsgrid-align-center' style='width:5%'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
                    "<td class='jsgrid-cell ' style='width:5%;text-align:center' id='slrow" + id + "'>" + slno + "</td>" +
                    "<td class='jsgrid-cell  jsgrid-align-left' style='width:53%'><input type='text' id='TestName" + id + "' value='" + result[i].TestName + "' onfocus='LoadTest(" + id + ")' class='form-control' style='height:30px' ></td>" +
                    "<td class='jsgrid-cell  jsgrid-align-center' style='width:25%'><select id='Department" + id + "' class='form-control Department text-center cmdts' style='height:30px' onkeydown=FocusManual('Department','TestName'," + id + ",event)>" + DeptLoad + "</select></td>" +
                    "<td class='jsgrid-cell  jsgrid-align-center' style='width:12%'><input type='text' id='TestAmount" + id + "' value=" + parseFloat(result[i].TestAmount).toFixed(Decimal) + " class='form-control text-center TestAmount cmdts' onkeydown=FocusManual('TestAmount','TestName'," + id + ",event) style='height:30px' onkeypress='isNumber(event,this)'></td>" +
                    "<td style=display:none><input type='text' id='TestId" + id + "' value=" + result[i].TestId + " class='form-control prmkey'>" +
                    "<input type='text' id='RateType" + id + "' value=" + result[i].RateType + " class='form-control'>" +
                    "</td></tr>";
    ...
    function SaveAndUpdate(flg) {
        ...
        if (($("#BillNo").val() || 0) == 0) {
            ...
            warningshow('Please Add Tests', 'TestName0');
            $('#btnsubmit').show();
        }
        else if ($('.TsRw').length == 0) {
        ...
        for (var k = 1; k <= Count; k++) {
            ...
            if (TestId != undefined) {
                oArray.push({
                    ...
                    'DelFlag': DelFlag,
                    'Type': Type,
                    'SurgeryDate': '' + '#' + a + '#' + b + '#' + c + '#' + d + '#' + $('#contnum').val() + '#' + BILLYR    //cash/upi/card
                })
            }
    ...
    function Showalerts(Status, BillNo, BillYear) {
        ...
        if (Status == 1) {
            ...
            swal('Same Bill Data Found', "Try after sometime", "warning");
            $('.swal-button swal-button--confirm').focus();
        }
