...
function PrintTestResult(Rowlen) {
   ...
   myWindow.document.write('</table>');
    ...
    myWindow.document.write('<table width=100% style="border-collapse: collapse;page-break-inside: avoid;'  + FontSize + '"><tr>' +
        '<td class="p-0" style="width:45%;' + BorderX + '">TEST</td>' +
        '<td class="p-0" style="width:25%;' + BorderX + '">OBSERVED VALUE</td>' +
        '<td class="p-0" style="width:30%;' + BorderX + '">REFERENCE RANGE</td><tr><tr><td colspan=3 ><hr></td><tr>');
    ...
    for (var i = 1; i <= Rowlen; i++) {
        ...
        if (Dept != CurrentDept) {
           ...
           // myWindow.document.write('<tr><td colspan=3 class="p-0" height="30" ><u>' + $("#MedDept_" + i).val() + '</u></td><tr>');
        }
        if(TestHead!=0 && SubTestId==0 && TestHead!=CurrentMainTest)
        {
            CurrentMainTest = $("#TestId_" + i).val();
            myWindow.document.write('<tr><td height="5px"></td></tr>');
            myWindow.document.write('<tr>' +
                '<td style="background-color: #e6e6e6;" colspan=3 class="p-0" height="20" style="' + BorderX + BorderY + '"><b>' + $("#TestName_" + i).text() + '</b></td><tr>');
        }
        else if ($.trim($("#Result_" + i).val())!='') {
            myWindow.document.write('<tr>' +
                '<td valign=top class="p-0"  style="' + BorderX + BorderY + '">' + $("#TestName_" + i).text() + '</td>' +
                '<td valign=top class="p-0"  style="' + BorderX + BorderY + '"><b>' + $("#Result_" + i).val() + ' ' + $("#StdUnit_" + i).text() + '</b></td>' +
                '<td valign=top class="p-0"  style="' + BorderX + BorderY + '">' + $("#NormalValue_" + i).val() + '</td>' +
                '<tr>');
        }
...
function PrintOPMedDescription(Rowlen, type, Flag) {
    if (type == 'HEADERPRINT') {
        ...
        else
        {
            ...
            myWindow.document.write('<table width=100% ><tr ><td width=100% align=center  style=color:#008000;font-weight:bold>' + (ComapnydivToPrintLab.outerHTML) + '</td></tr>');
            myWindow.document.write('</table>');   // myWindow.document.write('<table width=100% ><tr ><td><b>DR. C. N. NAHAS MBBS, MD, DTCD, FCCP </b> </td> <td align=right><b> Reg No:30945(TCMC).</td></tr> <tr ><td><b>CONSULTANT PULMONOLOGIST </b>  </td> <td align=right><b>  drnahas@rediffmail.com</b> </td></tr>  <tr ><td><b>Allergy, Asthma, TB</b>  </td> <td align=right> <b> Mobile:9447172209</td></tr> <tr ><td><b>Chest Specialist </b> </td> <td align=right><b> Consulting Time: 3 PM to 7 PM</b> </td></tr> <tr ><td><b>Govt. Taluk Hospital </b>  </td> <td align=right></td></tr>  <tr ><td><b>Sasthamkotta.</b>  </td> <td align=right></td></tr> </table>');
        ...
        for (var i = 0; i <= Rowlen; i++) {
            ...
            if ($('#PrintMedicine_' + Id).length && $.trim($('#PrintMedicine_' + Id).val()) != '') {
               ...
               // if (Dosage == 0) { Dosage = '' }  else {Dosage=Dosage+' Daily '}
                ...
                myWindow.document.write('<tr ><td colspan=8 ></td></tr><tr><td class="txbld1" colspan=5 style="padding-left:20px">' + medname + '&#160;&#160;&#160;&#160;&#160;' + Dosage + '&#160;&#160;&#160;&#160;&#160;' + Days + ' </td></tr>');
