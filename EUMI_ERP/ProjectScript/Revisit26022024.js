...
function GetAppointmentinfopatient(Pid) {
    debugger;
    /* alert(Pid)*/
...
function ShowAppointmentlist(result) {
    ...
    var EmptyFlag = 0
    var slno = 0;
    var responseText = "<table width=100%><thead><tr><th align=center>Sl#</th><th>RegId</th><th>Name</th><th>Doctor</th><th>Date</th><th>Time</th><th>Mob.No</th><th>Edit</th></tr></thead><tbody>";
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        if (result[i].LastName != '' && result[i].AppointmentDate == LabelDate && result[i].Status1 == 0) {
            ...
            responseText += '<tr><td align=center>' + slno
                ...
                + '</td><td>  <button type="button" style="width:100px"  class="btn btn-primary btn-min-width mr-1 mb-1"  onclick="GetAppointmentinfopatient(' + result[i].Status2 + ',0)">Apply</button>'
                + '</td></tr>';
        }
