...
function AccountLoad(result) {
    ...
    function UnitLoad(result) {
        ...
        for (var i = 0; i < result.length; i++) {
            $("#select_price").append("<option value='" + result[i].UnitId + "'>" + result[i].UnitName + "</option>");
        }
    }
    function SalesLoad(result) {
        $("#select_sales").empty();
    ...
    function SaveAndUpdate(Flag) {
     ...
     data.CustTermsId =parseInt($('#select_terms').val()||0);
    ...
    function ShowSupplierlist(result) {
        disable_datatable('tblsupplier');
        $('#Entry').hide();
        $('#listing').show();
        var responseText = "<thead><tr><th width=1%>Sl#</th><th width=40%>Name</th><th width=10%>Opening Balance</th><th width=10%>Due Days</th><th width=40%>Address</th><th width=10%>Contact#</th width=3%><th>Edit</th></tr></thead><tbody>";
        for (var i = 0; i < result.length; i++) {
            var slno = parseInt(i + 1);
            responseText += '<tr><td>' + slno + '</td><td>' + result[i].CustName + '</td><td align=right>' + parseFloat(result[i].OpenBalance).toFixed(Decimal) + '</td><td>' + result[i].DueDays + '</td><td>' + result[i].CustStreet1 + '</td><td>' + result[i].CustContactNo1 + '</td><td align=center><a onclick="GetRows(' + result[i].CustId + ')">' + Editbutton + '</a></td></tr>';
        }
        $('#tblsupplier').html(responseText + '</tbody><tfoot><tr><th>Sl#</th><th>Name</th><th> </th><th> </th><th>Address</th><th>Contact#</th><th>Edit</th></tr></tfoot>');
        datatableWithsearch1('tblsupplier');
    }
    function datatableWithsearch1(tablename, download, title, tableButtonContainerId) {
        ...
        $(a).each(function () {
            ...
            if (title == 'Name'|| title == 'Address' || title == 'Contact#')
    ...
    function ShowSupplierGet(result) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            $('#myImg').attr('src', "../ProjectImages/Supplier/" + result[i].CustId + ".png");
            $('#txtacnt').focus();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (CustId == 0)
                    ...
                    ShowSupplierlist(result.oList);
                else
                    ShowSupplierGet(result.oList);
    ...
    function fnImageSave(imageName) {
        ...
        if ((imageName != "" && totalFiles != 0)) {
            ...
            if (browsedFile.type.match('image.*')) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/Master/UploadSupplierImage',
                    data: formData,
                    dataType: "html",
                    contentType: false,
                    processData: false,
