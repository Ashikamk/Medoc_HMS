...
function PrintTestResult(Rowlen) {
    ...
    for (var i = 1; i <= Rowlen; i++) {
        ...
        if (Dept != CurrentDept) {
            CurrentDept = Dept;
            myWindow.document.write('<tr><td align=left colspan=3 class="p-0" height="" ><u>' + $("#MedDept_" + i).val() + '</u></td><tr><tr><td height=10px></td></tr>');
        }
        if (TestHead != 0 && SubTestId == 0 && TestHead != CurrentMainTest) {
            CurrentMainTest = $("#TestId_" + i).val();
            myWindow.document.write('<tr><td colspan=3 class="p-0" height="20" style="background-color: #8c8c8c">' + $("#TestName_" + i).text() + '</td><tr>');
        }
        else if ($.trim($("#Result_" + i).val()) != '') {
            myWindow.document.write('<tr>' +
                '<td class="p-0"  >' + $("#TestName_" + i).text() + '</td>' +
                '<td class="p-0"  ><b>' + $("#Result_" + i).val() + ' ' + $("#StdUnit_" + i).text() + '</b></td>' +
                '<td class="p-0"  >' + $("#NormalValue_" + i).val() + '</td>' +
                '<tr>');
        }
        linecount += 1;
    ...
    myWindow.document.write('<table width=100%><tr><td height=20px></td></tr><tr><td colspan=3 align=center>*****  END OF REPORT  *****</td></tr><tr><td height=20px colspan=3 align=center>&#160;</td></tr><tr><td align=left></td><td width=400px align=center></td><td align=center>&#160;Verified By</td></tr></table>');
