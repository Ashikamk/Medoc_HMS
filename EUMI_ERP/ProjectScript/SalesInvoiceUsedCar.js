...
function SalesTransType()
{
      ...
      if ($("#TransPrdtId0").val() != 0) {
          SalesTransactions($("#TransPrdtId0").val(), $('#txtproduct0').val(), $('#ProductDesc0').val());
      }
      else if ($("#HiddenItemId").val() != 0) {
          SalesTransactions($("#HiddenItemId").val(), $("#HiddenItem").val(), $("#HiddenDesc").val());
      }
}
...
function PurchaseTransType()
{
    ...
    if ($("#TransPrdtId0").val() != 0) {
        PurchaseTransactions($("#TransPrdtId0").val(), $('#txtproduct0').val(), $('#ProductDesc0').val());
    }
    else if ($("#HiddenItemId").val() != 0) {
        PurchaseTransactions($("#HiddenItemId").val(), $("#HiddenItem").val(), $("#HiddenDesc").val());
    }
}
...
function AllTransactionsTransType()
{
    ...
    if ($("#TransPrdtId0").val() != 0) {
        ...
        AllTransactions($("#TransPrdtId0").val(), $('#txtproduct0').val(), $('#ProductDesc0').val());
    }
    else if ($("#HiddenItemId").val() != 0) {

        AllTransactions($("#HiddenItemId").val(), $("#HiddenItem").val(), $("#HiddenDesc").val());
    }
}
...
$(document).keydown(function (e) {
    ...
    var x = event.keyCode;
        if ((x > 111 && x < 124)) {
            ...
            if ((x == 118) && (!$("#addacnttype").is(":visible"))) {                     // F7 - Pop Up to Show Sales Transaction Details of Selected Product 
            ...
            else if ((x == 113) && (!$("#addacnttype").is(":visible")) && (!$("#iconForm").is(":visible"))) {    // F2 -   Multiple Products List 
                if (editflag == 0 && copyflag == 0) {
                    ...
                    if (($('#txtproduct0').val()).toUpperCase() == "JOB") {
                        ...
                        Multipleproduct();
                        $('#temptxtproduct0').val($('#txtproduct0').val());
                        $('#txtproduct0').val('');
                        $("#TransPrdtId0").val(0);
                    }
                }
            }
            else if ((x == 119) && (!$("#addacnttype").is(":visible"))) {                         // F8 - Pop Up to Show Purchase Transaction Details of Selected Product 
            ...
            else if ((x == 120) && (!$("#addacnttype").is(":visible"))) {  // F9 :   All Transaction details Popup      
                if (!(($("#HiddenItemId").val() == 0) && ($("#iconForm").is(":visible")))) {
                    salestranspopuprefresh();
                    $('#status_type').prop("checked", false);
                    //closepopupprdtlist();
   ...
   else if ((e.altKey && e.keyCode == 83) && (!$("#addacnttype").is(":visible")) && (!$("#iconForm,#salestranspopupdiv").is(":visible"))) {                        //Alt+S
       if (copyflag != 1 && updateflag == 0 && editflag == 0)
    ...
    else if ((e.altKey && e.keyCode == 67) && (!$("#addacnttype").is(":visible")) && (!$("#iconForm,#salestranspopupdiv").is(":visible"))) {                 //Alt+C        
        if (copyflag != 1 && updateflag == 0)
            GetRows();
    }
    else if ((e.altKey && e.keyCode == 78) && (!$("#addacnttype").is(":visible")) && (!$("#iconForm,#salestranspopupdiv").is(":visible"))) {                  //Alt+N
        formrefreshconfirm();
    }
        ...
        if (copyflag != 1) {
          ...
          //  $('#iconForm').hide();
            ...
            if ($('#PrdtId0').val() != 0 && $("#itemsold").is(":visible")) {               
                if ($('#descpopup').text() != '')
                    $('#popupdesc').show();
                    $('#txtquantity0').focus();
            }
        ...
        $('#CancelOTPDiv').hide();
        $('#cancelotp').val('');
        $('#cancelotpremarks').val('');
    }
    //else if ((e.altKey && e.keyCode == 49) ) {                      //Pop Up to Show Sales Transaction Details of Selected Product by pressing Alt+1
    //    if (!(($("#HiddenItemId").val() == 0) && ($("#iconForm").is(":visible"))))
    //    {
    ...
    //}
    //else if ((e.altKey && e.keyCode == 51) ) {                      //Pop Up to Show Purchase Transaction Details of Selected Product by pressing Alt+3
        
    //}
    //else if ((e.altKey && e.keyCode == 52) ) {                      //Alt+4    :   All Transaction details Popup      
      
    //}
    //else if ((e.altKey && e.keyCode == 50) )               //alt+2   -   Multiple Products List 
    //{
        
    //}

    //else if (e.keyCode == 115 && $("#iconForm").is(":visible"))     //f4 - add Multiple Products List to grid
    //{
    //    addgridproducts();
    //}

    else if (e.keyCode == 188) {                                     //Comma - For Product Details Popup
        if (copyflag == 0 && $("#PrdtId0").val()!=0) { 
            GetPopup($("#PrdtId0").val());
        }        
    }
});

function GetPopup(ItemId)
{
    if (copyflag == 0) 
    {
    var data = {};
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            CustPrdctLoad(result.oList, result.oList[0].ProductCode);
        }
    });
    }
...
//Sales Transactions Functioncall
function SalesTransactions(ProductId,product, ProductDesc)
{
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var ProdRow = "<thead><tr><th>SalesInvoice</th><th>Date</th><th style='width:12%'>AccountName</th><th style='width:15%'>Address</th><th style='width:6%'>Qty.</th><th>Price</th><th>Location</th><th>SalesMan</th><th style='width:7%'>LPO</th><th>Department</th></tr>" +
                               "<tr><th>SalesInvoice</th><th>Date</th><th style='width:12%'>AccountName</th><th style='width:15%'>Address</th><th style='width:6%'>Qty.</th><th>Price</th><th>Location</th><th>SalesMan</th><th style='width:7%'>LPO</th><th>Department</th></tr></thead><tbody>";
...
//Purchase Transactions Functioncall
function PurchaseTransactions(ProductId, product, ProductDesc)
{
    if ($('#status_type').prop("checked"))
        var flag = 1;
    else
...
//All Transactions Functioncall
function AllTransactions(ProductId, product, ProductDesc) {
    $('#popupdesc').fadeOut();
...
function ShowMultipleProductsList(result, st) {
    ...
    if (result != 0) {
        var responseText1 = "<thead><tr><th style='width:90px;'> </th><th>Product</th><th>Description</th><th>Stock</th><th>Unit</th><th>Quantity</th><th>Rate</th><th>Discount</th><th>Tax</th><th>Location</th></tr></thead><tbody>";
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if ((MultiPdtArray.indexOf("" + result[l].ItemId + "") == -1)) {
                ...
                var lot = result[l].LOTNo;
              
                if (result[l].LOTNo == null)
                    lot = '';

                slno++;
                var responseText =
                    '<tr onclick="GetPrdtId(' + result[l].ItemId + ",'" + result[l].Description + "','" + result[l].ItemCode + "'," + slno + ')" id="mltplerow' + slno + '"><td style="width:90px;" ><input name="Checkprdtlist" onchange=clearcheck(' + slno + ') type="checkbox" disabled id= ' + 'SlNoHeadCheckgrid' + slno + '  class="form-control" style="zoom:.7;align:center"></td>' +
                    '<td style=display:none;><input type="text" style="display:none;" id= ' + 'ItemIdgrid' + slno + ' value= ' + result[l].ItemId + '></td>' +
                    '<td id="col' + slno + '">' + result[l].ItemCode + '<input type="text" style="display:none;" id= ' + 'Itemcodegrid' + slno + ' value= ' + result[l].ItemCode + '><input type="text" style="display:none;" id=' + 'LotNogrid' + slno + ' value= ' + lot + '></td>' +
                    '<td id=' + 'Itemdescgrid' + slno + '>' + result[l].Description + '</td>' +
                    '<td id=' + 'ItemStockqty' + slno + '>' + result[l].TotQty + '</td>' +
                    ...
                    '<td style="text-align:center;width:130px;" ><input type="text" style="height:30px;" class="form-control" id= ' + 'qtypr' + slno + ' onfocus=Getproddetails(' + slno + ') onkeypress=isNumberInt(event,this) onkeyup=gridamountcalculation(' + slno + '),checkqtystock(' + slno + '),Checkme(' + slno + ') onkeydown=Focusnext(event,' + slno + ',"q")></td>' +
                    '<td style="text-align:center;width:130px;" >' +
                    '<input type="text" style="height:30px;" class="form-control" id= ' + 'prrate' + slno + ' value=' + result[l].SellingPrice.toFixed(Decimal) + ' onblur=checkratestock(' + slno + ') onfocus=Getproddetails(' + slno + ') onkeypress=isNumber(event,this)  onkeyup=gridamountcalculation(' + slno + '),Checkme(' + slno + ') onkeydown=Focusnext(event,' + slno + ',"r")>' +
                    '<input type="text" style="height:30px;display:none" class="form-control" id= ' + 'prdfcrate' + slno + '></td>' +
                    '<td style="text-align:center;width:130px;">' +
                    '<input type="text" style="height:30px;" class="form-control" id= ' + 'prdisc' + slno + ' onkeypress=isNumber(event,this) onfocus=Getproddetails(' + slno + ')  onkeyup=gridamountcalculation(' + slno + '),checkdiscount(' + slno + ') onkeydown=Focusnext(event,' + slno + ',"d")>' +
                    ...
                    '<td style="text-align:center;" ><select  style="height:30px" class=form-control id=' + 'ItemLocngrid' + slno + ' onchange=Locationqtycheck(' + slno + ') onkeydown=Focusnext(event,' + slno + ',"l")>' + LocnSelect + '</select></td></tr></tbody>';
        ...
        var responseText = '<tr id="emptyrslt"><td colspan=9 style="text-align:center"><b>No Datas Found</b></td><tr></tbody>';
        $('#Productlist').append(responseText);
    }
...
function addgridproducts() {
    ...
    if (rateflg == 0) {
        console.log('rows')
        console.log($('#mltrcurrlength').val())
            ...
            for (var b = 0; b < Prodarray.length; b++) {
                ...
                if ($("#SlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#Itemunitgrid" + m).val() == '') {
                        ...
                        var lotno = $('#LotNogrid' + m).val();
                       
                        if ($('#LotNogrid' + m).val() == null) {
                            lotno = '';
                        }
                        ...
                        var ProdRows = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
                            ...
                            "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input  class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button'  type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                            "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + $('#ItemIdgrid' + m).val() + "')><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $('#ItemIdgrid' + m).val() + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $('#Itemcodegrid' + m).val() + "'></td>" +
                             "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + $('#ItemIdgrid' + m).val() + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $('#Itemdescgrid' + m).text() + "'></td>" +
                              "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + "  value='" + lotno + "' ></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat($('#prdfcdiscount' + m).val()).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat($('#prdisc' + m).val() || 0).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:38px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)'>" + TaxSelect + "</select></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtAdvance' + id + " value='0.00' onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:60px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_location' + id + " class='form-control'>" + LocnSelect + "</select></td>" +
                            "</tr>";
...
function salestranspopuprefresh() {
    ...
    else if ($('#PrdtId0').val() != 0 && $("#salestranspopupdiv").is(":visible")) {
        $('#txtquantity0').focus();
        $('#txtquantity0').select();
        if ($('#descpopup').text() != '')
            $('#popupdesc').show();
...
function savesales() {
    ...
    if ($("#txtBillSlNo").val() == '') {
        ...
        console.log($('#select_salesman').val())
...
function savesalesconfirm() {
    ...
    var excessamt = ($('#ExcessAmt').val() * $('#txtcrncyrate').val());
    ...
    for (var k = 1; k <= i - 1; k++) {
        ...
        var CashAdvance = $('#txtAdvance'+k).val();
        ...
        $.ajax(
    {
        ...
        type: "POST",
        url: "../SalesInvoice/UsedCarSalesInsertandUpdate", 
        data: data,
        success: function (result) {
    ...
    if (result.length > 0)
    {
        ...
        for (var n = 0; n < result.length; n++) {
            ...
            var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
               ...
               "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input  class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button'  type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
               "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
               "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "') ><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + Otherdescription + "'><input type='text' style='display:none;height:30px' id=" + 'MRP' + id + " value='0'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
               "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "') ><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
               "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
               "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
               "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + parseInt(result[n].ProdQty) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt(result[n].stocktotloseqty) + " style='display:none'></td>" +
               "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat(result[n].FcProdRate).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat(result[n].ProdRate).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)' onblur=checkrate(" + id + ")></td>" +
               "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (result[n].AvgCost * result[n].ProdQty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat(result[n].AvgCost).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
               ...
               "<td class='jsgrid-cell jsgrid-align-center' style='width:38px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)'>" + TaxSelect + "</select></td>" +
               "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:22px'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + (result[n].TaxPercent || 0) + " onkeyup='amountcalculation(" + id + ")'></td>" +
               ...
               "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtAdvance' + id + " value=" + parseFloat(result[n].PdtCashAdvance).toFixed(Decimal) + " onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
               "<td class='jsgrid-cell jsgrid-align-center' style='width:60px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_location' + id + " class='form-control' onchange ='Locationqtycheckmain(" + id + ")'>" + LocnSelect + "</select></td>" +
               "</tr>";
...
function qtyloadmain(result, id) {
    if (result.length == 'undefined')
        ...
        console.log('qty'+$('#txtstocktotloseqty' + id).val())
    }
}
...
$(document).ready(function () {
    ...
    $("#txtotpremarks").keypress(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            $("#btnotpsave").focus();
        }
    ...
    $("#searchGroup,#searchSubGroup,#searchCat,#searchSubcat,#searchItemCode").focus(function (e) { 
        Getproddetails(0);
    ...
    $("#txtproduct0").keydown(function (e) {
        ...
        if (key == 13 && qtyflag == 1 && (Negativebill == 'NO')) {
            qtyflag == 1;
    ...
    $("#txtAdvance0").keydown(function (e) { 
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            e.preventDefault();
            $('#btnadd').focus();
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                        ...
                        "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input type='text' id=" + 'enqno' + id + " style='display:none' value='" + enqno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "'></td>" +
                         "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + LotNoEn + "' ></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                        ...
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
                        ...
                        "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcamnt' + id + " value=" + fctotal + "  style='display:none'><input typ='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtamnt' + id + " value=" + total + "></td>" +
                          "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtAdvance' + id + " value='0.00' onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                         "<td class='jsgrid-cell jsgrid-align-center' style='width:60px'><select disabled='' id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control'>" + LocnSelect + "</select></td>" +
                        "</tr>";
                    $('#tblsalesinvoice').append(ProdRow1);
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckQtnItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                        ...
                        "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + qtnno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + LotNoQt + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + fcrate + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + rate + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSoItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                        ...
                        "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input type='text' id=" + 'slordsubid' + id + " style='display:none' value='" + ordersubid + "' /><input type='text' id=" + 'slordeno' + id + " style='display:none' value='" + orderno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + LotNoSO + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + fcrate + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + rate + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSIItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                        ...
                        "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input type='text' id=" + 'bilsers' + id + " style='display:none' value='" + srsno + "' /><input type='text' id=" + 'bilnum' + id + " style='display:none' value='" + srlno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + LotNoSI + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + fcrate + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + rate + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSoItem" + m).is(":checked")) {
                    ...
                    var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row'  >" +
                        ...
                        "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                        "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input type='text' id=" + 'delvsubid' + id + " style='display:none' value='" + ordersubid + "' /><input type='text' id=" + 'delvno' + id + " style='display:none' value='" + orderno + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + Productcode + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + ProductId + "')><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + "  value='" + LotNoDO + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + qty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + fcrate + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + rate + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (avgcst * qty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + avgcst + " style='background-color:white;height:30px'></td>" +
...
function CustomerEnquiryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                  ...
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' id=" + 'enqno' + id + " style='display:none' value='" + result[n].EnquiryNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat(result[n].FcProdRate).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat(result[n].ProdRate).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat(result[n].AvgCost * result[n].ProdQty).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat(result[n].AvgCost).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat(result[n].FcProdDisc).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat(result[n].ProdDisc).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxableamnt' + id + " value=" + parseFloat(result[n].FCTaxableAmount).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' style='background-color:white;height:30px' id=" + 'txttaxableamnt' + id + " value=" + parseFloat(result[n].TaxableAmount).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:38px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)'>" + TaxSelect + "</select></td>" +
...
function ShowItemGet(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'enqnoRow' + slno + '>' + result[l].EnquiryNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].EnquiryNo + '></td>' +
        ...
        '<td id=' + 'Des' + slno + '>' + result[l].ProductDescr + ' <input type="text" style="display:none;" id=' + 'LotNoEn' + slno + ' value= ' + result[l].LOTNo + '></td>' +
...
function ShowItemGetQtn(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'qtnnoRow' + slno + '>' + result[l].QuotationNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].QuotationNo + '></td>' +
        '<td style=display:none;><input type="text" id= ' + 'ItemId' + slno + ' value= ' + result[l].ProductId + '></td>' +
        '<td id=' + 'Productcode' + slno + '>' + result[l].ProductCode + '</td>' +
        '<td id=' + 'Des' + slno + '>' + result[l].ProductDescr + ' <input type="text" style="display:none;" id=' + 'LotNoQt' + slno + ' value= ' + result[l].LOTNo + '></td>' +
        '<td id=' + 'UnitName' + slno + '>' + result[l].UnitName + '<input type="text" style="display:none;" id=' + 'UnitIdgrid' + slno + ' value= ' + result[l].UnitId + '></td>' +
        '<td id=' + 'qty' + slno + '>' + parseInt(result[l].ProdQty) + '<input type="hidden" id="qty_' + slno + '" value=' + result[l].ProdQty + '></td>' +
        '<td id=' + 'rate' + slno + '>' + parseFloat(result[l].ProdRate).toFixed(Decimal) + '<input type="hidden" id="rte_' + slno + '" value=' + result[l].ProdRate + '><input type="hidden" id="fcrte_' + slno + '" value=' + result[l].FcProdRate + '><input type="hidden" id="avg_' + slno + '" value=' + result[l].AvgCost + '></td>' +
        '<td style=display:none; id=' + 'discount' + slno + '>' + parseFloat(result[l].ProdDisc).toFixed(Decimal) + '<input type="hidden" id="dis_' + slno + '" value=' + result[l].ProdDisc + '><input type="hidden" id="fcdis_' + slno + '" value=' + result[l].FcProdDisc + '></td>' +
...
function QtnEntryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                  ...
                  "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' id=" + 'qutnnum' + id + " style='display:none' value='" + result[n].QuotationNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function SalesOrderGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                  ...
                  "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' id=" + 'slordsubid' + id + " style='display:none' value='" + result[n].SalesOrderSubId + "' /><input type='text' id=" + 'slordeno' + id + " style='display:none' value='" + result[n].OrderNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcamnt' + id + " value=" + parseFloat(result[n].FCAmount).toFixed(Decimal) + "  style='display:none'><input typ='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtamnt' + id + " value=" + parseFloat(result[n].Amount).toFixed(Decimal) + "></td>" +
                    "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtAdvance' + id + " value='0.00' onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:60px'><select id=" + 'select_location' + id + " value=" + result[n].LocnId + " style='background-color:white;height:30px' disabled='' class='form-control'>" + LocnSelect + "</select></td>" +
                  "</tr>";
        $('#tblsalesinvoice').append(ProdRow1);
        $('#select_unit' + id).val(result[n].UnitId);
        $('#select_tax' + id).val(result[n].TaxId);
...
function ShowItemGetSalesOrder(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'qtnnoRow' + slno + '>' + result[l].OrderNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].OrderNo + '></td>' +
        ...
        '<td style=display:none;><input type="text" id= ' + 'ItemId' + slno + ' value= ' + result[l].ProductId + '></td>' +
        '<td id=' + 'Productcode' + slno + '>' + result[l].ProductCode + '</td>' +
        '<td id=' + 'Des' + slno + '>' + result[l].ProductDescr + '  <input type="text" style="display:none;" id=' + 'LotNoSO' + slno + ' value= ' + result[l].LOTNo + '></td>' +
        '<td id=' + 'UnitName' + slno + '>' + result[l].UnitName + '<input type="text" style="display:none;" id=' + 'UnitIdgrid' + slno + ' value= ' + result[l].UnitId + '></td>' +
        '<td id=' + 'qty' + slno + '>' + parseInt(result[l].ProdQty) + '<input type="hidden" id="qty_' + slno + '" value=' + result[l].ProdQty + '></td>' +
        '<td id=' + 'rate' + slno + '>' + parseFloat(result[l].ProdRate).toFixed(Decimal) + '<input type="hidden" id="rte_' + slno + '" value=' + result[l].ProdRate + '><input type="hidden" id="fcrte_' + slno + '" value=' + result[l].FcProdRate + '><input type="hidden" id="avg_' + slno + '" value=' + result[l].AvgCost + '></td>' +
        '<td style=display:none; id=' + 'discount' + slno + '>' + parseFloat(result[l].ProdDisc).toFixed(Decimal) + '<input type="hidden" id="dis_' + slno + '" value=' + result[l].ProdDisc + '><input type="hidden" id="fcdis_' + slno + '" value=' + result[l].FcProdDisc + '></td>' +
...
function SalesInvGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        $('#TotalAvgCost').val((parseFloat(result[n].TotalCost).toFixed(Decimal)));
        $('#PhoneNo').val(result[n].PhoneNumber);
        $('#ExcessAmt').val((parseFloat(result[n].ExcessAmt).toFixed(Decimal)));
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                  ...
                  "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfctaxamnt' + id + " value=" + parseFloat(result[n].FCTaxAmount).toFixed(Decimal) + "  style='display:none'><input typ='text' style='background-color:white;height:30px' class='form-control text-center'  id=" + 'txttaxamnt' + id + " value=" + parseFloat(result[n].TaxAmount).toFixed(Decimal) + " disabled=''></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcamnt' + id + " value=" + parseFloat(result[n].FCAmount).toFixed(Decimal) + "  style='display:none'><input typ='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtamnt' + id + " value=" + parseFloat(result[n].Amount).toFixed(Decimal) + "></td>" +
                    "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtAdvance' + id + " value=" + parseFloat(result[n].PdtCashAdvance || 0).toFixed(Decimal) + " onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:60px'><select id=" + 'select_location' + id + " value=" + result[n].LocnId + " style='background-color:white;height:30px' disabled='' class='form-control'>" + LocnSelect + "</select></td>" +
                  "</tr>";
        $('#tblsalesinvoice').append(ProdRow1);
        $('#select_unit' + id).val(result[n].UnitId);
...
function ShowItemGetSalesInvoice(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'billnoRow' + slno + '>' + result[l].BillSlNo + '<input type="hidden" id=' + 'billnumber' + slno + ' value=' + result[l].BillSlNo + '></td>' +
        '<td style=display:none;><input type="text" id= ' + 'ItemId' + slno + ' value= ' + result[l].ProductId + '></td>' +
        '<td id=' + 'Productcode' + slno + '>' + result[l].ProductCode + '</td>' +
        '<td id=' + 'Des' + slno + '>' + result[l].ProductDescr + ' <input type="text" style="display:none;" id=' + 'LotNoSI' + slno + ' value= ' + result[l].LOTNo + '></td>' +
        '<td id=' + 'UnitName' + slno + '>' + result[l].UnitName + '<input type="text" style="display:none;" id=' + 'UnitIdgrid' + slno + ' value= ' + result[l].UnitId + '></td>' +
        '<td id=' + 'qty' + slno + '>' + parseInt(result[l].ProdQty) + '<input type="hidden" id="qty_' + slno + '" value=' + result[l].ProdQty + '></td>' +
        '<td id=' + 'rate' + slno + '>' + parseFloat(result[l].ProdRate).toFixed(Decimal) + '<input type="hidden" id="rte_' + slno + '" value=' + result[l].ProdRate + '><input type="hidden" id="fcrte_' + slno + '" value=' + result[l].FcProdRate + '><input type="hidden" id="avg_' + slno + '" value=' + result[l].AvgCost + '></td>' +
        '<td style=display:none; id=' + 'discount' + slno + '>' + parseFloat(result[l].ProdDisc).toFixed(Decimal) + '<input type="hidden" id="dis_' + slno + '" value=' + result[l].ProdDisc + '><input type="hidden" id="fcdis_' + slno + '" value=' + result[l].FcProdDisc + '></td>' +
...
function DeliveryOrderGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row' >" +
                  ...
                  "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",0)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' id=" + 'delvsubid' + id + " style='display:none' value='" + result[n].DeliveryOrderSubId + "' /><input type='text' id=" + 'delvno' + id + " style='display:none' value='" + result[n].DeliveryOrderNo + "' /><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + result[n].ProductId + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + result[n].LOTNo + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
...
function ShowItemGetDeliveryOrder(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'delvordsubid' + slno + ' style=display:none;>' + result[l].DeliveryOrderSubId + '</td>' +
        '<td style=display:none;><input type="text" id= ' + 'ItemId' + slno + ' value= ' + result[l].ProductId + '></td>' +
        '<td id=' + 'Productcode' + slno + '>' + result[l].ProductCode + '</td>' +
        '<td id=' + 'Des' + slno + '>' + result[l].ProductDescr + ' <input type="text" style="display:none;" id=' + 'LotNoDO' + slno + ' value= ' + result[l].LOTNo + '></td>' +
        '<td id=' + 'UnitName' + slno + '>' + result[l].UnitName + '<input type="text" style="display:none;" id=' + 'UnitIdgrid' + slno + ' value= ' + result[l].UnitId + '></td>' +
        '<td id=' + 'qty' + slno + '>' + parseInt(result[l].ProdQty) + '<input type="hidden" id="qty_' + slno + '" value=' + result[l].ProdQty + '></td>' +
        '<td id=' + 'rate' + slno + '>' + parseFloat(result[l].ProdRate).toFixed(Decimal) + '<input type="hidden" id="rte_' + slno + '" value=' + result[l].ProdRate + '><input type="hidden" id="fcrte_' + slno + '" value=' + result[l].FcProdRate + '><input type="hidden" id="avg_' + slno + '" value=' + result[l].AvgCost + '></td>' +
        '<td style=display:none; id=' + 'discount' + slno + '>' + parseFloat(result[l].ProdDisc).toFixed(Decimal) + '<input type="hidden" id="dis_' + slno + '" value=' + result[l].ProdDisc + '><input type="hidden" id="fcdis_' + slno + '" value=' + result[l].FcProdDisc + '></td>' +
...
function formrefresh() {
    if (editflag != 0) {
      ...
      //  $('#select_location0').prop('disabled', 'true');
        ...
        $('#CashAdvance').prop("disabled", true);
        closewarningdesc();
    }
...
function TemporarySalesDelete() {
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            //console.log(result.oList[0].Status);
        }
    });
...
function CalcGrandTotal(Id) {
    ...
    for (var i = 1; i <= Id; i++) {
        ...
        Advance = Advance + parseFloat($('#txtAdvance' + i).val() || 0); 
    }
    ...
    BalanceAmt = GrandTotal - (Advance || 0);
    if (GrandTotal > 0)
        $('#BalanceAmt').text('Balance : ' + BalanceAmt.toFixed(Decimal));
    else
        $('#BalanceAmt').text('');
...
function Focusnextgrid(e, col, Id) {
    ...
    else if (key == 40 && Id != 0)          // Down Arrow
    {
        ...
        if (col == 'q') {
            ...
            $('#txtAdvance' + Rid).focus();
            $('#txtAdvance' + Rid).select();
        }
    }
...
function productadd() {
    ...
    var ad = parseFloat($('#txtAdvance0').val());
    $("#txtdiscount0").val(isNaN(a) ? 0 : a);
    $('#txtrate0').val(isNaN(c) ? 0 : c); 
    $('#txtAdvance0').val(isNaN(ad) ? 0 : ad);
    ...
    if ($.trim($("#txtproduct0").val()) == '') {
    ...
    else if (($("#PrdtId0").val() != 0) && (qtyflag == 1) && (Negativebill == 'NO')) {
        warningshow('Not enough stock!', 'txtproduct0');
    }
    ...
    else if ((($("#txtrate0").val() > 0) && (parseFloat($("#txtAdvance0").val()) > parseFloat($("#txtamnt0").val())))) { 
        warningshow('Advance should be less than ' + $("#txtamnt0").val(), 'txtAdvance0');
        $('#txtAdvance0').select();
    }
    ...
    else if ($("#txtquantity0").val() > (parseInt($('#txtstocktotloseqty0').val()) - pdtsumqty1) && (Negativebill == 'NO')) {
        ...
        var slno = rowcount + 1;
        var id = parseInt(i);
        var ProdRow1 = "<tr  onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
            "<td id= 'col13_" + id + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-edit-button jsgrid-align-center' style='display:none;'  id='btnroedit' type='button' onclick='EditRow(" + id + ")' title='Edit'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + id + ")' title='Delete'></td>" +
            "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input  class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ",1)' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button'  type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
            "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
            "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + $("#PrdtId0").val() + "')><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $("#PrdtId0").val() + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + $("#Otherdescription0").val() + "'><input type='text' style='display:none;height:30px' id=" + 'MRP' + id + " value='" + $("#MRP0").val() + "'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $("#txtproduct0").val() + "'></td>" +
            "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center' onclick=GetPopup('" + $("#PrdtId0").val() + "')><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $("#ProductDesc0").val() + "'></td>" +
            "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + " value='" + $("#LOTNo0").val() + "'></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)' onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ") onkeydown=Focusnextgrid(event,'u'," + id + ")>" + UnitSelect + "</select></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + parseInt($('#txtquantity0').val()) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)' onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ") onkeydown=Focusnextgrid(event,'q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat($('#txtfcrate0').val()).toFixed(Decimal) + "  style='display:none'><input type='text' class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat($('#txtrate0').val()).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)' onblur=checkrate(" + id + ") onfocusout=focusoutgrid('r'," + id + ") onfocusin=focusingrid('r'," + id + ") onkeydown=Focusnextgrid(event,'r'," + id + ")></td>" +
            "<td class='jsgrid-cell jsgrid-align-center ShowAverageCost' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + parseFloat($('#LineAvgCost0').val()).toFixed(Decimal) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat($('#AvgCost0').val()).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat($('#txtfcdiscount0').val()).toFixed(Decimal) + "  style='display:none'><input type='text'  id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat($('#txtdiscount0').val() || 0).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)' onfocusout=focusoutgrid('d'," + id + ") onfocusin=focusingrid('d'," + id + ") onkeydown=Focusnextgrid(event,'d'," + id + ")></td>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-center' style='width:38px'><select style='background-color:white;height:30px'  id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)' onfocusout=focusoutgrid('t'," + id + ") onfocusin=focusingrid('t'," + id + ") onkeydown=Focusnextgrid(event,'t'," + id + ")>" + TaxSelect + "</select></td>" +
            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:22px'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + ($('#txttaxpercent0').val() || 0) + " onkeyup='amountcalculation(" + id + ")'></td>" +
            ...
            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:35px'><input type='text' class='form-control text-center' style='background-color:white;height:30px'  id=" + 'txtAdvance' + id + " value=" + parseFloat($('#txtAdvance0').val()).toFixed(Decimal) + " onkeyup='checkadvance(" + id + ")' onkeypress='isNumber(event,this)' onfocusout=focusoutgrid('a'," + id + ") onfocusin=focusingrid('a'," + id + ") onkeydown=Focusnextgrid(event,'a'," + id + ")></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:60px'><select style='background-color:white;height:30px' id=" + 'select_location' + id + " class='form-control' onchange ='Locationqtycheckmain(" + id + ")' onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ") onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
            "</tr>";
        $('#tblsalesinvoice').append(ProdRow1);
...
//find total products and quantity
...
function totaladvance() { 
    ...
    $('#CashAdvance').val(''); 
    for (var p = 1; p <= i; p++) {
        totadvnc = totadvnc + parseFloat($('#txtAdvance' + p).val() || 0);
    }    
    $('#CashAdvance').val(totadvnc.toFixed(Decimal)); 
}
...
function CalcDiscountSplitTax1() {
    if ($('#tblsalesinvoice tr').length > 0) {
        if (parseFloat($('#disc').val() || 0) > 0) {
            ...
            console.log('');
            console.log('New Taxable :::' + NetAMT);
...
function JobAdd() {
    ...
    if ($.trim($('#productjob').val()) == "") {
        ...
        console.log($('#tblsalesinvoice tr').length)
        //if ($('#tblsalesinvoice tr').length == 0)
        //    i = 1;
        var id = parseInt(i);
        ...
        var prodjob = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-right' style='width:50px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'LotNo' + id + "></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
            "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + parseInt($('#quantity_job').val() || 0) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")'></td>" +
...
var eunit = ''; var eqnty = ''; var erate = ''; var edis = ''; var etaxable = ''; var etax = ''; var etaxperc = ''; var etaxamnt = ''; var eamnt = ''; var eadvanceamnt = ''; var elocn = '';
//Edit Button Click
function EditRow(id) {
    if (editflag == 0) {
        editflag = 1;
        ...
        eadvanceamnt = $('#txtAdvance' + id).val();
        ...
        $('#txtAdvance' + id).prop('disabled', false);
...
function editcancel(id) {
    ...
    $('#txtAdvance' + id).val(eadvanceamnt);
    ...
    $('#txtAdvance' + id).prop('disabled', true); 
    $('#select_location' + id).prop('disabled', true);

    $('#txtproduct0').focus();
    CalcGrandTotal(i);

    eunit = ''; eqnty = ''; erate = ''; edis = ''; etaxable = ''; etax = ''; etaxperc = ''; etaxamnt = ''; eamnt = ''; elocn = ''; eadvanceamnt = '';
    $('#Warningpopup').fadeOut();
...
function focusoutgrid(col, Id) {
    ...
    { $('#txtAdvance' + Id).css("background-color", 'white'); }
...
function focusingrid(col, Id) {
    ...
    { $('#txtAdvance' + Id).css("background-color", '#58dbe4'); }
}
...
function updaterow(id,flg) {
    ...
    if (($('#select_unit' + id).val() == 0)) {
    ...
    else if (($('#txtrate' + id).val() == '')||($('#txtrate' + id).val() == 0)) {
        warningshow('Please Enter the Rate', 'txtrate' + id);
        $('#txtrate' + id).select();
       editflag = 1;
        return false;
    }
    else if (parseFloat($('#txtdiscount' + id).val()) >= parseFloat($('#txtrate' + id).val())) { 
        warningshow('Discount Should be less than ' + $('#txtrate' + id).val(), 'txtdiscount' + id);
        $('#txtdiscount' + id).select();
        editflag = 1;
        return false;
    }
    else if ((($("#txtrate" + id).val() > 0) && (parseFloat($("#txtAdvance" + id).val()) > parseFloat($("#txtamnt" + id).val())))) {
        warningshow('Advance should be less than ' + $("#txtamnt" + id).val(), 'txtAdvance');
        $('#txtAdvance' + id).select();
        editflag = 1;
        return false;
    }
       ...
       // editflag--;
        ...
        var advum = parseFloat($("#txtAdvance" + id).val() || 0);
        $("#txtAdvance" + id).val(advum.toFixed(Decimal));
...
function ConfirmboxResult(Result, status, rowid) {
    ...
    if (Result == 'true' && status == 'delete') {
        ...
        $('#cancelotpremarks').prop('disabled', false);
        $('#cancelotp').val('');
        $('#cancelotpremarks').val(''); 
        $('#cancelotp').focus();
    }
...
function UsedCarEditInvoice() {
    if (parseInt($('#SalesDepartmentId').val()) == parseInt(ERPDeptId)) {
        $('#OTPDiv').show();
        $('#txtotp').prop('disabled', false);
        $('#txtotpremarks').prop('disabled', false);
        $('#txtotp').focus();
    }
    else {
        warningshow('Bill NO : ' + $('#txtBillseriesId').find("option:selected").text() + ' - ' + $('#txtBillSlNocopy').val() + ' is From Department - ' +   $('#SalesDepartment').val());
