...
function LoadDate(flg) {
    ...
    $('#DischargeDate,#AdmitDate').daterangepicker({
        startDate: CurDate,
        minDate: minDate,
        maxDate: new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate()),
        singleDatePicker: true,
...
function RevisitIDLoad() {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.oList.length > 0) {
                $("#BillNo").val(result.oList[0].IPBillNo); 
                //$("#BillNoCopy").text(result.oList[0].IPBillNo); 
            }
            else {
                $('.form-control').blur();
...
//Form Refresh
function formrefreshnew() {
    $('.rdonl').attr('readonly', true).css('background-color', 'white');
    $('#OpNo,#IpNo,#Name,#AdmitDate,#Age,#Remarks').val('');
    $('select').each((i, item) => {
        var $item = $(item);
...
function LoadRegNumber() {
    ...
    $("#RegNo").autocomplete({
                ...
                success: function (data) {
                    response($.map(data, function (item) {
                        return ({
                            ...
                            label2: item.Status.substring(2, 4)+'/'+ item.AadharNo,            //IP NUMBER
                            ...
                            DischargeDate: item.RegDate,       
                            AdmitTime: item.ToDate,
                            DischargeTime: item.RevisitDate,
                            IPYear: item.Status,
                            Regsername: item.OPSerName,
        ...
        select: function (event, ui) {
            ...
            $('#Opseriesname').val(ui.item.Regsername);
            $('#DischargeDate').val($('#BillDate').val());
            $('#AdmitDate').val(ui.item.AdmitDate + ' - ' + ui.item.AdmitTime);
            GetLastRevisitDetails(ui.item.PatientId);
            GetIPDetailsRoom(ui.item.IpNo, ui.item.IPYear, ui.item.PatientName);

            GetIPAdvanceDetailsGets(ui.item.IpNo, ui.item.IPYear, ui.item.PatientId);
            $('#TestName0').focus().select();
        },
...
function GetIPAdvanceDetailsGets(IPNo, IPYear,PID) {
    //var data = {};
    //data.IPNumber = IPNo;
    //data.IPYear = IPYear;
    //data.DeptId = ERPDeptId;
    ...
    //$.ajax({
    //    type: "POST",
    //    url: "../Revisit/HMS_IPAdvanceDetailsGets",
    ...
    $.ajax({
        type: "POST",
        url: "../Revisit/HMS_IPStatementautoFee",
        data: data,
        success: function (result) {
            LoadIPAdvancedetails(result.oList)
        }
    });
}

var ddhiabc = 0;
var Ipbilldetailsview = ""; var RW = 0
var Rno = 1; var PRno = 1; var LabTotal = 0; var Pharmatotal = 0;
function LoadIPAdvancedetails(result) {
    $('#c tr').remove();
  
    var Snnn0 = 1;


    $('#tblautobillList').html('')

    var ProdEditRow = "<tr class='jsgrid-row Billtr' style='border-top:2px solid black;font-weight: bold;'>" +
        "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow'  style= 'width:30px;text-align:center;background-color:#66ccff;' >Sl No</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;background-color:#66ccff;' >Date</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;background-color:#66ccff;' >Type</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='BillNo_" + id + "' style= 'width:15%;background-color:#66ccff;' >Bill No#</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='CashType_" + id + "'  style= 'width:50px;background-color:#66ccff;' align=center>PayType</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "'  style= 'width:90px;background-color:#66ccff;' align=right >Bill Amount</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;background-color:#66ccff;' align=right >Recev Amount</td>" +
        "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;background-color:#66ccff;' align=right >Balance</td>" +
        "</tr>";
    $('#tblautobillList').append(ProdEditRow)
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        var CashType = '';
        if (result[i].Paytype == 'CASH') CashType = '<span class="badge badge-success">&#160;CASH&#160;</span>';
        else if (result[i].Paytype == 'CREDIT') CashType = '<span class="badge badge-danger">CREDIT</span>';
        else {
            CashType = '<span class="badge badge-success">UPI/BANK</span>';
        }

        var BillNo = 0;
        var id = slno;

        if (result[i].Date == 'TOTAL') {
            var ProdEditRow = "<tr class='jsgrid-row Billtr' style='border:2px solid black;font-weight: bold;'>" +
                "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow' colspan=5  style= 'width:30px;text-align:right;background-color:#00b8e6;cursor:pointer' >TOTAL</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "' title='Total bill amount associated with this IP.'  style= 'width:90px;background-color:#00b8e6;cursor:pointer' align=right >" + result[i].billAmount + "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "' title='Total amount received with this IP, including advance' style= 'width:90px;background-color:#00b8e6;cursor:pointer' align=right >" + result[i].RecAmount + "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "' title='Non Paid amount.' style= 'width:90px;background-color:#00b8e6;cursor:pointer' align=right >" + result[i].balance + "</td>" +
                "</tr>";
            $('#tblautobillList').append(ProdEditRow)
            // var ProdEditRow = "<tr class='jsgrid-row Billtr' style='border:2px solid black;font-weight: bold;'><td height=10px colspan=8></td></tr>";
            // $('#tbl_IPGridauto').append(ProdEditRow)

            $('#printpaydetails').text('Total Receved Amount :' + result[i].RecAmount + '                 Total Bill Amount (PHARMA/OP/IP):  ' + result[i].billAmount)
            RecAmt = result[i].RecAmount;

        }

        else if (result[i].Date == 'BALANCE') {
            var ProdEditRow = "<tr class='jsgrid-row Billtr' style='border-bottom:2px solid black;border-right:2px solid black;border-left:2px solid black;font-weight: bold;'>" +
                "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow' colspan=5   style= 'width:30px;text-align:right;background-color: #ff99ff;' >BALANCE</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "'  style= 'width:90px;background-color: #ff99ff;' align=right ></td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;background-color: #ff99ff;' align=right ></td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "' title='Remaining balance amount.'  style='width:90px;background-color: #ff99ff;cursor:pointer;text-align:right' >" + result[i].balance + "</td>" +
                "</tr>";
            $('#tblautobillList').append(ProdEditRow);
           // $('#Advamount').val(result[i].balance)
           // $('#Advantotal').text('Advance :' + (result[i].balance))
        ...
        else if (result[i].Date == 'CLOSING BALANCE') {
            var ProdEditRow = "<tr class='jsgrid-row Billtr' style='border:2px solid black;font-weight: bold;'>" +
                "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow' colspan=5  style= 'width:30px;text-align:right;background-color:#bb99ff;' >CLOSING BALANCE</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "'  style= 'width:90px;background-color:#bb99ff;' align=right >" + result[i].billAmount + "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;background-color:#bb99ff;' align=right >" + result[i].RecAmount + "</td>" +
                "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;background-color:#bb99ff;' align=right >" + result[i].balance + "</td>" +
                "</tr>";
            $('#tblautobillList').append(ProdEditRow)
            Cbal = result[i].balance
        }

        else {

            if (result[i].BillType == 'AF') {
                proslno += 1;

                if (result[i].BillSeries == 1) { BType = 'PHARMACY CHARGES' } else if (result[i].BillSeries == 3) { BType = 'OP CHARGES' } else if (result[i].BillSeries == 3) { BType = 'OTHER CHARGES' }
                var ProdEditRow = "<tr class='jsgrid-row Billtr' id='Brow_" + id + "'>" +
                    "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow'  style= 'width:30px;text-align:center' >" + proslno + "</td>" +
                   /* "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;' >" + result[i].Date + "</td>" +*/
                    "<td class='jsgrid-cell p-1 GridBRow'   style= 'width:50px;' >  <input id='Ippdays" + id + "' type=hidden value=" + result[i].IPNumber + ">     <input id='IppId" + id + "' type=hidden value=" + result[i]. BillSeries+">   ROOM SERVICE</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='Ippname" + id + "'  colspan=3 style= 'width:15%;' >" + result[i].Paytype + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow'   id='Ippprate" + id + "'  style= 'width:90px;' align=right >" + result[i].billAmount + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow'   style= 'width:90px;' align=right >" + result[i].RecAmount + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow'   style= 'width:90px;' align=right >" + result[i].balance + "</td>" +
                    "</tr>";
                //debugger;
                if (ddhiabc == 0) {
                    ddhiabc = id;
                }
                $('#autofeeInfoPopUp').show();
            }
            else if (result[i].BillType == 'ADV') {
                var ProdEditRow = "<tr class='jsgrid-row Billtr' id='Brow_" + id + "'>" +
                    "<td id='td_" + id + "' class='jsgrid-cell p-1 GridBRow'  style= 'width:30px;text-align:center' >" + id + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;' >" + result[i].Date + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;' >" + result[i].BillType + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='BillNo_" + id + "' style= 'width:15%;' >" + result[i].BillNo + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='CashType_" + id + "'  style= 'width:50px;' align=center></td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "'  style= 'width:90px;' align=right ></td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;' align=right ><b>" + result[i].RecAmount + "</b></td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;' align=right ></td>" +
                    "</tr>";
            }
            else {
                var ProdEditRow = "<tr class='jsgrid-row Billtr' id='Brow_" + id + "'>" +
                    ...
                    "<td class='jsgrid-cell p-1 GridBRow' id='BillDate_" + id + "'  style= 'width:50px;' >" + result[i].BillType + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='BillNo_" + id + "' style= 'width:15%;' >" + result[i].BillNo + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='CashType_" + id + "'  style= 'width:50px;' align=center>" + CashType + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountCTr_" + id + "'  style= 'width:90px;' align=right >" + result[i].billAmount + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;' align=right >" + result[i].RecAmount + "</td>" +
                    "<td class='jsgrid-cell p-1 GridBRow' id='AmountTr_" + id + "'  style= 'width:90px;' align=right >" + result[i].balance + "</td>" +
                    "</tr>";
            }
            $('#tblautobillList').append(ProdEditRow)
        }
    ...
    if (Cbal < 0) {
        swal('Closing Balnce:' + Cbal + '*', "inform Customer to pay Bill/Advance", "error");
        $('.swal-button').hide();
        ...
        if (RecAmt == 0) {
            ...
            $('#Advamount').val(RecAmt)
            $('#Advantotal').text('Advance :' + RecAmt)
        }
...
function LoadIPAdvancedetails1(result) {
    var AdvAmount = 0; var prcototal = 0
    var Slno1 = 0; var Slno2 = 0;
    disable_datatable('tblAdList');
    disable_datatable('tblautobillList');
    $('#tblAdList tr').remove();
    $('#autofeeInfoPopUp').hide()
    var ProdRow = "<thead><tr>" +
        "<th style='width:2%' class='text-center'>Sl#</th><th style='width:5%' class='text-center'>Date</th>" +
    "<th style='width:17%' class='text-center'>Type</th>" +    
        "<th style='width:5%' class='text-right'>Amount</th></tr></tr></thead><tbody>";

    var procRow = "<thead><tr>" +
        "<th style='width:2%' class='text-center'>Sl#</th><th style='width:80%' class='text-center'>Service</th>" +
        "<th style='width:10%' class='text-center'>Rate</th><th style='width:2%' class='text-center'>Dys</th>" +
        "<th style='width:5%' class='text-center'>Hrs</th><th style='width:10%' class='text-right'>Amount</th>" +
        "</tr></tr></thead><tbody>";

    if (result.length != 0) {
        $('#Loadingsave').show();
        for (var n = 0; n < result.length; n++) {
            if (result[n].BillType == 'ADV' || result[n].BillType == 'PB' || result[n].BillType == 'LB' || result[n].BillType == 'SI') {
                Slno1 = Slno1+1
                AdvAmount = AdvAmount + (result[n].RecAmount)
                ProdRow += "<tr class='jsgrid-row'>" +
                    "<td class='text-center'> " + Slno1 + "</td>" +
                    "<td class='text-center'> " + result[n].Date + "</td>" +
                    "<td class='text-left'> ADVANCE</td>" +
                    //"<td class='text-center'> " + result[n].Patient + "</td>" +
                    "<td class='text-right'> " + parseFloat(result[n].RecAmount).toFixed(Decimal) + "</td>" +
                    "</tr>";
            }
            else {
                Slno2 = Slno2+1
                prcototal = prcototal+ parseFloat(result[n].ProcAmount)
                procRow += "<tr class='jsgrid-row'>" +
                    "<td class='text-center'> <input type=hidden value=" + result[n].ProcId + " id=" + 'IppId' + Slno2 + ">" + Slno2 + "</td>" +
                   
                    "<td id=" + 'Ippname' + Slno2 + " class='text-left'> " + result[n].procName + "</td>" +
                    "<td id=" + 'Ippprate' + Slno2 + " class='text-right'> " + result[n].procRate + "</td>" +
                    "<td id=" + 'Ippdays' + Slno2 + " class='text-center'> " + result[n].ProcDays + "</td>" +
                    "<td class='text-center'> " + result[n].Prochours + "</td>" +
                    "<td id=" + 'Ippamount' + Slno2 + " class='text-right'> " + parseFloat(result[n].ProcAmount).toFixed(Decimal) + "</td>" +
                    "</tr>";
                $('#autofeeInfoPopUp').show()

            }
        }
        ProdRow += "<tr class='jsgrid-row'><td class='text-center'> </td><td></td>" +
         "<td class='text-center'></td><td class='text-right'><b> " + parseFloat(AdvAmount).toFixed(Decimal) + "</b></td></tr>";


        procRow += "<tr class='jsgrid-row'><td class='text-center'> </td><td class='text-center'> </td><td class='text-center'> </td><td class='text-left'></td>" +
            "<td class='text-center'></td><td class='text-right'><b> " + parseFloat(prcototal).toFixed(Decimal) + "</b></td></tr>";


        $('#tblAdList').html(ProdRow + '</tbody>');
        $('#tblautobillList').html(procRow + '</tbody>');
        datatableWithsearch('tblAdList', 'Multiple');
        datatableWithsearch('tblautobillList', 'Multiple');

    }

    $('#Advamount').val((AdvAmount).toFixed(Decimal))
    $('#Advantotal').text('Advance :' + (AdvAmount).toFixed(2))
}
...
function LoadIPStatementRoom(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        if (result[i].Remarks != '') 
        $('#Remarks').val($('#Remarks').val()+' , '+ result[i].Remarks + ' - ' + parseFloat(result[i].Rate || 0).toFixed(Decimal)+' - ' + result[i].Days + ' Days, ' + result[i].RegSerId + 'Hours')
...
function GetPatientData(result) {
    ...
    Gettestadviceget(result[0].OPNumber, result[0].OPSerId, result[0].RevisitId);
}
...
function AddTestConfirm() {
    ...
    var ProdRow1 = "<tr id=" + 'Row' + id + " class='jsgrid-row TsRw' onclick='EditRow(" + id + ")' onfocusout=updaterow(" + id + ")>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-left' style='width:20%'><input type='text' id='TestName" + id + "' value='" + $('#TestCode0').val()  + "'  class='form-control TestName' style='height:30px' ></td>" +
        "<td class='jsgrid-cell jsgrid-align-left' style='width:40%'><input type='text' readonly id='TestCode" + id + "' value='" + $('#TestName0').val() + "' class='form-control' style='height:30px;background-color:white' ></td>" +
        "<td class='jsgrid-cell jsgrid-align-center' style='width:5%'><input id='PQty" + id + "' class='form-control text-center PQty cmdts' value='" + $('#PQty0').val() + "' style='height:30px' onkeydown=FocusManual('PQty','TestName'," + id + ",event) onkeyup=AmountCalc(" + id + ") onkeypress=isNumber(event,this)></td>" +
...
function CalcGrandTotal(flg) { // flg-0 : Disc%    ,flg-2 : DiscAmt    ,flg-1 : Other
    ...
    var Ad = parseFloat($('#Advamount').val() || 0)
    TotalAmountlessad = parseFloat((NetAmt - Ad) || 0)
    $('#Re_total').text(TotalAmount.toFixed(Decimal))

    $('#TotalAmt').text(TotalAmountlessad.toFixed(Decimal));
    $('#NetAmt,#GrandTotal').text(TotalAmountlessad.toFixed(Decimal));
...
function FocusManual(Id, Dest, key, e) {
    ...
    if ((Id == 'TestAmount') || (Id == 'PQty') || (Id == 'PRate'))
    { SLClass = Id; }
...
function CallViewList() {
    ...
    var data = {};
    data.BillNo = $("#PatientId").val();
    data.DeptId = ERPDeptId;
    data.BillDate = $("#DateFrom").val();
    data.Hospital = $("#DateTo").val();
    data.UserId = ERPUserId;
    data.Status = 'IPB';
    $.ajax({
        type: "POST",
...
function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    ...
    if (flg != 3)
    { $('.TsRw').remove(); $('#btnedit,#btndelete,#btnacctran').hide(); }
    CheckEOD();
}

function LoadBillSearch() {
    $("#BillNoCopy").autocomplete({
        delay: 0,
        minLength: 0,
        source: function (request, response) {
            ClearData(0, 0);
            ...
            data.DeptId = ERPDeptId;
            data.Status = 'IPB';
            if (parseInt(data.BillNo) < 0 || parseInt(data.BillNo) >= currentbillno) {
                warningshow('Bill Number Not Valid', 'BillNoCopy');
            }
            else {
                $.ajax({
                    url: '../Revisit/HMS_BillNoSearch',
                    type: "POST",
                    data: data,
                    dataType: "json",
                    success: function (data) {
                        response($.map(data, function (item) {
                            return ({
                                ColCount: '3',
                                label: item.BillYear + '-' + item.BillNo,
                                label1: item.Name,
                                label2: item.BillDate,
                                BillNo: item.BillNo,
                                DeptId: item.DeptId,
                                headers: ["BillNo", "Patient Name", "BillDate"]
                            })
                        }));
                    }
                })
        ...
        autoFocus: true,
        select: function (event, ui) {
            GetBillDetails(ui.item.Billyear,ui.item.BillNo, ui.item.DeptId);
        },
    });
}
var BILLYR = "";
function GetBillDetails(BilllYear,BillNo, DeptId) {
    ...
    data.BillNo = BillNo;
    data.DeptId = DeptId;  
    data.Status = 'IPB##' + BilllYear;
    $.ajax({
        type: "POST",
        url: "../Revisit/HMS_BillNumberGetandGets",
        data: data,
        success: function (result) {
...
function BillNumberGets(result) {
    ...
    if (result.length > 0) {
        ...
        //    $('#Cash').prop("checked", true);
        //}
        var userdet = (result[0].Others).split("##")
        BUserNm = userdet[0];
        BBilltime = userdet[1];
        $('#printiuserdet').text('USER-' + BUserNm + ' -' + BBilltime)
        ...
        $('#DischargeDate').val(result[0].DischargeDate + ' - ' + result[0].DischargeTime);
        $('#AdmitDate').val(result[0].AdmitDate + ' - ' + result[0].AdmitTime);
        ...
        var SSdate = (result[0].SurgeryDate).split("#")

        $('#SurgeryDate').val(SSdate[0]);
        $('#Advamount').val(SSdate[4]);
        if (SSdate[4] > 0) {
            $('#Advantotal').text('Advance: ' + SSdate[4]);
        }

        $('#Remarks').val(result[0].Remarks);


        $('#paycash').val(result[0].Cashrec);
        $('#payupi').val(result[0].Upirec);
        $('#paycard').val(result[0].CardRec);
        $('#Re_total').text(SSdate[5])
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            var ProdRow1 = "<tr id=" + 'Row' + id + " class='jsgrid-row TsRw' onclick='EditRow(" + id + ")' onfocusout=updaterow(" + id + ")>" +
                  ...
                  "<td class='jsgrid-cell p-1  jsgrid-align-left' style='width:20%'><input type='text' id='TestName" + id + "' value='" + result[i].TestName + "'  class='form-control TestName' style='height:30px' ></td>" +
                    "<td class='jsgrid-cell jsgrid-align-left' style='width:40%'><input type='text' readonly id='TestCode" + id + "' value='" + result[i].Status + "' class='form-control rdonl' style='height:30px;background-color:white' ></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:5%'><input id='PQty" + id + "' class='form-control text-center PQty cmdts' value='" + result[i].PQty + "' style='height:30px' onkeydown=FocusManual('PQty','TestName'," + id + ",event) onkeyup=AmountCalc(" + id + ") onkeypress=isNumber(event,this)></td>" +
            ...
            $('.jsgrid-cell> input,.jsgrid-cell >select').prop('disabled', true).css('border', 'none').css('background-color', 'white');
...
function SaveAndUpdate(flg) {
    var AA = 0;
     ...
     if (parseFloat($('#GrandTotal').text() || 0) <= 0) {
    ...
    else
    {
        ...
        if (AA == 0) {
            ...
            var Msg = 'Do You Want To ' + $('#Confirmflag').val() + 'this bill without the amount or the refund amount ?';

        }



        //Save this bill without the amount or the refund amount?
...
function SaveAndUpdateConfirm(flg) {
    ...
    for (var k = 1; k <= Count; k++) {
        ...
        var BillYear = $('#IpYear').val();                  //IPYEAR
        ...
        var Department = $('#Department' + k).val();
        var PQty = $('#PQty' + k).val();
        ...
        var a = parseFloat($('#paycash').val() || 0)
        var b = parseFloat($('#payupi').val() || 0)
        var c = parseFloat($('#paycard').val() || 0)
        ...
        if (TestId != undefined) {
            oArray.push({
                ...
                'DeptId': DeptId,
                'DelFlag': DelFlag,
                'Type': Type,       // LAB  - 0 , PROCEDURE - 1 , IPBILL - 2
                'SurgeryDate': SurgeryDate + '#' + a + '#' + b + '#' + c + '#' + AdAmount + '#' + Billamount,
                'Remarks': Remarks
            })
        }
        //console.log(oArray)
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            {
                type: "POST",
                url: "../Revisit/HMS_BillInsert",
                data: data,
                success: function (result) {
                    var status = result.oList[0].Status;
                    var BillNo = result.oList[0].BillNo;
                    var BillYear = result.oList[0].BillYear;
                    BUserNm = $('#UserDetsss').text();
                    BBilltime = (new Date().toLocaleString()).substring(10, 20);
...
var LabBillType = IPBillPrintType;        //HALFSIZE    , FULLSIZE  ,DOTMATRIXLAB
function PrintthisBill(flg) {
    PrintthisBillWindowsHMS('LABBILL', Count, 'HALFSIZEAYU', 'IB');
    //if (flg == 1) {

    //    if (LabBillType == 'HALFSIZEAYU') { PrintthisBillWindowsHMS('LABBILL', Count, 'HALFSIZEAYU', 'IB'); }
    //    else if (LabBillType == 'HALFSIZE')
    //    { PrintthisBillWindowsHMS('LABBILL', Count, 'COPYHALF', 'IB'); }
...
function Showalerts(Status, BillNo, BillYear) {
    if (Status == 1) {
        formrefresh();
        swal('Bill No ' + BillNo, "Saved Successfully", "success");
