...
function LoadDaterev() {
    $('#NextDate').daterangepicker({
        maxDate: new Date(new Date().getFullYear() + 1, new Date().getMonth(), new Date().getDate()),
        singleDatePicker: true,
        showDropdowns: true,
        locale: { format: 'DD/MM/YYYY' },
...
function ShowInsuranceCompanylist(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        var R3 = (result[i].InsuranceCompanyDescription).split("###");


        responseText += '<tr  style="pointer:cursor;height:40px"><td style="width: 8%;" align=center>' + slno + '</td><td  style="width: 10%;">' + R1[5] + '</td><td  style="width:38%;">  <a  target="_blank" href="' + R3[1] + '">' + result[i].InsuranceCompanyCode + '</a></td><td style="width: 30%;">' + R1[0] + '</td></tr>';
...
function ShowCaseSheetlist(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        responseText += '<tr>' +
            ...
            '<td align=center><a onclick=GetRows(' + result[i].CaseSheetNo + ',"CASE2",0)>' + Editbutton + '</a></td>' +
            '</tr>';
    }
...
function ShowCaseSheetGet(result, Flag) {
    ...
    $('#NextDate').val(result[0].Reviewdate);
...
function LoadRegNumber() {
    ...
    $("#PRegNo").autocomplete({
        ...
        select: function (event, ui) {
            ...
            GetLastRevisitDetailsnew(ui.item.PatientId, ui.item.OPNumber);
...
function GetLastRevisitDetailsnew(PatientId, RevisitId) {
...
function OKprocAddIMRow(flg)      //flg-0 : Add Medicine
{


    if (flg == 0) {

        var Id = parseInt($("#pIMMGridLength").val()) + 1;
        var Slno = $('#TblImmunizationproc tr').length + 1;
        ...
        var MedRow =
            ...
            '<td class="jsgrid-align-center" style="width: 8%;" id="pIMSlno_' + Id + '">' + Slno + '</td>' +
            '<td class="" style="width: 38%;">' +
            '<input type="text" class="form-control brdles"  id="pVaccine_' + Id + '" onfocus="LoadVaccineproc(\'pVaccine_' + Id + '\', \'pVaccineId_' + Id + '\', \'pVaccStatus_' + Id + '\')" />' +
            '<input type="hidden" value="0" id="pVaccineId_' + Id + '" />' +
            '</td>' +
            '<td class="" style="width: 18%;display:none">' +
            '<select type="text" style="dispalay:none" class="form-control brdles" id="pVaccCompany_' + Id + '"  onkeydown="FocusNext(event, \'\', \'pVaccCompany_\', \'pVaccDate_\', \'' + Id + '\', \'pIMTr_\',1)">' + GroupSelect + '</select>' +
            '</td>' +
            '<td class="" style="width: 12%;display:none">' +
            '<input style="dispalay:none" type="text" class="form-control brdles dedate" value="PB" id="PVaccDate_' + Id + '"  onkeydown="FocusNext(event, \'pVaccCompany_\', \'pVaccDate_\', \'pVacNextDate_\', \'' + Id + '\', \'pIMTr_\',1)" />' +
            '</td>' +
            '<td class="" style="width: 12%;display:none">' +
            '<input style="dispalay:none" type="text" class="form-control brdles dedate" id="VacNextDate_' + Id + '"  onkeydown="FocusNext(event, \'pVaccDate_\', \'pVacNextDate_\', \'pVaccStatus_\', \'' + Id + '\', \'pIMTr_\',1)" />' +
            '</td>' +
            ...
            //'</select>' +
            '<button type="button" id="pVaccStatus_' + Id + '" onclick="AddIMRowproc(event,' + Id + ')"  class="btn btn-outline-success btn-min-width mr-0 mb-0 btn-sm p-1 font-small-3" autocomplete="off"> &#160;&#160;&#160;&#160; ADD &#160;&#160;&#160;&#160; </button>' +

            '</td>' +
            '</tr>';
        $('#TblImmunizationproc').append(MedRow);
...
function OKAddIMRow(flg)      //flg-0 : Add Medicine
{
    ...
    if (flg == 0) {
        ...
        var MedRow =
            ...
            '</td>' +
            '<td class="" style="width: 12%;display:none">' +
            '<input style="dispalay:none" type="text" class="form-control brdles dedate" value=LB" id="VaccDate_' + Id + '"  onkeydown="FocusNext(event, \'VaccCompany_\', \'VaccDate_\', \'VacNextDate_\', \'' + Id + '\', \'IMTr_\',1)" />' +
            '</td>' +
            '<td class="" style="width: 12%;display:none">' +
...
function DeleteImmuneproc(Id) {
    $('#confirm').show();
    $('#confirmOk').focus();
    $('#Confirmflag').val('ImmuneDeletep'); $('#ConfirmRowId').val(Id);
    $('#confirmmessage').text('Do you want to delete this Entry?');
}

function OKDeleteImmuneproc(Id) {
    $("#pIMTr_" + Id).remove();
    var Count = $("#pIMMGridLength").val();
    var Slno = 1;
...
function LoadImmunizationDetails(result) {
    if (result.length > 0) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            console.log(result[i].VaccineDosage)

            if (result[i].VaccineDosage == 'LB') {
                if (i == 0) {
                ...
                if (b == 0) {
                    ...
                    var MedRow =
                        '<tr class="jsgrid-header-row IMRow" id="pIMTr_' + Id + '">' +
                        '<td class="jsgrid-control-field jsgrid-align-center" id="editheader" style="width: 5%;" onclick="DeleteImmuneproc(' + Id + ')" ><i class="icon-trash"></i></td>' +
                        '<td class="jsgrid-align-center" style="width: 8%;" id="pIMSlno_' + Id + '">' + Slno + '</td>' +
                        '<td class="" style="width: 38%;">' +
                        '<input type="text" class="form-control brdles"  id="pVaccine_' + Id + '" onfocus="LoadVaccine(\'Vaccine_' + Id + '\', \'VaccineId_' + Id + '\', \'pVaccStatus_' + Id + '\')" value="' + result[i].VaccineName + '" />' +
                        '<input type="hidden" value="' + result[i].VaccineId + '" id="pVaccineId_' + Id + '" />' +
                        '</td>' +
                        '<td class="" style="width: 18%;display:none">' +
                        '<select type="text" class="form-control brdles" id="pVaccCompany_' + Id + '"  onkeydown="FocusNext(event, \'\', \'VaccCompany_\', \'pVaccDate_\', \'' + Id + '\', \'pIMTr_\',1)">' + GroupSelect + '</select>' +
                        '</td>' +
                        '<td class="" style="width: 12%;display:none">' +
                        '<input type="text" class="form-control brdles dedate" id="pVaccDate_' + Id + '"  onkeydown="FocusNext(event, \'VaccCompany_\', \'pVaccDate_\', \'pVacNextDate_\', \'' + Id + '\', \'pIMTr_\',1)" />' +
                        '</td>' +
                        '<td class="" style="width: 12%;display:none">' +
                        '<input type="text" class="form-control brdles dedate" id="pVacNextDate_' + Id + '"  onkeydown="FocusNext(event, \'VaccDate_\', \'pVacNextDate_\', \'pVaccStatus_\', \'' + Id + '\', \'pIMTr_\',1)" />' +
                        '</td>' +
                        '<td class="" style="width: 7%;dispaly:none1">' +
                        '<button type="button" id="pVaccStatus_' + Id + '" onclick="AddIMRowproc(event,' + Id + ')" class="btn btn-outline-success btn-min-width mr-0 mb-0 btn-sm p-1 font-small-3" autocomplete="off">&#160;&#160;&#160;&#160; ADD &#160;&#160;&#160;&#160; </button>' +
...
function Noftabs(Id) {
    ...
    const oneQtyDosages = [
        "1ml bd", "1ml tds", "2ml bd", "2ml tds", "up to 10 ml",
        "7.5 ml bd", "7.5 ml tds", "local application bd", "local application tds"
    ];
    ...
    let dosageValue = $.trim($('#Dosage' + Id).val()).toLowerCase();
    if (oneQtyDosages.includes(dosageValue)) {
        NOTABS = 1;
        $('#Daily' + Id).val(NOTABS)
    }
    else {
    ...
    for (var d = 1; d <= Id; d++) {
        ...
        var Selaret = parseFloat($('#medselrate' + d).val() || 0).toFixed(2)

        Apxtotal = parseFloat(Apxtotal + parseFloat(Qty * Selaret))
    }
...
function AddMedicineRow(flg)      //flg-0 : Add Medicine
{
    if (flg == 0) {
        ...
        var MedRow =
            ...
            '<input type="text" style="display:none" class="form-control" id="MedicineId' + Id + '" /> <input type="text" style="display:none" class="form-control"  id="medselrate' + Id + '"  value="0" />' +
            '</td>' +
            ...
            '<td class="" style="width: 10%;"><input type="text" class="form-control mdclass brdles"  onfocus="mednoteload1(' + Id + ')"  onkeypress="return isNumberInt(event, this)" onfocusout=UpdateMedicineRow(' + Id + ',1) name="Daily" id="Daily' + Id + '" /> <span class="spanrate" id="spanrate' + Id + '"  style="margin-top: -12px; right: 5%; position: fixed; font-family: tahoma; font-size: 15px"></span>     </td>' +
            '</tr>';
...
function ConfirmboxResult(Result, Status, RowId) {
    ...
    else if (Result == 'true' && Status == 'ImmuneDeletep') {
        OKDeleteImmuneproc(RowId);
    }
