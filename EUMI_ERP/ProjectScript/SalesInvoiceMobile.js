...
var BaseCurrency; var LocnSelect = ''; var UnitSelect = ''; var TaxSelect = '';
...
$(document).keydown(function (e) {
    ...
    if ((x > 111 && x < 124)) {
        ...
        if (e.keyCode == 113 && $("#IMEIDiv").is(":visible")) {          //F2 - multiple IMEI add
            IMEIProductAdd();
        }
        else if ((x == 118) && (!$("#addacnttype").is(":visible")))            //Pop Up to Show Sales Transaction Details of Selected Product by pressing F7
        {
            ...
            if ($("#TransPrdtId0").val() != 0) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $('#salesheader').text(ProductDesc + '(' + product + ')' + ' -Sales Transaction Details');
                        $('#salestransdiv').show();
                        $('#txtquantity0').focus();
    ...
    else if ((e.altKey && e.keyCode == 78) && (!$("#addacnttype").is(":visible"))) {                  //Alt+N
        formrefreshconfirm();
    }
    ...
    else if ((e.altKey && e.keyCode == 50) && (!$("#addacnttype").is(":visible")))               //alt+2   -   Multiple Products List 
    {
...
function Multipleproduct() {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (result) {
            ShowMultipleProductsList(result);
...
function ShowMultipleProductsList(result) {
    $("#Productlist tr").remove();
    $('#Productlist thead').remove();
    $('#iconForm').show();

    if (result != 0) {
        var responseText1 = "<thead><tr><th style='width:90px;'> <input type='checkbox'  disabled  id= 'SlNoHeadCheckgrid0'  'custom-control-input cz-bg-image-display'>&nbsp;&nbsp;&nbsp;</th><th>Product</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th>Discount</th><th>Tax</th><th>Location</th></tr></thead><tbody>";
        $('#Productlist').append(responseText1);
        for (var l = 0; l < result.length; l++) {
            var slno = parseInt(l + 1);

            var responseText =
                '<tr><td style="width:90px;" ><input name="Checkprdtlist" onchange=clearcheck(' + slno + ') type="checkbox" disabled id= ' + 'SlNoHeadCheckgrid' + slno + '  "custom-control-input cz-bg-image-display" style="align:center"></td>' +
                '<td style=display:none;><input type="text" style="display:none;" id= ' + 'ItemIdgrid' + slno + ' value= ' + result[l].ItemId + '></td>' +
                '<td>' + result[l].ItemCode + '<input type="text" style="display:none;" id= ' + 'Itemcodegrid' + slno + ' value= ' + result[l].ItemCode + '></td>' +
                '<td id=' + 'Itemdescgrid' + slno + '>' + result[l].Description + '</td>' +
                '<td style="text-align:center;" ><select  style="height:30px;" class=form-control id=' + 'Itemunitgrid' + slno + ' onkeypress=Focusnext(event,' + slno + ',"u")>' + UnitSelect + '</select></td>' +
                '<td style="text-align:center;width:130px;" ><input type="text" style="height:30px;" class="form-control" id= ' + 'qtypr' + slno + ' onfocus=Getproddetails(' + slno + ') onkeypress=isNumberInt(event,this) onkeyup=gridamountcalculation(' + slno + '),checkqtystock(' + slno + '),Focusnext(event,' + slno + ',"q"),Checkme(' + slno + ')></td>' +
                '<td style="text-align:center;width:130px;" >' +
                '<input type="text" style="height:30px;" class="form-control" id= ' + 'prrate' + slno + ' value=' + result[l].SellingPrice.toFixed(Decimal) + ' onblur=checkratestock(' + slno + ') onfocus=Getproddetails(' + slno + ') onkeypress=isNumber(event,this)  onkeyup=gridamountcalculation(' + slno + '),Focusnext(event,' + slno + ',"r"),Checkme(' + slno + ')>' +
                '<input type="text" style="height:30px;display:none" class="form-control" id= ' + 'prdfcrate' + slno + '></td>' +
                '<td style="text-align:center;width:130px;">' +
                '<input type="text" style="height:30px;" class="form-control" id= ' + 'prdisc' + slno + ' onkeypress=isNumber(event,this) onfocus=Getproddetails(' + slno + ')  onkeyup=gridamountcalculation(' + slno + '),checkdiscount(' + slno + '),Focusnext(event,' + slno + ',"d")>' +
                ...
                '<td style="text-align:center;" ><select  style="height:30px;" class=form-control id=' + 'Itemtaxgrid' + slno + ' onkeypress=Focusnext(event,' + slno + ',"t") onchange=gridamountcalculation(' + slno + ')>' + TaxSelect + '</select></td>' +
                '<td style="text-align:center;" ><select  style="height:30px;" class=form-control id=' + 'ItemLocngrid' + slno + ' onchange=Locationqtycheck(' + slno + ') onkeypress=Focusnext(event,' + slno + ',"l")>' + LocnSelect + '</select></td></tr>';
        ...
        var responseText = '<tr><td style="text-align:center"><b>No Datas Found</b></td><tr>';
        $('#Productlist').html(responseText);
    }
                ...
                if ($("#SlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#Itemunitgrid" + m).val() == '') {
                        ...
                        var ProdRows = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $('#ItemIdgrid' + m).val() + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $('#Itemcodegrid' + m).val() + "'></td>" +
                             "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + $('#Itemdescgrid' + m).text() + "'></td>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + $('#Itemtaxgrid' + m).find("option:selected").attr("name") + " onkeyup='amountcalculation(" + id + ")'></td>" +
...
function LoadProddetails(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        if (result[n].custstats == 'Last Selling Price' && $('#txtCustId').val() == 0) {
            var ProdRow =
             "Average Cost : " + (parseFloat(result[n].AvgCost || 0).toFixed(Decimal)) + " , Last Purchase Cost : " + (parseFloat(result[n].LPCost || 0).toFixed(Decimal)) + " , " + custstat + " :" + (parseFloat(result[n].LastSellingPrice || 0).toFixed(Decimal)) + " , Total Stock : " + (result[n].Sumtotqty || 0) + "";
            var ProdRow2 = strr3;
            $('#pcol1').text(ProdRow);
            ...
            var ProdRow =
            "Average Cost : " + (parseFloat(result[n].AvgCost || 0).toFixed(Decimal)) + " , Last Purchase Cost : " + (parseFloat(result[n].LPCost || 0).toFixed(Decimal)) + " , Customer Last Selling Price :" + (parseFloat(result[n].CustLastSellingPrice || 0).toFixed(Decimal)) + " , Last Selling Price :" + (parseFloat(result[n].LastSellingPrice || 0).toFixed(Decimal)) + " , Total Stock :" + (result[n].Sumtotqty || 0) + " ";
            var ProdRow2 = strr3;
            $('#pcol1').text(ProdRow);
            $('#pcol2').append(ProdRow2);
            $('#ItemDetails').show();
...
function savesales() {
    ...
    for (var g = 1; g <= i; g++) {
        if ($('#select_unit' + g + ' :selected').text() != 'FOC') { 
            foc = 0;
            break;
        }
    }
    ...
    else if ((parseFloat($('#GrandTotal').val()) <= 0) && foc!=0) {
        warningshow('GrandTotal Cannot be Negative or Zero');
        return false;
    }
    else if (((parseFloat($('#GrandTotal').val()) <= 0) && ($('#disc').val() != 0)) && foc != 0) {
        warningshow('GrandTotal Cannot be Negative or Zero', 'disc');
        $('#disc').select();
    }
    else if (((parseFloat($('#disc').val()) >= parseFloat($('#CheckForDiscount').val())) && parseFloat($('#disc').val() != 0)) && foc != 0) {
...
function savesalesconfirm() {
        ...
        $.ajax(
    {
        ...
        type: "POST",
        url: "../SalesInvoice/MobileSalesInsertandUpdate",
        data: data,
        success: function (result) {
           // for (var i = 0; i <= result.oList.length; i++) {
                ...
                if (status != 0 && status != 6) {
                    ...
                    $('#alertdiv').show();
                    $('#alertdiv1').hide();
                    var Prod1 = "<tr class='jsgrid-row'><td><h2 style='color:#FF586B'>IMEI NUMBER ALREADY SALED!</h2></td></tr>" +
                      "<tr class='jsgrid-row' style='color:#607D8B'><th class='jsgrid-cell jsgrid-control-field jsgrid-align-center'>IMEI Number</th></tr>";
                    $('#tblAlert').append(Prod1);
                    $('#btnokalert').focus();

                    for (var d = 0; d < result.oList.length; d++) {

                        var salesimei = result.oList[d].ProductCode;
                        var Prod =
                        "<tr class='jsgrid-row' style='color:#607D8B'>" +
                        "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center'>" + result.oList[d].ProductCode + "</td></tr>";
...
//Print After Save
function PrintBillSalesMobile(Rowlen) {
    var AmountinWords = convertNumberToWords($('#GrandTotal').val())
    var myWindow = window.open("", "", "width=1500,height=1500");
    myWindow.document.write('<table width=100%><tr align=center><td rowsspan=2><img src="/ProjectImages/Company/Companylogo.png" height="100" width="500" alt="company logo"></td></td></tr> ');
    ...
    myWindow.document.write('<table width=100%><tr><td>Bill To</td><td align=right># ' + $('#txtBillseriesId option:selected').html() + ' - ' + $('#savedbillno').val() + '</td></tr><tr><td>' + $('#txtcustomer').val() + '</td><td align=right>Invoice Date : ' + $('#txtivdate').val() + '</td></tr>   <tr><td>' + $('#txtaddress').val() + '</td><td align=right>Terms شروط : ' + $('#select_payterms option:selected').html() + '</td></tr>   <tr><td>' + $('#select_place option:selected').html() + '</td><td align=right>Currency دقة : ' + $('#select_crncy option:selected').html() + ' </td></tr><tr><td colspan=2><hr></td></tr></table>')

    myWindow.document.write('<table width=100%><tr><td>#</td><td>Code/Description</td><td>Unit</td><td align=center>Qty</td><td align=center>Rate</td><td align=center>Amount</td></tr><tr><td></td><td>الرمز / الوصف</td><td>وحدة</td><td align=center>كمية</td><td align=center>معدل</td><td align=center>كمية</td></tr><tr><td colspan=6><hr></td></tr>');
    ...
    for (var i = 0; i <= Rowlen; i++) {
        ...
        if ($('#txtproduct' + Id).length) {
            myWindow.document.write('<tr><td>' + $('#td' + Id).text() + '</td><td><b>' + $('#txtproduct' + Id).val() + '</b></td></tr><tr><td></td><td>' + $('#ProductDesc' + Id).val() + ' ' + sr + '</td><td>' + $('#select_unit' + Id + ' option:selected').html() + '</td><td align=center>' + $('#txtquantity' + Id).val() + '</td><td align=right>' + $('#txtrate' + Id).val() + '</td>  <td align=right>' + $('#txtamnt' + Id).val() + '</td></tr><tr><td></td><td>' + $('#txtproductimei' + Id).val() + '</td></tr>');
        }
        else {
            myWindow.document.write('<tr><td height="30px"  colspan=6></td></tr>');
        }

    }
    myWindow.document.write('<tr><td colspan=6><hr></td></tr>');
    myWindow.document.write('<tr><td colspan=5>Grand Total المجموع الكلي :  ' + AmountinWords + ' Only' + '</td><td align=right><b>' + $('#GrandTotal').val() + '</b></td></tr>');
    myWindow.document.write('<tr><td colspan=6><hr></td></tr>');


    myWindow.document.write('<tr><td colspan=6></td></tr>');
    myWindow.document.write('<tr><td colspan=3>"Thank You For Your Business!"</td><td colspan=3><u>Sales Man : ' + $('#select_salesman option:selected').html() + '</u></td></tr><tr><td colspan=6>شكرا لك على عملك</td></tr><tr><td colspan=3>Please Come Again!</td><td colspan=3><u>Accountant : </u></td></td></tr><tr><td colspan=6>ارجوك عد مجددا</td>   </tr>    <tr><td colspan=3></td> <td colspan=3><u>Receiver Signature : </u>         </tr> <tr rowsapn=3><td colspan=6></br>    </br>      </tr>');
 
    myWindow.document.write('<tr align="center"><td colspan=1></td><td colspan=6><b>Terms and Conditions ' + '&nbsp;&nbsp;&nbsp' + '  الأحكام والشروط</b> </td></tr>');
    myWindow.document.write('<tr rowspan=2><td colspan=6></td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td  colspan=6>*Customers are requested to check the goods at the time of delivery </td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td  colspan=6> يطلب من العملاء التحقق من البضائع في وقت التسليم </td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td colspan=6>*Physical damage and water damage will not cover under warranty  </td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td   colspan=6>  الأضرار المادية والأضرار الناجمة عن المياه لن تغطي تحت الضمان </td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td  colspan=6>*Keep the invoice for warranty </td></tr>');
    myWindow.document.write('<tr align="center"><td colspan=1></td><td   colspan=6>  احتفظ بالفاتورة للضمان </td></tr>');
...
$(document).ready(function () {
    ...
    //ProductAdd Button Click 
    $('#btnadd').click(function () {
        if ($("#multipleimei").is(":checked"))
        {
          
            if ((parseFloat($('#txtrate0').val()) < parseFloat($('#originalrate0').val())) && ($('#select_unit0 :selected').text() != 'FOC')) {
                warningshow('Rate must be >= ' + (parseFloat($('#originalrate0').val())), 'txtrate0');
                $('#txtrate0').select(); 
                return false;
            }
            else
    ...
    $('#btnlocn').css("height", '85%');
    $('#select_locn').css("height", '85%');
    $('#btnsales').css("height", '85%');
    $('#select_salesman').css("height", '85%');
    ...
    $("#confirmOk").focus(function (e) {
        $("#confirmOk").removeClass("btn btn-outline-primary");
        $("#confirmOk").addClass("btn btn-primary");
    });

    $("#confirmOk").focusout(function (e) {
        $("#confirmOk").removeClass("btn btn-primary");
        $("#confirmOk").addClass("btn btn-outline-primary");
    });

    $("#confirmOk").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 37) {
            $("#confirmCancel").focus();
        }
    ...
    $("#confirmCancel").focus(function (e) {
        $("#confirmCancel").removeClass("btn grey btn-outline-secondary");
        $("#confirmCancel").addClass("btn grey btn-secondary");
    });

    $("#confirmCancel").focusout(function (e) {
        $("#confirmCancel").removeClass("btn grey btn-secondary");
        $("#confirmCancel").addClass("btn grey btn-outline-secondary");
    });

    $("#confirmCancel").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 39) {
            $("#confirmOk").focus();
        }
    ...
    $("#txtproduct0").keydown(function (e) {
       ...
       if (key == 39 && $('#txtproduct0').val() == '') {
            $('#txtproductimei0').focus();
            e.preventDefault();
        }
        else if (key == 37 && $('#txtproduct0').val() == '') {
            $('#btnadd').focus();
            e.preventDefault();
        }
    });
    ...
    $("#txtproductimei0").keydown(function (e) {
       ...
       else if (key == 37 && $("#txtproductimei0").val() == '') {
            e.preventDefault();
            $('#txtproduct0').focus(); 
        }
    ...
    $('#select_crncy,#txtcrncyrate').keypress(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            $('#txtproduct0').focus();
            e.preventDefault();
    ...
    $('#select_unit0').keypress(function (e) {
        ...
        if (key == 13) {
            ...
            if ($('#txtquantity0').prop("disabled") == true)
            {
                $('#txtrate0').focus();
                $('#txtrate0').select();
...
function EnquiryLoadCust(result) {
    disable_datatable('tblEnquirycust');
    var responseText = "<thead><tr><th style='width:90px;'><input type='checkbox' checked id= 'SlNoHeadCheckCust0' 'custom-control-input cz-bg-image-display' onchange='selectallcust()'>&nbsp;&nbsp;&nbsp;Select</th><th>Enquiry No</th><th>Customer</th><th>Address</th><th>Phone No</th><th>Invoice Date</th><th>Subject</th><th>Document Number</th><th>Salesman</th><th>Area</th><th>Currency</th><th style=display:none></th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        ...
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;" ><input type="checkbox" checked  name="checkenqry"  id=' + 'SlNoHeadCheckCust' + slno + '  "custom-control-input cz-bg-image-display" style="align:center"></td>' +
...
function ShowItemGet(result) {
    ...
    $('#Enquirydivsub').show();
    disable_datatable('tblEnquirypsub');
    var responseText = "<thead><tr><th style='width:90px;'> <input type='checkbox' checked id= 'SlNoCheckItem0' 'custom-control-input cz-bg-image-display' onchange='selectallprdt()'>&nbsp;&nbsp;&nbsp;Select</th><th>Enquiry No</th><th style=display:none></th><th>Product Code</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th style=display:none></th><th style=display:none></th><th style=display:none></th><th style=display:none></th><th>Tax Amount</th><th>Amount</th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;"><input type="checkbox" name="CheckItemen" checked  id= ' + 'SlNoCheckItem' + slno + ' ></td>' +
...
function QuotationLoad(result) {
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
            '<td ><input type="checkbox" name="CheckRevision" id= ' + 'SlNoCheckRevision' + slno + ' ></td>' +
...
function QuotationCustLoad(result) {
    disable_datatable('tblQtncust');
    var responseText = "<thead><tr><th style='width:90px;'><input type='checkbox' checked id= 'SlNoHdCheckCust0' 'custom-control-input cz-bg-image-display' onchange='selectallcust5()'>&nbsp;&nbsp;&nbsp;Select</th><th>Quotation No</th><th>Customer</th><th>Address</th><th>Phone No</th><th>Invoice Date</th><th>Document Number</th><th>Salesman</th><th>Area</th><th>Currency</th><th style=display:none></th><th style=display:none></th><th></th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
            '<td ><input type="checkbox" name="Checkcust" checked id= ' + 'SlNoHdCheckCust' + slno + ' ></td>' +
...
function ShowItemGetQtn(result) {
    ...
    $('#Qtndivsub').show();
    disable_datatable('tblQtnsub');
    var responseText = "<thead><tr><th style='width:90px;'> <input type='checkbox' checked id= 'SlNoCheckQtnItem0' 'custom-control-input cz-bg-image-display' onchange='selectallprdtqtn()'>&nbsp;&nbsp;&nbsp;Select</th><th>Quotation No</th><th style=display:none></th><th>Product Code</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th style=display:none></th><th style=display:none></th><th style=display:none></th><th style=display:none></th><th>Tax Amount</th><th>Amount</th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;"><input type="checkbox" name="CheckQtnItem" checked  id= ' + 'SlNoCheckQtnItem' + slno + ' ></td>' +
...
function SalesOrderCustLoad(result) {
    disable_datatable('tblSalesOrdercust');
    var responseText = "<thead><tr><th><input type='checkbox' checked id= 'SlNoHdCheckCust0' 'custom-control-input cz-bg-image-display' onchange='selectallcust4()'>&nbsp;&nbsp;&nbsp;Select</th><th>Order No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
            '<td ><input type="checkbox" checked name="Checkcust" id= ' + 'SlNoHdCheckCust' + slno + ' ></td>' +
...
function ShowItemGetSalesOrder(result) {
    ...
    disable_datatable('tblSalesOrdersub');
    var responseText = "<thead><tr><th style='width:90px;'> <input type='checkbox' checked id= 'SlNoCheckSoItem0' 'custom-control-input cz-bg-image-display' onchange='selectallprdtso()'>&nbsp;&nbsp;&nbsp;Select</th><th>Order No</th><th style='display:none'></th><th style='display:none'></th><th>Product Code</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th>Tax Amount</th><th>Amount</th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;"><input type="checkbox" name="CheckSordItem" checked  id= ' + 'SlNoCheckSoItem' + slno + ' ></td>' +
...
function SalesInvGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
                  ...
                  "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ")' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                  "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + result[n].ProductId + "' /><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + result[n].ProductCode + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + id + " value='" + result[n].ProductDescr + "'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat(result[n].FcProdRate).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat(result[n].ProdRate).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
...
function ShowItemGetSalesInvoice(result) {
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;"><input type="checkbox" name="CheckSInvItem" checked  id= ' + 'SlNoCheckSIItem' + slno + ' ></td>' +
...
function DeliveryOrderLoad(result) {
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
            '<td ><input type="checkbox" name="CheckDelivery" id= ' + 'SlNoCheckDelivery' + slno + ' ></td>' +
...
function ShowItemGetDeliveryOrder(result) {
    ...
    disable_datatable('tblDeliveryOrdersub');
    var responseText = "<thead><tr><th style='width:90px;'> <input type='checkbox' checked id= 'SlNoCheckSoItem0' 'custom-control-input cz-bg-image-display' onchange='selectallprdtdo()'>&nbsp;&nbsp;&nbsp;Select</th><th>DO No</th><th style='display:none'></th><th style='display:none'></th><th>Product Code</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Rate</th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th style='display:none'></th><th>Tax Amount</th><th>Amount</th></tr>" +
    ...
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;"><input type="checkbox" name="CheckDelordItem" checked  id= ' + 'SlNoCheckSoItem' + slno + ' ></td>' +
...
function DeliveryOrderCustLoad(result) {
    disable_datatable('tblDeliveryOrdercust');
    var responseText = "<thead><tr><th style='width:90px;'><input type='checkbox' checked id= 'SlNoHdCheckCust0' 'custom-control-input cz-bg-image-display' onchange='selectallcust3()'>&nbsp;&nbsp;&nbsp;Select</th><th>DO No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th><th style='display:none'></th></tr>" +
...
function formrefresh() {
    if (editflag != 0) {
        ...
        $('.form-control,.btn-outline-primary,.custom-control-input').prop("disabled", false);
        $('#txtBillSlNo').prop("disabled", true);
...
//Select Multiple IMEI no for a product
function IMEINoSelect() {
    var pdtsumqty = 0;
    var pdtsumqty1 = 0;
    for (var g = 1; g <= i; g++) {
    ...
    if ($.trim($("#txtproduct0").val()) == '') {
    ...
    else if ((parseFloat($("#txtrate0").val()) < parseFloat($('#AvgCost0').val()))  && (BelowCost=='NO') &&($('#select_unit0 :selected').text() != 'FOC')) {
        ...
        data.invtype = 0; 
        $.ajax({
            url: '../SalesInvoice/ProductSearchSalesMobile',
            type: "POST",
            data: data,
            dataType: "json",
            success: function (result) {
                $("#tblimeinumber tr").remove();
                $('#prdtimeiqty').text('Select ' + $('#txtquantity0').val() + ' IMEI Number');
                for (var k = 0; k < result.length; k++) {
                    var slno = parseInt(k + 1);
                    IMEIrow = "<tr class='jsgrid-row' id='" + slno + "'>" +
                   "<td class='text-center' style='width:auto;width:10%;'><input name='Checkimeino'  id='gridimeichk" + slno + "' type='checkbox' onchange='chkiemi()' >  <label for='gridimeichk" + slno + "'></label></td>" +
                   "<td class='text-center' style='width:auto;width:40%;border:1px solid #BABFC7;'><input type='text' id='gridimeino" + slno + "' disabled style='background-color:white;border:none;height:30px' class='form-control text-center' value=" + result[k].IMEI_Number + "></td>" +
                   "</tr>";
                    $('#tblimeinumber').append(IMEIrow);
                }
            }
        })
    }
...
//Add Products to Grid when using multiple imeino 
function IMEIProductAdd() {
    ...
    var flgs = 0;
    var checkboxes = document.getElementsByName('Checkimeino');
    for (var k = 0, j = checkboxes.length; k < j; k++) {
        if (checkboxes[k].checked == true) {
            flgs++;
        }
    ...
    for (var p = 1; p <= i; p++) {
        ...
        for (var m = 1; m <= checkboxes.length; m++) {

            if ($("#gridimeichk" + m).is(":checked")) {
                if (($('#txtproductimei' + p).val() == $("#gridimeino" + m).val())) {
                    IMEIFlag = 1;
                }
            }
        }
    ...
    if (flgs != $("#txtquantity0").val()) {
        warningshow('Please Select ' + $("#txtquantity0").val() + ' IMEI Number', 'txtproductimei0');
    }
    else if (IMEIFlag == 1) {
        warningshow('IMEI Number Already added');
    }
    else {
        if (rowcount == 0) {
            i = 1;
        ...
        for (var m = 1; m <= checkboxes.length; m++) {
            ...
            if ($("#gridimeichk" + m).is(":checked")) {
                ...
                "<td id= 'col14_" + id + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input  class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + id + ")' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button'  type='button' id='canceledit' onclick='editcancel(" + id + ")' title='Cancel edit'></td>" +
                "<td id=" + 'td' + id + " class='jsgrid-cell' style='width:15px;text-align:center'>" + slno + "</td>" +
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' id=" + 'PrdtId' + id + " style='display:none' value='" + $("#PrdtId0").val() + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + id + " value='" + $("#Otherdescription0").val() + "'><input type='text' style='display:none;height:30px' id=" + 'MRP' + id + " value='" + $("#MRP0").val() + "'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + id + " value='" + $("#txtproduct0").val() + "'></td>" +
                ...
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproductimei' + id + " value='" + $("#gridimeino" + m).val() + "'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this)'>" + UnitSelect + "</select></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + id + " value=1 style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseInt($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat($('#txtfcrate0').val()).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat($('#txtrate0').val()).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress='isNumber(event,this)' onblur=checkrate(" + id + ")><input type='text' id=" + 'originalrate' + id + " value=" + parseFloat($('#txtrate0').val()).toFixed(Decimal) + " style='display:none'></td>" +
                ...
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)'>" + TaxSelect + "</select></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + ($('#txttaxpercent0').val() || 0) + " onkeyup='amountcalculation(" + id + ")'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxamnt' + id + " value=" + parseFloat($('#txtfctaxamnt0').val()).toFixed(Decimal) + "  style='display:none'><input typ='text' style='background-color:white;height:30px' class='form-control text-center'  id=" + 'txttaxamnt' + id + " value=" + parseFloat($('#txttaxamnt0').val()).toFixed(Decimal) + " disabled=''></td>" +
        ...
        var qt = parseInt($('#txtquantity0').val());
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.oList.length >= 1) {
                    var ss = parseInt(slno);
                    var m = id;
                    for (var j = 0; j <= result.oList.length; j++) {
                        var ProdRow2 = "<tr id=" + 'row' + m + " class='jsgrid-row'>" +
                "<td id= 'col13_" + m + "' class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:15px'><input class='jsgrid-button jsgrid-edit-button jsgrid-align-center'  id='btnroedit' type='button' onclick='EditRow(" + m + ")' title='Edit'><input class='jsgrid-button jsgrid-delete-button jsgrid-align-center' id='btnrodelt' type='button' onclick='rowdelete(" + m + ")' title='Delete'></td>" +
                "<td id= 'col14_" + m + "'  class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='display:none;width:15px'><input  class='jsgrid-button jsgrid-update-button' id='Update' type='button' onclick='updaterow(" + m + ")' title='Update'><input class='jsgrid-button jsgrid-cancel-edit-button'  type='button' id='canceledit' onclick='editcancel(" + m + ")' title='Cancel edit'></td>" +
                "<td id=" + 'td' + m + " class='jsgrid-cell' style='width:15px;text-align:center'>" + ss + "</td>" +
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' id=" + 'PrdtId' + m + " style='display:none' value='" + result.oList[j].AccessoriesId + "' /><input type='text' style='display:none;height:30px' id=" + 'Otherdescription' + m + " ><input type='text' style='display:none;height:30px' id=" + 'MRP' + m + " ><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproduct' + m + " value='" + result.oList[j].AccessoryCode + "'></td>" +
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'ProductDesc' + m + " value='" + result.oList[j].AccessoriesName + "'></td>" +
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproductimei' + m + " value='" + $("#txtproductimei0").val() + "'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + m + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + m + ",this)'>" + UnitSelect + "</select></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + m + " value=" + qt + " style='background-color:white;height:30px' onkeyup='checkqty(" + m + "),amountcalculation(" + m + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + m + " style='display:none'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + m + " value='0.00' style='display:none'><input type='text' disabled='' class='form-control text-center' id=" + 'txtrate' + m + " value='0.00' style='background-color:white;height:30px' onkeyup='amountcalculation(" + m + ")' onkeypress='isNumber(event,this)' onblur=checkrate(" + m + ")></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' id=" + 'LineAvgCost' + m + " value='0.00' style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + m + " value='0.00' style='background-color:white;height:30px'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + m + " value='0.00'  style='display:none'><input type='text' disabled='' id=" + 'txtdiscount' + m + " class='form-control text-center' style='background-color:white;height:30px' value='0.00' onkeyup='amountcalculation(" + m + ")' onkeypress='isNumber(event,this)'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxableamnt' + m + " value='0.00'  style='display:none'><input type='text' disabled='' class='form-control text-center' style='background-color:white;height:30px' id=" + 'txttaxableamnt' + m + " value='0.00' onkeyup='amountcalculation(" + m + ")'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + m + " class='form-control' onchange='ChangeTax(" + m + ",this)'>" + TaxSelect + "</select></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + m + " value=" + ($('#txttaxpercent' + id).val() || 0) + " onkeyup='amountcalculation(" + m + ")'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxamnt' + m + " value='0.00'  style='display:none'><input typ='text' style='background-color:white;height:30px' class='form-control text-center'  id=" + 'txttaxamnt' + m + " value='0.00' disabled=''></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcamnt' + m + " value='0.00'  style='display:none'><input typ='text' class='form-control text-center' style='background-color:white;height:30px' disabled='' id=" + 'txtamnt' + m + " value='0.00'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:40px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_location' + m + " class='form-control'>" + LocnSelect + "</select></td>" +
                "</tr>";
                        $('#tblsalesinvoice').append(ProdRow2);
                        $('#select_unit' + m).val($('#select_unit' + (id - 1)).val());
                        $('#select_tax' + m).val($('#select_tax' + (id - 1)).val());
                        $('#select_location' + m).val($('#select_location0').val());
                        m++;
                        ss++;
                        i++;
                    }
        ...
        $('#temptxtproduct0').val('');
        $("#IMEIDiv").hide();
        $('#multipleimei').prop("checked", false);
...
var qtyflag = 0;
var foc = 0;
//Add Products to Grid when using single imeino
function productadd() {
    var imeilen = $('#txtproductimei0').val();
    var a = parseFloat($('#txtdiscount0').val());
    var c = parseFloat($('#txtrate0').val());
    $("#txtdiscount0").val(isNaN(a) ? 0 : a);
    $('#txtrate0').val(isNaN(c) ? 0 : c);
    ...
    if ($.trim($("#txtproduct0").val()) == '') {
    ...
    else if ($('#IMEIlength').val() != imeilen) {
    ...
    else if ((parseFloat($('#txtrate0').val()) < parseFloat($('#originalrate0').val())) && ($('#select_unit0 :selected').text() != 'FOC')) {
        warningshow('Rate must bee >=  ' + (parseFloat($('#originalrate0').val())), 'txtrate0');
        //$('#txtrate0').val(parseFloat($('#originalrate' + id).val()));
        $('#txtrate0').select();
        return false;
    }
    else if (foc == 0 && ($("#txtamnt0").val() == 0)) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                "<td class='jsgrid-cell jsgrid-align-right' style='width:75px;text-align:center'><input type='text' style='height:30px;background-color:white' disabled='' class='form-control text-left' id=" + 'txtproductimei' + m + " value='" + $("#txtproductimei0").val() + "'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select disabled='' id=" + 'select_unit' + m + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + m + ",this)'>" + UnitSelect + "</select></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' disabled='' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + m + " value=" + parseInt($('#txtquantity' + id).val()) + " style='background-color:white;height:30px' onkeyup='checkqty(" + m + "),amountcalculation(" + m + ")' onkeypress='isNumberInt(event,this)'><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + m + " style='display:none'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'> <input type='text' id=" + 'txtfcrate' + m + " value='0.00' style='display:none'><input type='text' disabled='' class='form-control text-center checks' id=" + 'txtrate' + m + " value='0.00' style='background-color:white;height:30px' onkeyup='amountcalculation(" + m + ")' onkeypress='isNumber(event,this)' onblur=checkrate(" + m + ")></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><input type='text' id=" + 'LineAvgCost' + m + " value='0.00' style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + m + " value='0.00' style='background-color:white;height:30px'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px'><input type='text' id=" + 'txtfcdiscount' + m + " value='0.00'  style='display:none'><input type='text' disabled='' id=" + 'txtdiscount' + m + " class='form-control text-center' style='background-color:white;height:30px' value='0.00' onkeyup='amountcalculation(" + m + ")' onkeypress='isNumber(event,this)'></td>" +
                "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxableamnt' + m + " value='0.00'  style='display:none'><input type='text' disabled='' class='form-control text-center' style='background-color:white;height:30px' id=" + 'txttaxableamnt' + m + " value='0.00' onkeyup='amountcalculation(" + m + ")'></td>" +
                "<td class='jsgrid-cell jsgrid-align-center' style='width:30px'><select style='background-color:white;height:30px' disabled='' id=" + 'select_tax' + m + " class='form-control' onchange='ChangeTax(" + m + ",this)'>" + TaxSelect + "</select></td>" +
                ...
                "</tr>";
                        $('#tblsalesinvoice').append(ProdRow2);
                        $('#select_unit' + m).val($('#select_unit' + id).val());
                        $('#select_tax' + m).val($('#select_tax' + id).val()); 
                        $('#select_location' + m).val($('#select_location' + id).val());
                        m++;
                        ss++;
                        i++;
                    }
...
//---------check selecting multiple iemi no 
function checkmultipleimei() {
    if ($('#PrdtId0').val() != 0 && $('#IEMICount').val()!=0) { 
        if ($("#multipleimei").is(":checked")) {
            $('#txtproductimei0').prop("disabled", true);
            $('#txtproductimei0').val('')
            $('#txtquantity0').prop("disabled", false);
            $('#txtquantity0').focus();
            $('#txtquantity0').select();
...
function JobAdd() {
    ...
    if ($.trim($('#productjob').val()) == "") {
        ...
        var prodjob = "<tr id=" + 'row' + id + " class='jsgrid-row'>" +
            ...
            "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:40px'><input type='text' id=" + 'txtfctaxamnt' + id + " value=" + parseFloat($('#fctxttax_job').val()).toFixed(Decimal) + "  style='display:none'><input typ='text' style='background-color:white;height:30px' class='form-control text-center'  id=" + 'txttaxamnt' + id + " value=" + parseFloat($('#txttax_job').val()).toFixed(Decimal) + " disabled=''></td>" +
...
function updaterow(id) {
    ...
    else if ((($('#txtrate' + id).val() || 0) == '') && !($('#txtrate' + id).is(".checks")) && ($('#select_unit' + id + ' :selected').text() != 'FOC')) {
        warningshow('Please Enter the Rate', 'txtrate' + id);
        return false;
    }
    else if (($("#txtamnt" + id).val() < 0) && !($('#txtrate' + id).is(".checks")) && ($('#select_unit' + id + ' :selected').text() != 'FOC')) {
        warningshow('Amount Cant be negative', 'txtrate' + id);
        $('#txtrate' + id).select();
    }
    else if ((foc == 0 && ($("#txtamnt" + id).val() == 0)) && !($('#txtrate' + id).is(".checks")) && ($('#select_unit' + id + ' :selected').text() != 'FOC')) {
        warningshow('Amount Cant be Zero', 'txtrate' + id);
        $('#txtrate' + id).select();
    }
    else if ((BelowCost == 'NO') && ($('#PrdtId' + id).val() != 0) && ($('#AvgCost' + id).val() != 0) && (parseFloat($('#txtrate' + id).val()) < parseFloat($('#AvgCost' + id).val())) && !($('#txtrate' + id).is(".checks")) && ($('#select_unit'+id+' :selected').text() != 'FOC')) {
        warningshow('Rate must be greater than ' + $('#AvgCost' + id).val());
        $('#txtrate' + id).select();
        return false;
    }
    else if ((parseFloat($('#txtrate' + id).val()) < parseFloat($('#originalrate' + id).val())) && !($('#txtrate' + id).is(".checks")) && ($('#select_unit'+id+' :selected').text() != 'FOC')) {
        warningshow('Rate must be >= ' + (parseFloat($('#originalrate' + id).val())), 'txtrate' + id);
        $('#txtrate' + id).select();
        return false;
    }
