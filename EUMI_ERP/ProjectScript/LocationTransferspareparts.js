    ...
    if ($.trim($('#txtproduct0').val()) == "") {
        ...
        var ProdRowEdit = "<tr id=" + 'row' + slno + " class= jsgrid-row >" +
                "<td  id=" + 'td' + slno + " class= jsgrid-cell  style= width:3%;text-align:center >"
                 ...
                 + parseInt($('#txtquantity').val()) + "'><input type='hidden' id='hiddenqty" + slno + "' value='" + LOcQty + "'/>  </td>" +
                 ...
                 + parseFloat($('#txttotal').val() || 0).toFixed(Decimal) + "></td>" +
...
function formrefresh() {
    if (editflag != 0) {
        ...
        $('#Location,#TransferNo').prop('disabled', false);
        //$('#Location').css('background-color', 'white');
        $('#GrandTotal').css("background-color", 'white');
        $('#hiddenquantitycheck').val('0');
...
function SaveAndUpdate(Flag) {
    ...
    //}
        ...
        warningshow('Please Select a transfer', 'TransferNo');
        return false; 
    }
...
function transactionsave() {
    ...
    if (oArray != "") {
        ...
        $.ajax({
            type: "POST",
            url: "../inventory/LocationTransferSparepartsInsert", 
            data: data,
            success: function (result) {
                $('#Loadingsave').hide();
...
function CheckCancelTransfer() {
    if ($.trim($('#cancelotp').val()) == '') {
        ...
        data.Operation = 'Location Transfer- OTP - Cancel';
...
function CheckEditTransfer() {
    if ($.trim($('#txtotp').val()) == '') {
        ...
        data.Operation = 'Location Transfer- OTP - Edit';
        data.DeptId = ERPDeptId;
        $.ajax({
            type: "POST",
            url: "../Home/OTPCheckforUser",
...
function LocationTransferGet(result,flag) {
    ...
    for (var m = 0; m < result.length; m++) {
        ...
        var ProdRow = "<tr id=" + 'row' + slno + " class= jsgrid-row >" +
           "<td  id=" + 'row' + slno + " class= jsgrid-cell  style= width:3%;text-align:center >"
          + slno + "<input type='hidden' id='SlNo" + slno + "' value=" + slno + "></td>" +
          "<td class= 'jsgrid-cell jsgrid-align-left'  style='width:15%;text-align:center' ><input type='hidden' id='ProductId" + slno + "' value='" + result[m].ProductId + "'/> <input typ='text' class='form-control text-center' disabled=disabled  style='background-color:white;height:30px;' id='txtprd" + slno + "' value='"
          + result[m].ItemCode + "'>  </td>" +
          "<td class= 'jsgrid-cell jsgrid-align-left'  style='width:20%;text-align:center' display='none;'> <input type='text' class='form-control text-center'  disabled  style='background-color:white;height:30px;' id='txtdesc" + slno + "' value='"
          ...
          + result[m].UnitId + "' onclick='Editrow(" + slno + ")' onfocusinonclick='Editrow(" + slno + ")' onfocusout='Updaterow(" + slno + ")'>" + UnitSelect + "</td>" +
          ...
          + result[m].Quantity + "'  onkeypress='return isNumber1(event)' onclick='Editrow(" + slno + ")' onfocusinonclick='Editrow(" + slno + ")' onfocusout='Updaterow(" + slno + ")'><input type='hidden' id='hiddenqty" + slno + "' value='" + result[m].Qty + "'/> </td>" +
          "<td class= 'jsgrid-cell jsgrid-align-center'  style='width:6%;' ><input typ='text' class='form-control text-center' disabled=disabled style='background-color:white;height:30px;'  onkeypress='isNumbercheck(event,this)'  id='rate" + slno + "' value="
          + (result[m].Price.toFixed(Decimal)) + " onclick='Editrow(" + slno + ")' onfocusinonclick='Editrow(" + slno + ")' onfocusout='Updaterow(" + slno + ")'></td>" +
          "<td class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;' ><input typ='text' class='form-control text-center' disabled=disabled  style='background-color:white;height:30px;background-color:white'readonly onkeyup=''   id='tamnt" + slno + "' value="
          + (result[m].Total.toFixed(Decimal)) + "></td>" +
          "<td id='Edit" + slno + "' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='background-color:white;width:3%;' ><input class= 'jsgrid-button jsgrid-edit-button editgridtr' style='display:none'  onclick='Editrow(" + slno + ")' id='Edit' type=button title=Edit ><input class= 'jsgrid-button jsgrid-delete-button deletegridtr' disabled  style='' type= button id='delete' title= Delete onclick='rowdelete(" + slno + ")' ></td>" +
          "<td id='Update"
        + slno + "'  class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='background-color:white;width:3%;display:none'><input class='jsgrid-button  disabled jsgrid-update-button' onclick='Updaterow(" + slno + ")' type=button id='update' title=Update style='' ><input class='jsgrid-button jsgrid-cancel-edit-button' style='' onclick='CancelEditrow(" + slno + ")' type='button' id='CancelEdit' title='Cancel edit' value='' ></tr>";
        ...
        if (flag == 1) {
            $('.editgridtr').hide(); 
            $('.deletegridtr').prop('disabled',false);
            $('#select_unit' + slno).prop('disabled', false);
            $('#txtqnty' + slno).prop('disabled', false);
            $('#rate' + slno).prop('disabled', false);
