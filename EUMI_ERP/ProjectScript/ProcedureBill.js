    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.oList.length > 0) {
                $('.TsRw').remove();
                for (var i = 0; i <= result.oList.length; i++) {
                    var DDh = (result.oList[i].TestName).split("##");
                    $('#TestName0').val(DDh[1])
                    $('#TestCode0').val(DDh[1])

                    $('#TestId0').val(result.oList[i].TestId);
                    $('#PQty0').val(parseInt(result.oList[i].Department||1));
                    $('#PRate0').val(result.oList[i].PRate);
                    AmountCalc(0)
                    AddTest();
                }
...
function AddTestConfirm() {
    ...
    var ProdRow1 = "<tr id=" + 'Row' + id + " class='jsgrid-row TsRw' onclick='EditRow(" + id + ")' onfocusout=updaterow(" + id + ")>" +
          ...
          "<td class='jsgrid-cell jsgrid-align-left' style='width:40%'><input type='text' readonly id='TestCode" + id + "' value='" + $('#TestCode0').val() + "' class='form-control' style='height:30px;background-color:white' ></td>" +
    ...
    if (result.length != 0) {
        ...
        for (var n = 0; n < result.length; n++) {
            ...
            ProdRow += "<tr class='jsgrid-row' ondblclick='ngOnDestroy(),copyrefresh(2),GetBillDetails(" + result[n].BillYear + ", " + result[n].BillNo + ", " + result[n].DeptId + ");'>" +
                ...
                "<td class='text-center'> " + result[n].RegNo + "</td>" +
...
function SaveAndUpdate(flg) {
    ...
    if (($("#OpNo").val() || 0) == 0) {
        warningshow('Please select valid patient', 'RegNo');
    }
    else if (($("#Opseriesname").val() || 0) == 0) {
        warningshow('Please select valid patient', 'RegNo');
    }
    else if (($("#BillNo").val() || 0) == 0) {
    ...
    for (var k = 1; k <= Count; k++) {
        ...
        if (TestId != undefined) {
            oArray.push({
                ...
                //'SurgeryDate': '' + '#' + a + '#' + b + '#' + c + '#' + BILLYR  //cash/upi/card

                'SurgeryDate': '' + '#' + a + '#' + b + '#' + c + '#' + d + '#' + ContactNo + '#' + BILLYR    //cash/upi/card
            })
        }
        //console.log(oArray)
