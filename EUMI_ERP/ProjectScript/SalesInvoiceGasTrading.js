var LocnSelect = ''; var SavingFlg = 0;
                ...
                for (var n = 0; n < result.length; n++) {
                    ...
                    ProdRow += "<tr class='jsgrid-row' id=" + 'pdctrow' + (n + 1) + ">" +
                       ...
                       "<td class='text-left'>" + result[n].DepartmentName + "                                   </td>" +
                        "</tr>";
                    qty = parseFloat(qty) + parseFloat(result[n].ProdQty);
                }
        ...
        success: function (result) {
            ...
            if (result.length != 0) {
                ...
                for (var p = 0; p < result.length; p++) {
                    ...
                    qtys = parseFloat(qtys) + parseFloat(result[p].Quantity);
                }
...
function AllTransactions(ProductId, product, ProductDesc, flag) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.length != 0) {
                var BalQty = parseFloat(result[0].OpeningQty);

                for (var n = 0; n < result.length; n++) {
                    if (result[n].Status == 'IN') {
                        BalQty += parseFloat(result[n].Quantity)
                    }
                    else {
                        BalQty = BalQty - parseFloat(result[n].Quantity)
                    }
        ...
        for (var l = 0; l < result.length; l++) {
            ...
            if ((MultiPdtArray.indexOf("" + result[l].ItemId + "") == -1)) {
                ...
                var responseText =
                    ...
                    '<td style="text-align:center;width:130px;" >' +//***** JOB CHANGE *****
                    '<input type="text" style="height:30px;" class="form-control" id= ' + 'qtypr' + slno + '  onkeypress=isNumber(event,this) onkeyup=gridamountcalculation(' + slno + '),checkqtystock(' + slno + '),Checkme(' + slno + ') onkeydown=Focusnext(event,' + slno + ',"q")></td>' +
...
function checkqtystock(id) {
    if (Negativebill == 'NO' && ($("#ItemLocngrid" + id).find("option:selected").attr("name") == 0)) {
        ...
        if (pflg == 0) {                      //If Product is not already added in product grid-check qty<stockqty
            //***** JOB CHANGE *****
            if (($('#ItemIdgrid' + id).val() != 0) && (parseFloat($('#qtypr' + id).val()) > parseFloat($('#prStockqty' + id).val()))) {
        ...
        else                          //else check qty < stockqty-(sum of qty of pdts in pdt grid)
        {
            ...
            for (var g = 1; g <= i; g++) {
                if (($('#PrdtId' + g).val() == $("#ItemIdgrid" + id).val()) && ($('#select_location' + g).val() == $("#ItemLocngrid" + id).val()) && ($('#txtquantity' + g).attr('name') == 'prdaddqty')) {
                    pdtsumqty = parseFloat(pdtsumqty) + parseFloat($("#txtquantity" + g).val());
                    pdtsumqty1 = parseFloat(pdtsumqty);
                }
            }
            if ($("#qtypr" + id).val() > (parseFloat($('#prStockqty' + id).val()) - pdtsumqty1)) {
                warningshow('Available Quantity is ' + (parseFloat($('#prStockqty' + id).val()) - pdtsumqty1), 'qtypr' + id);
                $('#qtypr' + id).val('');
                $('#qtypr' + id).focus();
                return false;
            }
...
function checkratestock(id) {
    ...
    if ((BelowCost == 'NO') && ($('#BCost1_' + id).text() == 0) && ($('#pravgcost' + id).val() != 0) && (parseFloat($('#prrate' + id).val()) < parseFloat($('#pravgcost' + id).val()))) {
...
function addgridproducts() {
    ...
    for (j = 1; j <= $('#mltrcurrlength').val() ; j++) {
        if ($("#SlNoHeadCheckgrid" + j).is(":checked")) {
            if ((BelowCost == 'NO') && ($('#BCost1_' + j).text() == 0) && ($('#pravgcost' + j).val() != 0) && (parseFloat($('#prrate' + j).val()) < parseFloat($('#pravgcost' + j).val()))) {
                rateflg = 1;
                break;
            }
            else
...
function savesalesconfirm() {
    ...
    for (var k = 1; k <= i - 1; k++) {
        ...
        var BatchSNo = 1;
        var Batch = $('#Batch').val();           //$('#txtlgrgname').val() + '-#%%#-' + $('#txtlgrgadr').val() + '-#%%#-' + $('#PhoneNo').val();        //Save Garrage Details
        ...
        var FractionQty = $('#FractionQty'+k).val();
        var CalcQty = $('#CalcQty'+k).val();
        ...
        $.ajax(
    {
        ...
        type: "POST",
        url: "../SalesInvoice/SalesGasTradingInsertandUpdate", 
        data: data,
        success: function (result) {
...
function focusoutgrid(col, Id) {
    ...
    if (col == 'q')
    { $('#CalcQty' + Id).css("background-color", 'white'); }
    else if (col == 'r')
...
function focusingrid(col, Id) {
    ...
    if (col == 'q')
    { $('#CalcQty' + Id).css("background-color", '#58dbe4'); }
    else if (col == 'r')
...
{
    if ($.trim(result[0].BankName) != '')
    {
        $('#CmpnyBank').val(result[0].BankName);
        $('#CmpnyAcnt').val(result[0].AccountNo);
        $('#CmpnyIban').val(result[0].IBANNo);
        $('#CmpnySwiftCode').val(result[0].SwiftCode);
        ...
        $.ajax({
            ...
            data: srlno,
            success: function (result) {
                getcmpnybank(result.oList);
            }
        });
    }  
}

function getcmpnybank(result)
{
    $('#CmpnyBankAcnt').val(result[0].GIBank);
}
...
$(document).ready(function () {
    ...
    $("#select_location0").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            e.preventDefault();
            $('#txtproduct0').focus();
    ...
    $("#txtBillSlNo").keydown(function (e) {
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key == 13) {
            e.preventDefault();
            $('#txtivdate').focus();
        ...
        for (m = 1; m <= row; m++) {
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckItem" + m).is(":checked")) {
                    ...
                    var qty = parseFloat($('#qty_' + m).val() || 0);
        ...
        for (m = 1; m <= row; m++) {
            ...
            if (flg == 0) {
                ...
                if ($("#SlNoCheckSoItem" + m).is(":checked")) {
                    ...
                    var DelDate = $('#DelDate' + m).val();
                    var Dellpo = $('#Dellpo' + m).val();
                    ...
                    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                        ...
                        "<td class='jsgrid-cell jsgrid-align-right' style='width:13%;text-align:center'><input disabled='' class='form-control text-left' type='text' style='height:30px;background-color:white' id=" + 'ProductDesc' + id + " value='" + ProductDescr + "' data-toggle='tooltip' title='" + ProductDescr + "'></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select  id=" + 'select_unit' + id + " style='background-color:white;height:30px' class='form-control' onchange='changeunit(" + id + ",this),QtyCal(" + id + ")' onkeydown=Focusnextgrid(event,'u'," + id + ") onfocusout=focusoutgrid('u'," + id + ") onfocusin=focusingrid('u'," + id + ")>" + UnitSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:4%;'><select  id=" + 'select_location' + id + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                        "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' name='prdaddqty' class='form-control text-center' id=" + 'CalcQty' + id + " value="//***** JOB CHANGE *****
                        + parseFloat(qty) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + "),QtyCal(" + id + ")' onkeydown=Focusnextgrid(event,'q'," + id + ") onkeypress=isNumber(event,this) onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input style=display:none type=text id=" + 'FractionQty' + id + " value=1 /></td>" +
                        ...
                        "<td style='display:none' id=" + 'Bin' + id + ">" + bindel + "</td>" +
                         "<td style='display:none'><input type='text'  id=" + 'BelowCostFlag' + id + " value=" + $('#BCost6_' + m).val() + " ><input type='text'  id=" + 'DelvLponum' + id + " value='" + Dellpo + "' ><input type='text'  id=" + 'DelvDate' + id + " value='" + DelDate + "' ></td>" +
                        "</tr>";
...
function Enquirypopupwindow(id) {
    ...
    {
        if (($('#txtCustId').val()||0) == 0)              //If CustId==0 Get all DeliveryOrder Details
        {
            $("#Enquirypopup").css("margin-top", '-50px');
            $('#Enquirypopup').show();
        ...
        else if (($('#txtCustId').val() || 0) > 0)                                        //If CustId!=0 Get DeliveryOrder Details against That Customer                     
        {
            $("#Enquirypopup").css("margin-top", '-50px');
            $('#Enquirypopup').show();
            $('#Enquiryheader').text('Delivery Order Details');
...
function CustomerEnquiryGets(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                  ...
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text'  class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseFloat($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                  "<td class='jsgrid-cell jsgrid-align-center' style='width:3%'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat(result[n].FcProdRate).toFixed(Decimal) + "  style='display:none'><input type='text'  class='form-control text-center' id=" + 'txtrate' + id + " value=" + parseFloat(result[n].ProdRate).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'r'," + id + ") onfocusout=focusoutgrid('r'," + id + ") onfocusin=focusingrid('r'," + id + ")></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px;display:none'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat(result[n].FcProdDisc).toFixed(Decimal) + "  style='display:none'><input type='text'  id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat(result[n].ProdDisc).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'d'," + id + ") onfocusout=focusoutgrid('d'," + id + ") onfocusin=focusingrid('d'," + id + ")></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:3%'><input type='text' id=" + 'txtfctaxableamnt' + id + " value=" + parseFloat(result[n].FCTaxableAmount).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' style='background-color:white;height:30px' id=" + 'txttaxableamnt' + id + " value=" + parseFloat(result[n].TaxableAmount).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")'></td>" +
                  "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:22px;display:none'><input typ='text' disabled='' style='background-color:white;height:30px' class='form-control text-center' id=" + 'txttaxpercent' + id + " value=" + result[n].TaxPercent + " onkeyup='amountcalculation(" + id + ")'></td>" +
...
function ShowItemGet(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'enqnoRow' + slno + '>' + result[l].EnquiryNo + '<input type="hidden" id="enqno"' + slno + '" value=' + result[l].EnquiryNo + '></td>' +
        ...
        '<td id=' + 'qty' + slno + '>' + parseFloat(result[l].ProdQty) + '<input type="hidden" id="qty_' + slno + '" value=' + result[l].ProdQty + '></td>' +
...
function DeliveryOrderLoad(result) {
    disable_datatable('tblDeliveryOrder');
    var responseText = "<thead><tr><th></th><th>DO No</th><th>LPO No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th></tr>" +
                              "<tr><th>Select</th><th>DO No</th><th>LPO No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th></tr></thead><tbody>";
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
...
function DeliveryOrderGets(result) {
    ...
    if (result.length > 0)
    {
        if (result[0].CustId > 0)
        {
        var data = {};
        data.CustId = result[0].CustId;
        data.cstyp = 0;
        $.ajax({
            type: "POST",
            url: "../Master/CustomerGetandGets",
            data: data,
            success: function (result) {
                balload(result.oList[0].CustAccount);
            }
        });
        }
        ...
        for (var n = 0; n < result.length; n++) {
            ...
            var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + result[n].ProductId + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                      ...
                      "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:4%'><select  id=" + 'select_location' + id + " value=" + result[n].LocnId + " style='background-color:white;height:30px' class='form-control' onchange ='GetBin(" + id + "," + result[n].ProductId + ",0)' onkeydown=Focusnextgrid(event,'l'," + id + ") onfocusout=focusoutgrid('l'," + id + ") onfocusin=focusingrid('l'," + id + ")>" + LocnSelect + "</select></td>" +
                      "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' name='prdaddqty'  class='form-control text-center' id=" + 'CalcQty' + id + " value="//***** JOB CHANGE *****
                       + parseFloat(result[n].ProdQty || 0) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + "),QtyCal(" + id + ")' onkeydown=Focusnextgrid(event,'q'," + id + ") onkeypress=isNumber(event,this) onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input style=display:none type=text id=" + 'FractionQty' + id + " value=1 /></td>" +
                      "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' disabled class='form-control text-center' id=" + 'txtquantity' + id + " value=" + result[n].ProdQty + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + parseFloat($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
                      "<td class='jsgrid-cell jsgrid-align-center' style='width:3%'> <input type='text' id=" + 'txtfcrate' + id + " value=" + parseFloat(result[n].FcProdRate).toFixed(Decimal) + "  style='display:none'><input type='text'  class='form-control text-center'  id=" + 'txtrate' + id + " value=" + parseFloat(result[n].ProdRate).toFixed(Decimal) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'r'," + id + ") onfocusout=focusoutgrid('r'," + id + ") onfocusin=focusingrid('r'," + id + ")></td>" +
                      "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:30px;display:none'><input type='text' id=" + 'txtfcdiscount' + id + " value=" + parseFloat(result[n].FcProdDisc).toFixed(Decimal) + "  style='display:none'><input type='text' id=" + 'txtdiscount' + id + " class='form-control text-center' style='background-color:white;height:30px' value=" + parseFloat(result[n].ProdDisc).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'d'," + id + ") onfocusout=focusoutgrid('d'," + id + ") onfocusin=focusingrid('d'," + id + ")></td>" +
                      "<td class='jsgrid-cell jsgrid-control-field jsgrid-align-center' style='width:3%'><input type='text' id=" + 'txtfctaxableamnt' + id + " value=" + parseFloat(result[n].FCTaxableAmount).toFixed(Decimal) + "  style='display:none'><input type='text' disabled='' class='form-control text-center' style='background-color:white;height:30px' id=" + 'txttaxableamnt' + id + " value=" + parseFloat(result[n].TaxableAmount).toFixed(Decimal) + " onkeyup='amountcalculation(" + id + ")'></td>" +
                      ...
                      "<td class='jsgrid-cell jsgrid-align-center' style='width:30px;display:none'><input type='text' id=" + 'LineAvgCost' + id + " value=" + (result[n].AvgCost * result[n].ProdQty) + " style='display:none'> <input type='text' disabled='' class='form-control text-center' id=" + 'AvgCost' + id + " value=" + parseFloat(result[n].AvgCost).toFixed(Decimal) + " style='background-color:white;height:30px'></td>" +
                       "<td class='jsgrid-cell jsgrid-align-center' style='width:3%'><select style='background-color:white;height:30px' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)' onkeydown=Focusnextgrid(event,'t'," + id + ") onfocusout=focusoutgrid('t'," + id + ") onfocusin=focusingrid('t'," + id + ")>" + TaxSelect + "</select></td>" +
                      "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
                      "<td style='display:none'><input type='text'  id=" + 'BelowCostFlag' + id + " value=" + result[n].BelowCostFlag + " ><input type='text'  id=" + 'DelvDate' + id + " value=" + result[n].InvDate + " >" +
                      "<input type='text'  id=" + 'DelvLponum' + id + " value='" + result[n].LPONumber + "' ></td>" +
                      "</tr>";
            $('#tblsalesinvoice').append(ProdRow1);
            $('#select_unit' + id).val(result[n].UnitId);
            $('#select_tax' + id).val(result[n].TaxId);
...
function ShowItemGetDeliveryOrder(result) {
    ...
    for (var l = 0; l < result.length; l++) {
       ...
       '<td id=' + 'delvordsubid' + slno + ' style=display:none;>' + result[l].DeliveryOrderSubId + '</td>' +
        ...
        '<td style=display:none;><input type="text" style="display:none;" id= ' + 'taxableamt' + slno + ' value= ' + result[l].TaxableAmount + '><input type="text" style="display:none;" id= ' + 'fctaxableamt' + slno + ' value= ' + result[l].FCTaxableAmount + '></td>' +
        '<td id=' + 'taxamt' + slno + '>' + parseFloat(result[l].TaxAmount).toFixed(Decimal) + '</td><input type="hidden" id="taxamt_' + slno + '" value=' + result[l].TaxAmount + '><input type="hidden" id="fctaxamt_' + slno + '" value=' + result[l].FCTaxAmount + '>' +
        '<td id=' + 'total' + slno + '>' + parseFloat(result[l].Amount).toFixed(Decimal) + '<input type="hidden" id="total_' + slno + '" value=' + result[l].Amount + '><input type="hidden" id="fctotal_' + slno + '" value=' + result[l].FCAmount + '><input type="hidden" id="bindelv' + slno + '" value=' + bins + '><input type="hidden" id="BCost6_' + slno + '" value=' + result[l].AreaId + '><input type="hidden" id="DelDate' + slno + '" value=' + result[l].InvDate + '><input type="hidden" id="Dellpo' + slno + '" value=' + result[l].LPONumber + '></td></tr>';
    }
...
function DeliveryOrderCustLoad(result) {
    disable_datatable('tblDeliveryOrdercust');
    var responseText = "<thead><tr><th style='width:90px;'><input type='checkbox' style='zoom:1.5;' checked id= 'SlNoHdCheckCustdo0' 'custom-control-input cz-bg-image-display' onchange='selectallcust3()'>&nbsp;&nbsp;&nbsp;Select</th><th>DO No</th><th>LPO No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th><th style='display:none'></th></tr>" +
                              "<tr><th style='width:90px;'>Select</th><th>DO No</th><th>LPO No</th><th>Customer</th><th>Address</th><th>Date</th><th>Salesman</th><th>Area</th><th>Currency</th><th style='display:none'></th><th style='display:none'></th></tr></thead><tbody>";
    for (var l = 0; l < result.length; l++) {
        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '>' +
...
function formrefresh() {
  ...
  //  $('#txtBillSlNo').prop("disabled", true);
    ...
    $('#select_unit0').val(0);
    $('#txtquantity0,#CalcQty0,#FractionQty0').val('');
  ...
  //  $('#txtBillSlNo').prop("disabled", true);
    ...
    $('#TotalAvgCost').val(0);
    $('#LineAvgCost0,#SalesMainId').val(0);
    $('#savedbillno').val('');
...
function productaddconfirm() {
    ...
    else if ($("#CalcQty0").val() == '' || parseFloat($("#CalcQty0").val() || 0) == 0) { //***** JOB CHANGE *****
        warningshow('Please Enter The Quantity', 'CalcQty0');
        $('#CalcQty0').select(); 
    }
    else if ($("#CalcQty0").val() > (parseFloat($('#txtstocktotloseqty0').val()) - pdtsumqty1) && (Negativebill == 'NO') && ($("#select_location0").find("option:selected").attr("name") == 0)) {//***** JOB CHANGE *****
        warningshow('Available Quantity is ' + (parseFloat($('#txtstocktotloseqty0').val()) - pdtsumqty1), 'CalcQty0');//***** JOB CHANGE *****
        $('#CalcQty0').select();
    }
    ...
    else if ((parseFloat($("#txtrate0").val()) < parseFloat($('#AvgCost0').val())) && (BelowCost == 'NO') && ($('#BelowCostFlag0').val() == 0)) {
...
function productadd() {
    ...
    var ProdRow1 = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $("#PrdtId0").val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")'  id=" + 'row' + id + " class='jsgrid-row'>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' name='prdaddqty' class='form-control text-center' id=" + 'CalcQty' + id + " value="//***** JOB CHANGE *****
        + parseFloat($('#CalcQty0').val()) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + "),QtyCal(" + id + ")' onkeydown=Focusnextgrid(event,'q'," + id + ") onkeypress=isNumber(event,this) onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input style=display:none type=text id=" + 'FractionQty' + id + " value=" + $('#FractionQty0').val() + " /></td>" +//***** JOB CHANGE *****
        "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' name='prdaddqty' class='form-control text-center' id=" + 'txtquantity' + id + " value="//***** JOB CHANGE *****
        + parseFloat($('#txtquantity0').val()) + " disabled style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + ")' onkeydown=Focusnextgrid(event,'q'," + id + ") onkeypress=isNumber(event,this) onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value="//***** JOB CHANGE *****
        + parseFloat($('#txtstocktotloseqty0').val()) + " style='display:none'></td>" +
        ...
        "<td class='jsgrid-cell jsgrid-align-center' style='width:3%;'><select style='background-color:white;height:30px' id=" + 'select_tax' + id + " class='form-control' onchange='ChangeTax(" + id + ",this)' onkeydown=Focusnextgrid(event,'t'," + id + ") onfocusout=focusoutgrid('t'," + id + ") onfocusin=focusingrid('t'," + id + ")>" + TaxSelect + "</select></td>" +
        "<td style='display:none' id=" + 'Bin' + id + ">" + bins + "</td>" +
        "<td style='display:none'><input type='text'  id=" + 'BelowCostFlag' + id + " value=" + $('#BelowCostFlag0').val() + " ><input type='text'  id=" + 'DelvLponum' + id + " ></td>" +
        "</tr>";
...
function JobAdd() {
    ...
    if ($.trim($('#productjob').val()) == "") {
        ...
        var prodjob = "<tr onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
            ...
            "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' class='form-control text-center' id=" + 'txtquantity' + id + " value=" + parseFloat($('#quantity_job').val() || 0) + " style='background-color:white;height:30px' onkeyup='amountcalculation(" + id + ")' onkeydown=Focusnextgrid(event,'q'," + id + ") onkeypress=isNumber(event,this) onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")></td>" +
...
function updaterow(id) {
    ...
    var b = parseFloat($('#CalcQty' + id).val());

    $("#txtdiscount" + id).val(isNaN(a) ? 0 : a);
    $('#txtrate' + id).val(isNaN(c) ? 0 : c);
    if (($('#CalcQty' + id).val() == '')) {
        warningshow('Please Enter Quantity', 'CalcQty' + id);
        $('#CalcQty' + id).select();
        editflag = 1;
        return false;

    }
    else if (($('#CalcQty' + id).val() == 0)) {
        warningshow('Quantity can not be Zero', 'CalcQty' + id);
        $('#CalcQty' + id).select();
        editflag = 1;
        return false;
    ...
    else if ((BelowCost == 'NO') && ($('#PrdtId' + id).val() != 0) && (($('#BelowCostFlag' + id).val()) == 0) && ($('#AvgCost' + id).val() != 0) && (parseFloat($('#txtrate' + id).val()) < parseFloat($('#AvgCost' + id).val()))) {
...
function copyrefresh() {
    ...
    $('#select_unit0').val(0);
    $('#txtquantity0,#CalcQty0').val('');
    $('#txtstocktotloseqty0,#SalesMainId').val(0);
    $('#txtrate0').val('');
    $('#txtdiscount0').val('');
    $('#select_tax0').val(0);
    $('#txtamnt0').val('');
...
function Showalerts(Status, billsrs, billsrlno) {
    ...
    if (Status == 1) {
        ...
        swal('Bill No : ' + billsrs + '-' + billsrlno + ' already exists!', "", "warning");
        $('.swal-button swal-button--confirm').focus();
    }
}
...
function bouncerowupdate(id) {
    ...
    $('#BounceQty' + id).val(parseFloat($('#BounceQty' + id).val()) || 0);
...
function bouncerowcancel(id) {
    ...
    $('#BounceQty' + id).val(parseFloat(brqty));
...
function autocheckqtystock(id) {
    if (Negativebill == 'NO' && ($("#autoItemLocngrid" + id).find("option:selected").attr("name") == 0)) {
        ...
        //***** JOB CHANGE *****
        if (pflg == 0) {                      //If Product is not already added in product grid-check qty<stockqty
            if (($('#autoItemIdgrid' + id).val() != 0) && (parseFloat($('#autoqtypr' + id).val()) > parseFloat($('#autoprStockqty' + id).val()))) {
        ...
        else                          //else check qty < stockqty-(sum of qty of pdts in pdt grid)
        {
            ...
            if ($("#autoqtypr" + id).val() > (parseFloat($('#autoprStockqty' + id).val()) - pdtsumqty1)) {
                warningshow('Available Quantity is ' + (parseFloat($('#autoprStockqty' + id).val()) - pdtsumqty1), 'autoqtypr' + id);
                $('#autoqtypr' + id).val('');
                $('#autoqtypr' + id).focus();
                return false;
            }
...
function autocheckratestock(id) {
    ...
    if ((BelowCost == 'NO') && ($('#BCost7_' + j).val() == 0) && ($('#autopravgcost' + id).val() != 0) && (parseFloat($('#autoprrate' + id).val()) < parseFloat($('#autopravgcost' + id).val()))) {
...
function autoaddgridproducts() {
    ...
    for (j = 1; j <= cnt ; j++) {
        if ($("#autoSlNoHeadCheckgrid" + j).is(":checked")) {
            if ((BelowCost == 'NO') && ($('#BCost7_' + j).val() == 0) && ($('#autopravgcost' + j).val() != 0) && (parseFloat($('#autoprrate' + j).val()) < parseFloat($('#autopravgcost' + j).val()))) {
                rateflg = 1;
                break;
            }
            else
                ...
                if ($("#autoSlNoHeadCheckgrid" + m).is(":checked")) {
                    ...
                    if ($("#autoItemunitgrid" + m).val() == '') {
                        ...
                        var ProdRows = "<tr onclick=GetproddetailsGrid(" + id + "),GetPrdtId(" + $('#autoItemIdgrid' + m).val() + ",'" + desc + "','" + code + "'," + 2 + ") onfocusout='updaterow(" + id + ")' id=" + 'row' + id + " class='jsgrid-row'>" +
                            ...
                            "<td class='jsgrid-cell jsgrid-align-center' style='width:2%'><input type='text' class='form-control text-center' name='prdaddqty' id=" + 'txtquantity' + id + " value=" + parseFloat($('#autoqtypr' + m).val()) + " style='background-color:white;height:30px' onkeyup='checkqty(" + id + "),amountcalculation(" + id + ")' onkeypress=isNumber(event,this) onkeydown=Focusnextgrid(event,'q'," + id + ") onfocusout=focusoutgrid('q'," + id + ") onfocusin=focusingrid('q'," + id + ")><input type='text' class='form-control text-center' id=" + 'txtstocktotloseqty' + id + " value=" + ($('#autoprStockqty' + m).val() || 0) + " style='display:none'></td>" +
