...
$(document).ready(function () {
    ...
    $("#btnprint").click(function (e) {
        if (($("#PatientId").val() || 0) != 0) {
            if (DischargePrintType == 'DEFAULT') { PrintthisDischargeSummary(0); }
...
function LoadDate() {
    ...
    $('#Dischargetime').val(moment().format('hh:mm'));
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.oList.length > 0) {
                ...
                GetPatientData1(result.oList, 1);
            }
        }
    });
}

function GetPatientData1(result, Flag) {
...
function ngOnDestroy(flg) {
    ...
    else if (flg == 1) { $("#myModal,#myModalnew").modal("hide"); }
    else if (flg == 2) {
        $("#myModal").modal("hide");
...
function ViewBillDetails() {
    ...
    if (flg == 0) { warningshow('Please Select Test'); }
    else {
        ...
        for (var k = 1; k <= parseInt(LCount); k++) {
            ...
            if ($('#Chkbil' + Id).prop('checked') == true && (Name == 'Chkbill')) {
                if (BillNo == '') { BillNo += $('#tdbillId' + Id).val(); }
                else { BillNo += ',' + $('#tdbillId' + Id).val(); }
            }
...
function CloseAdvice(flg) {
    ...
    else if (flg == -1) { $("#Divcomanddiagnosis").modal("hide"); }
    else if (flg == 1) { $("#DivAdvice").modal("hide"); }
    else if (flg == 2) {
        $('.advc').val('');
        $('#Medicine0').focus();
    }
...
function AddAdvice() {
    if ($.trim($('#Medicine0').val()) == '') { warningshow('Please Select Medicine', 'Medicine0'); }
    ...
    //{ warningshow('Please Select Days', 'Days0'); }
        ...
        var ProdRow = '<tr class="jsgrid-header-row MRow" height=30px; id="tdMRow' + Id + '">' +
            '<th onclick=Removemedrow(' + Id + ') class="jsgrid-control-field jsgrid-align-center" id="editheader" style="width: 5%;"><i class="icon-trash"></i></th>' +
...
function AddAdvicetoTable() {
    ...
    if (parseInt($('.MRow').length) == 0) { warningshow('Please Select Medicines', 'Medicine0'); }
    else {
        var ProdRow = '';
        for (var k = 1; k <= parseInt(ACount); k++) {
            var Id = parseInt(k); var Coma = ' , </br>'; if ($('#DischargeAdvice').text() != '') { Coma = ' , </br>'; }
            if ($('#Medicine' + Id).length != 0) {
                ...
                ProdRow += Id+'  .'+medname + '   ' + Dosage + Daily + Days + Coma;
                console.log(ProdRow)
            }
        }
...
function SaveAndUpdate(flag) {
    ...
    data.DischargeDate = $('#DischargeDate').val() + '-' + $('#Dischargetime').val();
    ...
    var x = $('#Allergies').val() + '$$' + $('#icd10').val() + '$$' + $('#MedPrevious').val() + '$$' + $('#emergencycase').val() + '$$' + $('#Clubbing').val() + '$$' + $('#Temperature2').val() + '$$' + $('#Oedema').val() + '$$' + $('#Pulse').val() + '$$' + $('#Pulse2').val() + '$$' + $('#Temperature').val() + '$$' + $('#Pallor').val() + '$$' + $('#Cavity').val() + '$$' + $('#Resp').val() + '$$' + $('#Resp2').val() + '$$' + $('#Cyanosis').val() + '$$' + $('#Icterus').val() + '$$' + $('#SPO22').val() + '$$' + $('#SPO2').val() + '$$' + $('#CNX').val() + '$$' + $('#PatAadhar').text();
    ...
    data.Variable1 = x;
    data.Variable2 = $('#invesmanual').val();
    data.Variable3 = '';
    data.Variable4 = '';
...
function GetRows(DischargeId) {
    ...
    $.ajax({
        ...
        data: data,
        success: function (result) {
            if (DischargeId == 0) { ShowDischargelist(result); }
            else { ShowDischargeGet(result); }
        }
    });
}
...
function ShowDischargeGet(result) {
    ...
    var Otherdetails = (result[0].Variable1).split("$$");
    console.log(Otherdetails)
    $('#PatAadhar').text(Otherdetails[19])
    ...
    if (result.length > 0) {
        ...
        $('#DischargeDate').val((result[0].DischargeDate).substring(0, 10));
        $('#Dischargetime').val((result[0].DischargeDate).substring(11, 20));
        ...
        //NEW ITEMS
        $('#Allergies').val(Otherdetails[0]);
        $('#icd10').val(Otherdetails[1]);
        $('#MedPrevious').val(Otherdetails[2]);
        $('#emergencycase').val(Otherdetails[3]);
        $('#Clubbing').val(Otherdetails[4]);
        $('#Temperature2').val(Otherdetails[5]);
        $('#Oedema').val(Otherdetails[6]);
        $('#Pulse').val(Otherdetails[7]);
        $('#Pulse2').val(Otherdetails[8]);
        $('#Temperature').val(Otherdetails[9]);
        $('#Pallor').val(Otherdetails[10]);
        $('#Cavity').val(Otherdetails[11]);
        $('#Resp').val(Otherdetails[12]);
        $('#Resp2').val(Otherdetails[13]);
        $('#Cyanosis').val(Otherdetails[14]);
        $('#Icterus').val(Otherdetails[15]);
        $('#SPO22').val(Otherdetails[16]);
        $('#SPO2').val(Otherdetails[17]);
        $('#CNX').val(Otherdetails[18]);
        

        $('#invesmanual').val(result[0].Variable2)
...
//Date Calculation
function GetNextReviewDaterecall(flg) {
    if (flg == 0) {
        var terms = ($('#AfterDays').val() || 0); terms = isNaN(terms) ? 0 : terms;
        var startdate = $('#DischargeDate').val();
        ...
        if (diffDays > 0) { $('#AfterDays').val(diffDays); }
        else { $('#AfterDays').val(''); }
    }
}
