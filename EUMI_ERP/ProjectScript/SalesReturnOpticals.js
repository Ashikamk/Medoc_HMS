...
function Billseriesload(result) {
    if (result.length == 0) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (result[i].BillType == 'SALESRETURNOPTICALS') {
                $("#HBillSeries").append("<option value='" + result[i].id + "' name='" + result[i].CurrentNo + "' >" + result[i].BillDescription + "</option>");
...
function SaveAndUpdate(flg) {
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesReturnInsertOpticals",
                data: data,
                success: function (result) {
                    if (result.oList.length > 0) {
        ...
        else if (flg == 2)                                            //Update  
        {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesReturnUpdateOpticals",
                data: data,
                success: function (result) {
                    if (result.oList.length > 0) {
                        Showalerts(result.oList[0].Status, result.oList[0].HBillNo);
...
function CallViewList() {
    ...
    $.ajax({
        type: "POST",
        url: '../SalesInvoice/SalesReturnGetListOpticals',
        data: data,
...
function copyrefresh(flg) {                                //flg:0 - Copy ,flg :1-View  ,flg:2 -New bill copy   ,flg:3 -Edit btn click
    ...
    if (flg != 3) { $('#btnedit,#btndelete,#btnacctran,#btnprint').hide(); }
    CheckEOD();
}
/*stephy4*/
function SalesGetCall(BillSeriesId, BillSlNo, DeptId, flg) {
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesReturnGetandGetsOpticals",
        data: data,
        success: function (result) {
            if (result.length > 0) {
