...
function EditInvoice(Flag) {
    ...
    if ((Flag == 1 && $("#opcost").prop("readonly") == true) || Flag == 0) {
...
function CheckEditInvoce(Flag) {
    if ($.trim($('#otp').val()) == '') {
        ...
        var Operation = '';
        if (Flag == 0)
            Operation = 'Item : ' + $("#code").val() + ' , OP Quantity Edit- OTP';
        else if (Flag == 1)
            Operation = 'Item : ' + $("#code").val() + ' , OP Cost Edit- OTP';
...
function CategoryLoad(result) {
    ...
    if (getQueryString('itemid') != null) {
        $('#code').val(getQueryString('itemid'))
        var itemid = {};
        itemid.ItemId = getQueryString('itemid');
        itemid.GroupId = getQueryString('grpid');
        itemid.CategoryId = getQueryString('cati');

        if (getQueryString('subgrpid') != null && getQueryString('subgrpid') == 1) {
            flg = 0;
            setTimeout(function () {
                GetItem(itemid.ItemId, itemid.GroupId, itemid.CategoryId), 5000
            });
        }
        else {
            flg = 1;
...
function VatLoad(result) {
    console.log(result)
    $("#vatcode").empty();
    $("#vatcode").append("<option value='0' style='display:none'>Select</option>");
    for (var i = 0; i < result.length; i++) {

        $("#vatcode").append("<option value='" + result[i].TaxId + "' name='" + result[i].TaxRate + "'>" + result[i].TaxName + "</option>");
    }
    $("#vatcode").val(result[0].TaxId);
    ItmeTaxId = result[0].TaxId;
...
function SubGroupLoad(result) {
    ...
    $("#sbgroup").empty();
    $("#sbgroup").append("<option value='0'>-Select-</option>");
    for (var i = 0; i < result.length; i++) {
        $("#sbgroup").append("<option value='" + result[i].SbgrpId + "'>" + result[i].SbgrpName + "</option>");
    }
   // $("#sbgroup").val($('#tempSubGrpId').val());
    $("#sbgroup").val(0);
...
function SaveAndUpdate(Flag) {
    ...
    //}
        ...
        data.MRP = $('#mrp').val();
        data.SellingPrice1 = $('#sellingprice1').val();
        data.SellingPrice2 = $('#sellingprice2').val();

        data.Model1 = $('#mod1').val();
        data.model2 = $('#mod2').val();
        ...
        data.StockOut = 0;
        data.DelFlag = Flag;
        data.Otherdescription = $('#otherdescription').val();



        data.modelm1 = $('#modelm1').val();
        data.modelm2 = $('#modelm2').val();
        data.modelm3 = $('#modelm3').val();
        data.modelm4 = $('#modelm4').val();
        data.modelm5 = $('#modelm5').val();
...
function thisformrefresh() {
    ...
    $('#BelowCostflg').prop("checked", false);
    $("#opcost").prop("readonly", true);
    $("#tab1").click();
    $('#btndelete').hide();
    generatebarcode();
    createQRCode();
    //file.value = file.defaultValue;
    $('#tblitemimport tr').remove();
...
//////////////////////////////////////////USE THIS FUNCTION ONLY FOR LISTING ALL ITEMS (NOT FOR GETTING SINGLE ITEM)  ////////////////////////////
...
function ShowItemlist(result) {
    //var EditPermission = 1;
    //if (EditPermission == 1) {
    //    alert('gfmkhnjklhkm')
    //    var columns = tblItem.column(0);
    //    console.log(columns)
    ...
    //}
    disable_datatable('tblItem', 'itemListButtonPlace');
    $('#Entry').hide();
    $('#listing').show();
    ...
    var table = $("#tblItem").DataTable({
        ...
        "columns": [
            ...
            { data: "TaxName", title: "Tax%" },
            { data: "SellingPrice", title: "Price" },
            { data: "AvgCost", title: "Cost" },
            { data: "Model1", title: "Model 1" },
            { data: "Model2", title: "Model 2" },
            { data: "Model3", title: "Model 3" },
            { data: "stock", title: "Stock" },
            { data: "Status", title: "Status" }

        ],
        "createdRow": function (row, data, index) {
            row.getElementsByTagName("td")[0].innerHTML = '<a  onclick="GetItem(' + data.ItemId + ',' + data.GroupId + ',' + data.CategoryId + ');">' + Editbutton + '</a>';
        },
        //dom: 'Blfrtip',
...
function ShowItemGet(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        $('#otherdescription').val(result[i].Otherdescription); 
        $('#unit').val(result[i].UnitId);
        $('#group').val(result[i].GrpId);
        $('#category').val(result[i].CategoryId);
        $('#modelm1').val(result[i].modelm1);
        $('#modelm2').val(result[i].modelm2);
        $('#modelm3').val(result[i].modelm3);
        $('#modelm4').val(result[i].modelm4);
        $('#modelm5').val(result[i].modelm5);

        //$('#tempSubGrpId').val(result[i].SbgrpId);
        //$('#tempSubCatId').val(result[i].SubCategoryId);
        ...
        $('#mod1').val(result[i].Model1);
        $('#mod2').val(result[i].Model2);
        $('#mod3').val(result[i].Model3);
        $('#maxqty').val(result[i].MaxQty);
        $('#minqty').val(result[i].MinQty);
        $('#bin_a').val(result[i].Bin_A);
        $('#bin_b').val(result[i].Bin_B);
        $('#bin_c').val(result[i].Bin_C);
        $('#bin_d').val(result[i].Bin_D);
        $('#bin_e').val(result[i].Bin_E);
        $('#bin_f').val(result[i].Bin_F);
        $('#bin_g').val(result[i].Bin_G);
        $('#bin_h').val(result[i].Bin_H);
        $('#size').val(result[i].Size);
        $('#weight').val(result[i].Weight);
        $('#length').val(result[i].Length);
        $('#width').val(result[i].Width);
        $('#thickness').val(result[i].Thickness);
        $('#density').val(result[i].Density);
        $('#spec').val(result[i].Specification);
        ...
        if (result[i].MultiPriceId == 0) { 
            $('#BelowCostflg').prop("checked", false);
        } else {
            $('#BelowCostflg').prop("checked", true);
        }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (ItemId == 0) {
                ...
                ShowItemGet(result);
                $('#Loading').hide();
                if (EditPermission == 1 && $('#searchcode').val() != ' ') {
                    $('#btnsubmit').hide();
                }
                else {
                    $('#btnsubmit').show();
