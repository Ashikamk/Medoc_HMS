...
function PrintthisBillHospitalSales(Type, Rowlen, Flag) {
  ...
  //TypeLine += '<tr><td>' + Leftalign + '</td><td>' + String.fromCharCode(27) + "E" + 'No  Description                             Qty     Rate       Amount' + String.fromCharCode(27) + "F" + '</td></tr>'
    ...
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Reg No :" + $('#HRegNo').val() + Space).substring(0, 35) +    (' Bill No   :' + BillNo + Space).substring(0, 25) + '</td></tr>'
    Line += '<tr><td>' + Leftalign + '</td><td>' + ("Patient:" + $('#HPatient').val() + Space).substring(0, 35) +  (' Date      :' + $('#HSalesDate').val() + Space).substring(0, 25) + '</td></tr>'
    ...
    for (var i = 1; i <= Rowlen; i++) {
        ...
        if ($.trim($('#Product' + Id).val()) != undefined && $.trim($('#Product' + Id).val()) != 'undefined' && $.trim($('#Product' + Id).val()) != ' ' && $.trim($('#Product' + Id).val()).length > 0) {
          ...
          //  Amnt = Amnt.substring(Amnt.length - 10, Amnt.length)
            ...
            if (i > 9) {
                Line += '<tr><td>' + Leftalign + '</td><td>'+ i+"  " + (Descr + Space).substring(0, 25)+"   " + (Batch + Space).substring(0, 6) + "  " + Exp.substring(0, 6) + Qty.substring(Qty.length - 3, Qty.length) + " " + Rate + " " + Amnt + '</td></tr>';    //+ Mrp.substring(Mrp.length - 10, Mrp.length) + " " + Taxable.substring(Taxable.length - 10, Taxable.length) + " " + Tax.substring(Tax.length - 10, Tax.length) + " "
               // Line += '<tr><td>#</td></tr>';
            }
            else {
