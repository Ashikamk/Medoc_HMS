    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.oList.length == 0) {
                ...
                $('#PINo_A').val(result.oList[0].MRVSlNo);
                NextInvoiceNo = result.oList[0].MRVSlNo;
                $('#ImpTax').val(result.oList[0].ImportPurTax);
                $('#accountdescription').val('Other Cost Against Purchase InvNo: ' + result.oList[0].PurSlno);
                NextInvoiceNo = parseInt(result.oList[0].MRVSlNo || 0);

                $('#PINo_S').val('PE-' + result.oList[0].MRVSlNo)
...
function OKProductAdd() {
    ...
    var ProductRow = '<tr id="MTr_' + Id + '" onfocusout="UpdateRow(' + Id + ')">' +
        ...
        '<td text-align: center; width="5%"><div class=custom-control custom-checkbox" style="float:left;"><input type = "checkbox" checked class="custom-control-input"  id="txxexc_' + Id + '"  onchange="CalcAmt(),ResetBillDiscount()" ><label class="custom-control-label" for="txxexc_' + Id + '"></label></div >   </td>' +
...
function CalcAmount(Id) {
    ...
    //DDDH
    if ($("#txxexc_" + Id).prop("checked") == true) {
...
function OKSavePurchase() {
    ...
    for (var i = 1; i <= GridLength; i++) {
        ...
        if ($("#txxexc_" + i).prop("checked") == true) { Purtaxflag = 1 } else { Purtaxflag = 0; }
    ...
    if (oArray.length > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "../Pharmacy/HMS_OpeningPurchaseInsert",
            data: data,
            success: function (result) {
...
function GetBillPrevousornext(Value) {
    ...
    if ((SlNo <= 0) || (SlNo >= NextInvoiceNo)) {
        ...
        $.ajax({
            type: "POST",
            url: "../Pharmacy/HMS_PurchaseGetandGetsOpening",
            data: data,
            success: function (result) {
                formrefresh(1);
                PurchaseGets(result);
...
function PurchaseGets(result) {
    ...
    if (result.length > 0) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (result[i].JobNo == 1) { chkflag = 'Checked'; } else { chkflag = ''; }
            ...
            var ProductRow = '<tr id="MTr_' + Id + '" onfocusout="UpdateRow(' + Id + ')">' +
                ...
                '<td text-align: center; width="5%"><div class=custom-control custom-checkbox" style="float:left;"><input type = "checkbox" ' + chkflag + ' class="custom-control-input"  id="txxexc_' + Id + '"  onchange="CalcAmt(),ResetBillDiscount()" ><label class="custom-control-label" for="txxexc_' + Id + '"></label></div >   </td>' +
...
function OKDeletePurchase() {
    ...
    $.ajax({
        type: "POST",
        url: "../Pharmacy/HMS_PurchaseDeleteopening",
        data: data,
        success: function (result) {
...
function OKUpdatePurchase() {
    ...
    if (oArray.length > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "../Pharmacy/HMS_PurchaseUpdateOpening",
            data: data,
            success: function (result) {
...
function GetPurchaseList() {
    ...
    $.ajax({
        type: "POST",
        url: "../Purchase/PurchaseInvoiceListOpening",
        data: dataPI,
        success: function (result) {
