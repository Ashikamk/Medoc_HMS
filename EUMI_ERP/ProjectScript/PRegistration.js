...
function formrefresh() {
    ...
    $('#btnsubmit').show();
    LoadDate();
    $('#btndelete,#btnprint,#btntoken,#printId,#btnsticker,#printpresId').hide();
    $("#RegSeries").prop("disabled", true);
    $('select').each((i, item) => {
        var $item = $(item);
...
function PrintRegistration() {
    ...
    if (parseFloat($("#RegFee").val() || 0) > 0) {
        Rowlen++
        myWindow.document.write('<tr><td style="font-family:tahoma;">' + Rowlen + '</td> <td  style="font-family:tahoma;"colspan=7>Registration Fees</td><td style="font-family:tahoma;" align=right>' + addCommas(parseFloat($("#RegFee").val() || 0).toFixed(Decimal)) + '</td></tr>');
    }
    if (parseFloat($("#ConsultFee").val() || 0) > 0) {
        Rowlen++
        myWindow.document.write('<tr><td style="font-family:tahoma;">' + Rowlen + '</td><td  style="font-family:tahoma;"colspan=7>Consultation Fees</td><td style="font-family:tahoma;" align=right>' + addCommas(parseFloat($("#ConsultFee").val() || 0).toFixed(Decimal)) + '</td></tr>');
    }
    ...
    if ($("#OtherFee").val() != 0) {
        myWindow.document.write('<tr><td></td><td style="font-weight: bold;font-size: 14px;" align=right>Disc %     :</td><td style="font-weight: bold;font-size: 14px;" align=right >' + addCommas(parseFloat($("#OtherFee").val() || 0).toFixed(Decimal)) + '</td></tr>');
        myWindow.document.write('<tr><td>This is a computer generated Bill.</td><td style="font-weight: bold;font-size: 14px;" align=right>Net Total  :</td><td style="font-weight: bold;font-size: 14px;" align=right >' + addCommas(parseFloat(TotalAmount).toFixed(Decimal)) + '</td></tr>');
    }
    else {
...
function Printprescription() {
    ...
    var ageParts = [];
    var yrs = parseInt($('#Age').val()) || 0;
    var mos = parseInt($('#Age1').val()) || 0;
    var dys = parseInt($('#Age2').val()) || 0;

    if (yrs > 0) ageParts.push(yrs + (yrs > 1 ? ' Years' : ' Year'));
    if (mos > 0) ageParts.push(mos + (mos > 1 ? ' Months' : ' Month'));
    if (dys > 0) ageParts.push(dys + (dys > 1 ? ' Days' : ' Day'));

    var ageSex = ageParts.join(', ') + '/' + $('#PGender :selected').text();
    var regNo = $('#RegSeries option:selected').text() + '-' + $('#RegNo').val();
    var visitDate = $('#RegDate').val() || '';
    myWindow.document.write('<!DOCTYPE html>');
    myWindow.document.write('<html>');
    myWindow.document.write('<head>');
    myWindow.document.write('<title>Prescription</title>');
    ...
    myWindow.document.write('}');
    myWindow.document.write('.blank-space {');
    myWindow.document.write('    height: 3.9cm;');
    myWindow.document.write('}');
    myWindow.document.write('.info-row {');
    myWindow.document.write('    display: flex;');
    myWindow.document.write('    flex-wrap: wrap;');
    myWindow.document.write('    align-items: center;');
    myWindow.document.write('    justify-content: space-between;');
    myWindow.document.write('    gap: 20px 45px;');
    myWindow.document.write('    font-size: 15px;');
    myWindow.document.write('    padding: 3px 0 3px 25px;');
    myWindow.document.write('}');
    myWindow.document.write('.info-row .field {');
    myWindow.document.write('    display: flex;');
    myWindow.document.write('    align-items: center;');
