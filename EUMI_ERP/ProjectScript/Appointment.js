...
$(document).ready(function () {
    ...
    //ge$("#Branch").val(window.LoggedInUserArray[4]);
...
function LoadDates() {
    $('#PDATE,#PDOB').daterangepicker({
        ...
        maxDate: new Date(new Date().getFullYear(), new Date().getMonth()+5, new Date().getDate()),
...
function GetRows(AppointmentId) {
...
function ShowTermlist(result) {
        $('#Entry').hide();
        $('#listing').show();
        disable_datatable('tblAppointment');
        var responseText = "<thead><tr><th width=1%;align=center>Sl#</th><th width=4% >RegId</th><th  width=20%>Name</th><th  width=10%>MR #</th><th width=15% class='text-left' >Doctor</th><th width=15%>Date</th><th width=10%>Time</th><th width=10% class='text-left' >Mobile No:</th><th width=10% class='text-left' >Token No:</th><th width=3%>Edit</th></tr></thead><tbody>";
        for (var i = 0; i < result.length; i++) {
            var slno = parseInt(i + 1);
            responseText += '<tr><td align=center>' + slno + '</td><td>' + result[i].AppointmentId + '</td><td>' + result[i].FirstName + '</td><td>' + result[i].LastName + '</td><td align=left>' + result[i].Doctor + '</td><td align=left>' + result[i].AppointmentDate + '</td><td align=left>' + result[i].AppointmentTime + '</td><td align=left>' + result[i].Contact + '</td> <td align=left>' + result[i].Branch + '</td>   <td onclick="GetRows(' + result[i].AppointmentId + ')" align=center width=10px bgcolor="#00ffff "><a>' + Editbutton + '</a></td></tr>';
        }
        $('#tblAppointment').html(responseText + '</tbody>');
        datatableWithsearch1('tblAppointment');
    }
