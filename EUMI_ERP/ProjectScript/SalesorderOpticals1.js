...
function SelectPayType() {
 SalesPayType = 'UPI Pay';
 if (SalesPayType == 'UPI Pay') {
        $('#PayType').val(3);
    }
...
function LoadAutoComplete() {
    ...
    $("#HPatient").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $('#OptPName').val(ui.item.PatientName)

            
            $('#OptPAge').val(ui.item.DOB)

            if (ui.item.Gender == 1) { $('#OptPGender').val('Male'); }
            else if (ui.item.Gender == 2) { $('#OptPGender').val('Female'); }
            else if (ui.item.Gender == 3) { $('#OptPGender').val('Others'); }

           // $('#OptPGender').val(ui.item.Gender)
            $('#OptPMobile').val(ui.item.Contact)
            $('#OptPEmail').val()
            $('#OptPAddress').val(ui.item.address)
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.oList.length > 0) {
                        ...
                        GetLastRevisitDetails(result.oList[0].PatientId, result.oList[0].RevisitId);
                        GethealthDetails(result.oList[0].RevisitId, result.oList[0].PatientId) 
...
function ApplyPDetails() {
   
  
    

    var OptPName = document.getElementById('OptPName').value;
    document.getElementById('HPatient').value = OptPName;

    var OptPMobile = document.getElementById('OptPMobile').value;
    var OptPAddress = document.getElementById('OptPAddress').value;
    var OPaddresss = OptPMobile + '## ' + OptPAddress;

    document.getElementById('custaddress1').value = OPaddresss;
            ...
            if (result.oList.length > 0) {
                ...
                if (OPCaseSheet != 0) {
                    if (($('#CaseSheetId').val() || 0) == 0)
                        GetRowsopticalsinfo(OPCaseSheet, "CASE", 1);
...
function GetRowsopticalsinfo(CaseSheetId, Status, Flag) {
                            ...
                            response($.map(data, function (item) {
                                return ({
                                    ...
                                    //label3: item.ItemExpiry,
                                      ColCount: '7',
                                    label: (item.Batch).split('##')[0],
                                    label1: (item.Batch).split('##')[1],
                                    label2: (item.Batch).split('##')[2],
                                    label3: (item.Batch).split('##')[3],
                                    label4: item.ProductDesc,
                                    label5: item.Company,
                                    label6: item.ItemExpiry,
                                    label7: item.Stock,
                                    label8: item.Sellingrate,
                                    label9: item.Mrp,
                                  ...
                                  headers: ["Batch", "Color", "Size", "Material", "Description", "Company", "ItemExpiry", "Stock", "Selling Price", "Mrp"]
...
function SerachSalesBill() {
    $("#HBillNoCopy").autocomplete({
        ...
        source: function (request, response) {
            ...
            data.DeptId = ERPDeptId;
            $.ajax({
                url: '../SalesInvoice/SerialNoSearchOPTICALSORDER',
                type: "POST",
                data: data,
                dataType: "json",
                success: function (data) {
...
function SaveAndUpdate(flg) {
    ...
    for (var i = 1; i <= RowCount; i++) {
        if ($('#Product' + i).val() != undefined) {
            ...
            oArray.push({
                ...
                'Remarks': $("#Remarks").val() + '##' + $("#custaddress1").val() + '##' + $("#OptPName").val() + '##' + $("#OptPAge").val() + '##' + $("#OptPGender").val() + '##' + $("#OptPMobile").val() + '##' + $("#OptPEmail").val() + '##' + $("#OptPAddress").val() + '##' + $("#dSPHR").val() + '##' + $("#dnvsphr").val() + '##' + $("#dCYLR").val() + '##' + $("#dnvcylr").val() + '##' + $("#dAXISR").val() + '##' + $("#dnvaxisr").val() + '##' + $("#dADDR").val() + '##' + $("#dnvaddr").val() + '##' + $("#dSPHL").val() + '##' + $("#dnvsphl").val() + '##' + $("#dCYLL").val() + '##' + $("#dnvcyll").val() + '##' + $("#dAXISL").val() + '##' + $("#dnvaxisl").val() + '##' + $("#dADDL").val() + '##' + $("#dnvaddl").val() + '##' + $("#Interpupilary").val() + '##' + $("#RefractiveAid").val() + '##' + glass + '##' + Polycarbonate + '##' + CR39 + '##' + Unifocal + '##' + DBifocal + '##' + Kryptok + '##' + Progessive + '##' + OfficeLens + '##' + Tint + '##' + Photochromic + '##' + ARCC + '##' + Polarised +  '##' + $("#GLRemarks").val(),
                'SubId': 0,
    ...
    if (oArray != "") {
        ...
        if (flg == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesInvoiceInsertOPTICALSORDER",
                data: data,
                success: function (result) {
        ...
        else if (flg == 2)                                            //Update  
        {
            ...
            $.ajax({
                type: "POST",
                url: "../Hospital/HMS_SalesInvoiceUpdteOPTICALSORDER",
                data: data,
                success: function (result) {
...
function CallViewList() {
    ...
    $.ajax({
        type: "POST",
        url: '../SalesInvoice/SalesInvoiceGetListOPTICALORDER',
        data: data,
        success: function (result) {
...
function SalesGetCall(BillSeriesId, BillSlNo, DeptId, flg) {
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesGetandGetsOPTICALORDER",
        data: data,
        success: function (result) {
            if (result.length > 0) {
...
function SalesGetandGets(result) {
    ...
    $('#dADDL').val(Qr[25]);
    $('#dnvaddl').val(Qr[26]);
    $('#Interpupilary').val(Qr[27]);
    $('#RefractiveAid').val(Qr[28]);
...
function DeleteBill(Flag) {      //Flag=0:Delete Contract  
    ...
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_SalesInvoiceDeleteOPTICALORDER",
        data: data,
        success: function (result) {
...
function mrpsellingratecalculation() {
    ...
    var blns = parseFloat(Mrptotal - SelTotal).toFixed(2);
    var Cash = parseFloat($("#BAmount").val() || 0);
    var Upi = parseFloat($("#UPISpecialFeeAmt").val() || 0);
    var Card = parseFloat($("#SpecialFeeAmt").val() || 0);
    var Result = (Cash + Upi + Card); 
    var Total = parseFloat($('#BaseTextTotalProc').text() || 0) - parseFloat(Result || 0)
    $('#mrpvalue').text('ADV PAID:' + (Result).toFixed(2));
    $('#savingvalue').text('BAL AMT:' + (Total).toFixed(2));
...
function PrintthisBillHospital(Type, Count, Flag) {
    ...
    if (PrintType == 'HMSSALESLOCAL')
        ...
        PrintthisBillWindowsHMS('SALESORDEROPTICALS', Count, Flag, 0);
...
function PatientDetailShow() {
    $("#PatientDetailsPopup").modal("show");
    $("#PatientDetailsPopup").appendTo("body");
}
function OptDetShow() {
    $("#OptDetPopup").modal("show");
    $("#OptDetPopup").appendTo("body");
}
...
function printglassprescription() {
    ...
    myWindow.document.write('<tr><td width=5% align=left>MR.No#</td>        <td  width=2%>:</td>  <td  width=30%>' + $("#HRegNowithseries").val() + '</td>           <td width=20% align=left></td>           <td width=5% align=left>Date</td>     <td  width=2%>:</td> <td  width=30%>' + $("#HSalesDate").val() + '</td> </tr>');
    myWindow.document.write('<tr><td width=5% align=left>Name</td>          <td  width=2%>:</td>  <td  width=30%>' + $("#HPatient").val() + '</td>      <td width=20% align=left></td>           <td width=5% align=left>Doctor#</td>    <td  width=2%>:</td> <td  width=30%>' + $("#UserDetsss").text() + '</td></tr>');
    myWindow.document.write('<tr><td width=5% align=left></td>    <td  width=2%></td>  <td  width=30%></td>      <td width=20% align=left></td>           <td width=5% align=left></td>       <td  width=2%></td> <td  width=30%></td> </tr>');
