            ...
            if ($('#Chkbilrs' + Id).prop('checked') == true && (Name == 'Chkbillrs')) {
                ...
                if ((TestId == 0) || (TestId != $('#ttdtestmainId' + Id).val())) {
                    ...
                    var Row = '<tr class="jsgrid-header-row RRow" style="background-color:#f1f1f1;font-weight:bold">' +
                     '<td class="text-center" style="width: 4%;" > <input type="checkbox" id="tstCHECK' + SId + '"></td>' +
        ...
        for (var k = 0; k < result.length ; k++) {
            ...
            var ProdRow = '<tr class="jsgrid-header-row RRow">' +
                   '<td class="text-center" style="width: 4%;" > <input type="checkbox" id="tstCHECK' + SId + '"></td>' +
                      ...
                      '<td class="text-center" style="width: 8%;" id="tstResult' + SId + '"> ' + result[k].MaxValue + '</td>' +
                      '<td class="text-center" style="width: 8%;" id="tstNormal' + SId + '"> ' + result[k].MinValue + '</td>' +
    ...
    else
    {
        ...
        for (var k = 1; k <= parseInt(ACount) ; k++) {
            ...
            if ($('#Medicine' + Id).length != 0) {
                ...
                ProdRow += Coma + Dosage + Daily + Days;
            }
        }
...
function GetRows(Ip,Regno) {
    ...
    data.DischargeId = 0;
    data.DeptId = ERPDeptId;
    data.Status = 'IP_Number= ' + Ip + ' and  RegNo= ' + Regno
    Regno;
    $.ajax({
        type: "POST",
        url: "../Hospital/HMS_DischargeSummaryGetandGetscopy",
        data: data,
        success: function (result) {
            if (DischargeId == 0)
...
{
    Medlen = result.length;
    var responseText1 = "<thead><tr><th></th><th>Medicine Advices</th></tr></thead><tbody>";
    for (var i = 0; i < result.length; i++) {
        responseText1 += '<tr><td align=center width="20px"><input id="chk' + i + '" type="checkbox"></td><td id="selectmedicine'+i+'">' + result[i].medicine + '</td></tr>'
    }
