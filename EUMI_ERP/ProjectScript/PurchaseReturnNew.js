...
var i = 1; var x = 1; var TId = ""; var SaveFlag = 0;
...
$(document).ready(function () {
    ...
    if (getQueryString('slno') != null) {
        var data = {};
        data.PRNo = getQueryString('slno');
        data.DepartmentId = getQueryString('dept');
        $.ajax({
            type: "POST",
            ...
            success: function (result) {
                ...
                $('#purchaseinvoiceno1').val(data.PRNo);
                $('#btnnew').focus();
...
function SavePurchaseReturn() {
    $('#rate').val(parseFloat($('#rate').val() || 0).toFixed(Decimal));
    CalcAmt();

    $("#CustType").val($("#CustType").val().toUpperCase());

    if ($('#tblpurchaseinvoice tr').length == 0) {
        warningshow('No Products Added', 'product_0');
    }
    else if ($('#supplierId').val() == 0 || $("#CustType").val() == 0) {
        warningshow('Please Select Supplier', 'suppliername');
        $("#suppliername").select();
    }
    else if ($("#CustType").val() != 'S' && $("#CustType").val() != 'CA') {
        warningshow('Please Select Supplier', 'suppliername');
        $("#suppliername").select();
    }
...
function OKPurchaseReturn() {
    ...
    //}
        ...
        if (oArray != "") {
            ...
            $.ajax({
                type: "POST",
                url: "../Purchase/PurchaseReturnInsertandUpdate",
                data: data,
                success: function (result) {
...
function UpdatePurchaseReturn() {
    ...
    //}
        ...
        $('#Confirmflag').val('purchasereturnupdate'), $('#ConfirmRowId').val(0)
        $('#confirmmessage').text('Do you want to Update the Purchase Return?')
        $('#confirm').show();
        $('#confirmOk').focus();
...
function OKPurchaseReturnUpdate() {
...
function CurncyLoad(result, a) {
    ...
    $("#rate").val(parseFloat($("#currency").find("option:selected").attr("name") || 0).toFixed(Decimal));
...
function ConfirmboxResult(Result, status, rowid) {
    ...
    else if (Result == 'true' && status == 'puchasereturndelete') {
        EditInvoice(1);
    }
    else if (Result == 'true' && status == 'purchasereturnupdate') {
        OKPurchaseReturnUpdate();
    }
    $('#confirm').fadeOut();
...
function formrefresh(RefreshFlag) {
    ...
    SaveFlag = 0;
    $('#LoadingSmall').hide();
    $('#copypurchaseinvo,#transferPI').val('');
    //$('#tour1').hide();
    $('#paytype').val(2);
    ...
    $('#jobid').val(0)
    $('#product_0').val('');
    $('#btndelete,#btnedit,#btnsaveedit').hide();
    $('#locn_0').val(ULocId);
    $('#unit').val(0);
    $('#quantity').val('');
    $('#txtrate').val('');
...
function PurchaseReturnGets(result) {
    ...
    if (result.length > 0) {
        ...
        $('#UserName').text('User : ' + (result[0].UserName)); 

        $('.distxtbox').val("0.00");
        $('.distxtbox').prop("disabled", true);
        for (var n = 0; n < result.length; n++) {

            $('#CustAccCode').val(result[n].AccCode);
            $('#CustType').val(result[n].CustType);

            $('#transferPI,#PI').val(result[n].PINo);
            $('#purchaseinvoiceno1').val(result[n].PRNo);
            ...
            $('#rate').val(parseFloat(result[n].CurrencyRate || 0).toFixed(Decimal));
            ...
            var ProdEditRow = "<tr class='jsgrid-row' id='row_" + id + "'>" +
                "<td id='edit_" + id + "' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:3%;' ><input class='jsgrid-button jsgrid-edit-button' type='button' title='Edit' onclick='Editrow(" + id + ")'><input class='jsgrid-button jsgrid-delete-button'  type= button onclick='DeleteRow(" + id + ")'  title= Delete ></td>" +
                "<td id='update_" + id + "' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:3%;display:none' ><input class='jsgrid-button jsgrid-update-button' type='button' title='Update' onclick='UpdateRow(" + id + ")'><input class='jsgrid-button jsgrid-cancel-edit-button' type='button' title='Cancel edit' onclick='CancelEdit(" + id + ")'></td>" +
                "<td id='" + id + "' class='jsgrid-cell'  style= 'width:3%;text-align:center' >" +
                ...
                parseFloat(0).toFixed(Decimal) + "' ></td>" +
                ...
                TaxSelect + "</select><input type='hidden' id='taxpercentage_" + id + "' value=" + result[n].TaxRate + "></td>" +
                ...
                result[n].TotalAmount.toFixed(Decimal) + "'><input type='hidden' id='baseamount_" + id + "' class='form-control'   disabled value='" + parseFloat($("#baseamount_0").val()).toFixed(Decimal) + "'><input type='hidden' id='basetaxableamount_" + id + "' class='form-control'   disabled value='" +
                ...
                parseFloat(0).toFixed(Decimal) + "'></td>" +
                "<td id='col_2' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;' > <select id='locn_" + id + "' class='form-control' disabled style='height:30px;background-color:white' >" +
                 LocationSelect + "</select></td>" +
                 "<td id='col_13' class= 'jsgrid-cell jsgrid-align-left'  style= 'width:250px;display:none;' ><input type='hidden' id='jobcodeid_" + id + "' value='"
...
function CheckeDeleted() {
    var datax = {};
    datax.BillNo = 'PR';
    datax.SlNo = $('#purchaseinvoiceno1').val();
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (status == 1) {
                //$('#popupmessageInfo').text('Return No: ' + slno + ' Cancelled!!!');
                //$('#Infopopup').show();
                //setTimeout(function () { $('#Infopopup').hide(); }, 3000);
                swal('Return No-' + slno + ' ', "Cancelled!!!", "error");
        ...
        $.ajax({
            ...
            success: function (result) {
                for (var i = 0; i <= result.oList.length; i++) {
                    ...
                    GetCurrency(id);
                    $('#rate').val(parseFloa(data.CurrencyRate || 0).toFixed(Decimal));
                }
            }
...
function ShowPISupplierList(result) {
    ...
    var responseText = "<thead><tr><th style='width:90px;'> <input type='checkbox' id= 'SlNoHeadInvoice' 'custom-control-input cz-bg-image-display' onchange='SelectAllInvoiceSup()'>&nbsp;&nbsp;&nbsp;Select</th><th>Serial No</th><th>Invoice No</th><th>Invoice Date</th><th>Supplier</th><th>Currency</th><th>Purchase Type</th><th>Remarks</th><th>Grand Total</th></tr>" +
        "<tr><th> </th><th>SerialNo</th><th>InvoiceNo</th><th>Date</th><th>Supplier</th><th>Currency</th><th>PurchaseType</th><th>Remarks</th><th>Grand Total</th></tr></thead><tbody>";
    for (var l = 0; l < result.length; l++) {

        var slno = parseInt(l + 1);
        responseText += '<tr id=' + "row" + slno + '><td style="width:90px;" ><input type="checkbox"  id=' + 'SlNoInvoiceCheck' + slno + '  "custom-control-input cz-bg-image-display" style="align:center"></td>' +
...
function InvoiceProductAdd() {
    ...
    for (m = 1; m <= row; m++) {
        if ($("#SlNoCheckInvoiceItem" + m).is(":checked")) {
            ...
            $('#rate').val(parseFloat($('#curr_rate1' + m).val() || 0).toFixed(Decimal));
            ...
            var ProdEditRow = "<tr class='jsgrid-row' id='row_" + id + "'>" +
                ...
                "<td id='td_" + id + "' class='jsgrid-cell'  style= 'width:3%;text-align:center' >" +
                    ...
                    "<td id='col_4' class='jsgrid-cell'  style='width:6%;display:none;' > <select id='unit_" + id + "' onchange='CheckFOC(" + id + ",this)' class='form-control' disabled style='height:30px;background-color:white' >" +
                    UnitSelect + "</select><input type='hidden' id='txtunit_" + id + "' class='form-control' /></td>" +
                    "<td id='col_5' class= 'jsgrid-cell jsgrid-align-center'  style='width:6%;' ><input type='text' id='quantity_" + id + "' class='form-control' disabled style='height:30px;background-color:white' value='" +
                    parseInt(qty) + "' onkeyup='CalcAmount(" + id + ")' onkeypress='isNumberInt(event,this)' ></td>" +
                    "<td id='col_6' class= 'jsgrid-cell jsgrid-align-center'  style='width:6%;' ><input type='text' id='txtrate_" + id + "' class='form-control' disabled style='height:30px;background-color:white'  value='" +
                    parseFloat(rate).toFixed(Decimal) + "' onkeyup='CalcAmount(" + id + ")' onkeypress='isNumber(event,this)'><input type='hidden' id='baserate_" + id + "' class='form-control' value='' ></td>" +
                    "<td id='col_7' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;display:none;' ><input type='text' id='discount_" + id + "' class='form-control' disabled style='height:30px;background-color:white'  value='" +
                    parseFloat(discount).toFixed(Decimal) + "' onkeyup='CalcAmount(" + id + ")' onkeypress='isNumber(event,this)'></td>" +
                    "<td id='col_8' class='jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;' ><input type='text' id='txttaxable_" + id + "' class='form-control' style='height:30px;background-color:white'  value='" +
                    parseFloat(taxableamt).toFixed(Decimal) + "' disabled></td>" +
                    "<td id='col_9' class='jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;display:none;' > <select  class='form-control' disabled style='height:30px;background-color:white' id='tax_" + id + "' onchange='ChangeTax(" + id + ",this)' >" +
                    TaxSelect + "</select><input type='hidden' id='taxpercentage_" + id + "' value=" +
                    taxrate + "></td>" +
                    "<td id='col_10' class= 'jsgrid-cell jsgrid-control-field jsgrid-align-center'  style='width:6%;' ><input type='text' id='txttax_" + id + "' class='form-control' style='height:30px;background-color:white' disabled value='" +
                    parseFloat(taxamt).toFixed(Decimal) + "'></td>" +
    ...
    var CheckIntPINumber = [];
    CheckIntPINumber = PINumber.split(',');
    if (CheckIntPINumber.length == 1)
        $('#transferPI').val(PINumber);
    $('#proddiv').animate({ scrollTop: 5000 }, 900);
...
function Showalerts(Status, no) {
    if (Status == 1) {
        ...
        formrefresh(0);
        swal('Purchase Return No-' + no + ' ', "Updated Successfully", "success");
        $('.swal-button swal-button--confirm').focus();
        ...
        createnew();
        swal('Purchase Return No-' + no + ' ', "Deleted", "error");
        $('.swal-button swal-button--confirm').focus();
...
function SalesTransLoad(result) {
    ...
    var ProdRow = "<thead><tr class='text-left'><th>SalesInvoice</th><th>Date</th><th style='width:27%'>AccountName</th><th style='width:6%'>Quantity</th><th>Price</th><th>Location</th><th style='width:15%'>SalesMan</th><th>Department</th></tr>" +
                             "<tr class='text-left'><th>SalesInvoice</th><th>Date</th><th style='width:27%'>AccountName</th><th style='width:6%'>Quantity</th><th>Price</th><th>Location</th><th style='width:15%'>SalesMan</th><th>Department</th></tr></thead><tbody>";
